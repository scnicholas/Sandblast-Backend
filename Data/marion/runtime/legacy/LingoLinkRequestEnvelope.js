 ...
 *
 * Standard input contract for LingoLink.
...
function createLingoLinkRequestEnvelope(input = {}) {
  const text = normalizeText(input.text || input.originalText || input.message);

  const envelope = {
    ok: Boolean(text),
    requestId: input.requestId || input.marionRequestId || generateRequestId('marion_lingolink'),
...
function validateLingoLinkRequestEnvelope(envelope = {}) {
  const errors = [];
