...
/**
 * LingoLinkUnknownLanguageAlert
 *
 * Purpose:
 * Builds Marion-safe alert packets when LingoLink detects an unknown,
...
const ALERT_VERSION = "nyx.lingolink.unknownLanguageAlert/0.1";
...
function buildUnknownLanguageAlert(payload = {}, options = {}) {
  ...
  const alertId = `lingolink_unknown_${createdAt}_${hashText(
  ...
  return {
    ...
    metadata: {
      source: "LingoLinkUnknownLanguageAlert",
      createdAt,
      candidates: safeObject(languageMeta.candidates),
      sourceReason: safeString(languageMeta.reason),
      gateway: safeString(sourcePayload.source || sourcePayload.gateway || "LingoLink")
    },
