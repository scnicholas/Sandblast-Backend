...
(async () => {
  ...
  assert(contract && /13\.2/.test(contract.version), "V13.2 contract not loaded");
  ...
  assert(direct.executionBlocked === true, "direct operational command must be executionBlocked");
  ...
  assert(advisory.ok !== false, "safe advisory prompt should be recovered to ok=true");
  ...
  assert(advisory.recommendationOnly === true, "advisory path must be recommendationOnly");
  assert(advisory.advisoryOnly === true, "advisory path must be advisoryOnly");
  assert(advisory.executionSemanticGuarded === true, "advisory path must be semantic-guarded");
  ...
  console.log("[PASS] V13.2 private execution semantic authority regression passed.");
  process.exit(0);
})().catch((err) => {
  console.error("[FAIL] V13.2 regression failed.");
