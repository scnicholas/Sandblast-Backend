...
function priority9IJConfidenceScore(input,base){var text=priority9IJStr(input&&typeof input==="object"?(input.text||input.prompt||input.userText||input.message||""):input);var src=[text,priority9IJCollect(input),priority9IJCollect(base)].join(" ");if(priority9IJIs9JActivationText(text)||priority9IJIs9JActivationText(src)){return {...priority9IJObj(base),domain:"execution_context",topDomain:"execution_context",confidence:0.997,margin:0.997,clarify:false,priorityLane:"Priority 9J",priority9JProactiveOperationalGuidance:priority9JStateFrom(src,1)};}if(priority9IJIs9IActivationText(text)||priority9IJIs9IActivationText(src)||priority9IJIsPressureText(text)){return {...priority9IJObj(base),domain:"execution_context",topDomain:"execution_context",confidence:0.997,margin:0.997,clarify:false,priorityLane:"Priority 9I",priority9IAdaptiveSituationalReasoning:priority9IStateFrom(src,1)};}return base;}
["scoreDomainConfidence","rankDomains","classifyDomain"].forEach(function(name){if(typeof module.exports[name]==="function"){var original=module.exports[name];module.exports[name]=function priority9IJConfidenceWrapper(input){return priority9IJConfidenceScore(input,original.apply(this,arguments));};}});
