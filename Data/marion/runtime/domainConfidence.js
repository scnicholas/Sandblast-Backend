...
function r18cDetectLawConfidenceSignals(text="",context={}){
  const t=r18cDcStr(text);
  const ctx=r18cDcObj(context);
  const ctxCarry=[ctx.knowledgeDomain,ctx.selectedDomain,ctx.primaryDomain,ctx.lastRoute,ctx.lastTopic,ctx.currentObjective].map(r18cDcStr).join(" ");
  const allowCarry=/(next|continue|keep going|what next|what now|passed|locked)/i.test(t)&&/(law|legal|contract|copyright|licensing|liability|compliance|jurisdiction)/i.test(ctxCarry);
  const src=[t,allowCarry?ctxCarry:""].join(" ");
  const categories=r18cDcCategories(src);
  const explicit=/(r18c|law domain|legal domain|legal lane|route.*law|activate.*law|law real[-\s]?world assessment|legal risk assessment)/i.test(src);
  const active=(categories.length>0||explicit||allowCarry)&&!r18cDcSuppressedByTechnical(text);
  const secondary=[];
  if(/(ai|artificial intelligence|model|llm|automation|agent)/i.test(src))secondary.push("ai");
  if(/(cyber|security|privacy|data protection|credential|access|identity)/i.test(src))secondary.push("cyber");
  if(/(revenue|pricing|cost|grant|funding|tax credit|moneti[sz]e|royalty|fee|damages)/i.test(src))secondary.push("finance");
  if(/(roku|ott|streaming|channel|distribution|commercial|business)/i.test(src))secondary.push("business");
  return {version:R18C_DOMAIN_CONFIDENCE_VERSION,active,domain:"law",knowledgeDomain:active?"law":"",legalCategory:categories[0]||"general_legal_risk",legalCategories:categories,secondaryDomains:Array.from(new Set(secondary.filter(d=>d!=="law"))).slice(0,4),confidence:active?(explicit?0.97:allowCarry?0.82:0.94):0,answerMode:"grounded",highStakes:!!active,routeLocked:!!active,assessmentFrame:R18C_LAW_CONFIDENCE_FRAME.slice(),legalBoundary:Object.assign({},R18C_LAW_CONFIDENCE_BOUNDARY),noCrossDomainBleed:true,noUserFacingDiagnostics:true};
}
...
/* R18C_FULL_STACK_DOMAIN_CONFIDENCE_WRAP_START */
(function(){
  try {
    const H = module.exports.marionR18CFullStackHelpers;
    if (!H || module.exports.__r18cFullStackDomainConfidenceWrapped) return;
    const oldBuild = module.exports.buildDomainConfidenceProfile;
    const oldNormalize = module.exports.normalizeDomainConfidenceProfile;
    const oldDetect = module.exports.detectCandidates;
    function textFromArgs(a,b){ return H.extractText(a) || H.extractText(b) || H.T(a && (a.rawText || a.text || a.userText || a.prompt), 4000) || H.T(b && (b.rawText || b.text || b.userText || b.prompt), 4000); }
    function promote(profile, text, ctx){
      if (!H.r18cIsLaw(text, ctx)) return profile;
      const p = H.r18cProfile(text, ctx);
      const out = Object.assign({}, H.O(profile), p, {
        primaryDomain: "law",
        selectedDomain: "law",
        domain: "law",
        knowledgeDomain: "law",
        confidence: Math.max(Number(H.O(profile).confidence || 0), p.confidence),
        confidenceScore: Math.max(Number(H.O(profile).confidenceScore || 0), p.confidenceScore),
        reason: p.reason,
        candidates: [{domain:"law", confidence:p.confidence, reasons:[p.reason, p.legalCategory], knowledgeDomain:"law"}].concat((H.A(H.O(profile).candidates)).filter(c => H.O(c).domain !== "law").slice(0,4)),
        r18cLawAssessment: p
      });
      return out;
    }
    if (typeof oldBuild === "function") {
      module.exports.buildDomainConfidenceProfile = function(input, fallback){
        const base = oldBuild.apply(this, arguments);
        const text = textFromArgs(input, fallback);
        return promote(base, text, input || fallback || {});
      };
    }
    if (typeof oldNormalize === "function") {
      module.exports.normalizeDomainConfidenceProfile = function(value, fallback){
        const base = oldNormalize.apply(this, arguments);
        const text = textFromArgs(value, fallback);
        return promote(base, text, value || fallback || {});
      };
    }
    if (typeof oldDetect === "function") {
      module.exports.detectCandidates = function(text, context){
        const base = oldDetect.apply(this, arguments);
        if (!H.r18cIsLaw(text, context)) return base;
        const p = H.r18cProfile(text, context);
        return [{domain:"law", confidence:p.confidence, reasons:[p.reason, p.legalCategory], knowledgeDomain:"law"}]
          .concat(H.A(base).filter(c => H.O(c).domain !== "law")).slice(0,6);
      };
    }
    module.exports.__r18cFullStackDomainConfidenceWrapped = true;
  } catch(_err) {}
})();
