...
/* MARION_ROUND4_CONFIDENCE_STABILITY_V2_START */
(function(){"use strict";const api=module.exports;if(!api||api.__marionRound4ConfidenceStabilityV2)return;let md=null;try{md=require("./marionRound4DomainIntegration.js")}catch(_){return}const V="nyx.marion.round4.confidenceStability/2.0";api.scoreRound4Domains=function(input,context={}){const p=md.build(input,context);return {version:V,domain:p.primaryDomain,knowledgeDomain:p.primaryDomain,confidence:p.rankedCandidates[0]?p.rankedCandidates[0].confidence:.72,band:"high",ambiguous:false,needsClarifier:false,answerMode:"grounded",candidates:p.rankedCandidates,secondaryDomains:p.secondaryDomains,multiDomainIntegration:p,recomputeProhibited:true,executionAuthorized:false,noUserFacingDiagnostics:true}};api.MARION_ROUND4_CONFIDENCE_STABILITY_VERSION=V;api.__marionRound4ConfidenceStabilityV2=true;
})();
