...
const DEFAULTS=Object.freeze({enabled:true,internalOnly:true,maxPasses:1,maxRecursionDepth:1,maxOutputBytes:48000,preserveReply:true,executionAuthorized:false,noUserFacingDiagnostics:true});
function resolve(input){const src=safeObject(input);const p=safeObject(src.policy||src);return Object.freeze({...DEFAULTS,...p,internalOnly:true,maxPasses:Math.max(0,Math.min(2,Number(p.maxPasses)||1)),maxRecursionDepth:Math.max(0,Math.min(1,Number(p.maxRecursionDepth)||1)),maxOutputBytes:Math.max(4000,Math.min(49000,Number(p.maxOutputBytes)||48000)),preserveReply:true,executionAuthorized:false,noUserFacingDiagnostics:true});}
