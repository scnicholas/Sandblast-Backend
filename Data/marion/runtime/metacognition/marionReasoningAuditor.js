...
const confidence=optionalRequire("./marionConfidenceAnalyzer.js",{analyze:(src)=>({confidence:clamp01(src&&src.confidence,0.6)})});
const bias=optionalRequire("./marionBiasDetector.js",{detect:()=>({findings:[]})});
const gaps=optionalRequire("./marionKnowledgeGapDetector.js",{detect:(src)=>{const gs=safeArray(src&&src.knowledgeGaps||src&&src.missingFacts);return{hasGaps:gs.length>0,gaps:gs}}});
