...
function confidenceAwareClarifier(route="",knowledgeDomain="",text=""){
  ...
  if(/\bfile|code|patch|backend|frontend|runtime|state spine|bridge|composer\b/i.test(t)||r==="technical")return"Which file or runtime layer do you want me to focus on?";
...
function applyConfidenceAwareResponseShaping(reply="",profile={},intent="",text="",input={}){
  ...
  if(p.needsClarifier&&p.clarifier&&!isNyxMarionBackendTechnicalContext(text)){
    ...
    if(isBroadAnswerableConceptPrompt(text,input,{}))return broadConceptFallbackReply(text,input,{})||out;
    return p.clarifier;
  }
  if(!out)return out;
...
function isDefinitionQuestion(text=""){
  const t=lower(text);
  if(!t)return false;
  return /\b(what\s+is|what\s+are|tell me about|define|definition\s+of|meaning\s+of|explain|describe)\b/i.test(t) && !isNyxMarionBackendTechnicalContext(t);
}
...
function ambiguousDefinitionClarificationReply(text="",input={},routed={}){
  ...
  if(/\baudit(?:ing)?\b/i.test(term))return broadConceptFallbackReply(text,input,routed)||knowledgeDomainReply("finance",text,input,routed);
  if(/\bcognitive\b/i.test(term))return broadConceptFallbackReply(text,input,routed)||knowledgeDomainReply("psychology",text,input,routed);
...
function broadConceptFallbackReply(text="",input={},routed={}){
  ...
  if(/\bcognitive\b/.test(key))return "Cognitive refers to the mental processes involved in understanding, learning, remembering, reasoning, and making decisions. In psychology, it describes how people interpret information; in AI, it describes systems that imitate parts of that reasoning process to classify, predict, or respond intelligently.";
