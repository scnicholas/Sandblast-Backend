...
const VERSION = "composeMarionResponse v3.14.0 REGISTRY-KNOWLEDGE-INGESTION-GATE";
...
function stripInstructionTail(text){return safeStr(text).replace(/\bdo not mention\b[\s\S]*$/i,"").replace(/\bin one clear sentence\b.*$/i,"").replace(/\bin one sentence\b.*$/i,"").trim();}
...
function registryKnowledgeAnswer(domain,text){
  ...
  if(best&&bestScore>=24){
    const title=safeStr(best.title),body=firstSentence(best.text);
    if(/\bdefine|definition of|meaning of|what is|what are|explain the word\b/i.test(lower(text))&&body)return body;
    return body||title;
  }
  return"";
}
...
function aiDomainAnswer(text){const t=lower(text);if(/\b(ai agent|agent)\b/i.test(t))return"An AI agent is a system that uses a model, instructions, tools, and feedback to pursue a goal through planned actions.";if(/\bllm|large language model\b/i.test(t))return"A large language model is an AI model trained to predict and generate language based on patterns learned from large text datasets.";if(/\brag|retrieval augmented generation\b/i.test(t))return"Retrieval-augmented generation is a method that lets an AI retrieve relevant external information before composing an answer.";if(/\bembedding|embeddings\b/i.test(t))return"An embedding is a numerical representation of meaning that lets software compare text, images, or other data by similarity.";return"AI systems work best when their inputs, tools, memory boundaries, verification gates, and logs are explicit enough to audit each action.";}
function cyberDomainAnswer(text){const t=lower(text);if(/\bleast privilege\b/i.test(t))return"Least privilege means giving each user, process, or system only the minimum access required to perform its intended task.";if(/\bthreat model|threat modeling\b/i.test(t))return"Threat modeling is the process of identifying what could go wrong, who might cause it, how likely it is, and which controls reduce the risk.";if(/\binput validation\b/i.test(t))return"Input validation means checking data before it is trusted or processed so malformed, unsafe, or unexpected input cannot break the system.";if(/\bsecrets?\b/i.test(t))return"Secrets are sensitive credentials such as API keys, tokens, and passwords that must be stored, transmitted, and logged with strict protection.";return"Cybersecurity reduces risk by validating inputs, limiting permissions, protecting secrets, monitoring behavior, and designing systems to fail safely.";}
function lawDomainAnswer(text){const t=lower(text);if(/\bjurisdiction\b/i.test(t))return"Jurisdiction is the legal authority of a court, regulator, or government to make decisions over a person, place, subject, or dispute.";if(/\bcontract\b/i.test(t))return"A contract is an agreement that creates legal obligations when the required elements, such as offer, acceptance, consideration, and capacity, are present.";if(/\bstatute\b/i.test(t))return"A statute is a written law passed by a legislature and interpreted through regulations, courts, and official guidance.";return"Legal analysis starts by identifying the jurisdiction, issue, governing sources, facts, and limits of what can be answered without professional legal advice.";}
function financeDomainAnswer(text){const t=lower(text);if(/\bcash[- ]?flow\b/i.test(t))return"Cash flow is the movement of money into and out of a business over a specific period.";if(/\bunit economics\b/i.test(t))return"Unit economics measure the revenue, cost, and profit attached to one customer, sale, product, or transaction.";if(/\bmargin|profit margin\b/i.test(t))return"Margin is the portion of revenue left after costs are subtracted, usually expressed as a percentage.";if(/\brunway\b/i.test(t))return"Runway is the amount of time a business can keep operating before it runs out of available cash.";if(/\bcac\b|customer acquisition cost/i.test(t))return"Customer acquisition cost is the average amount spent to gain one new customer.";if(/\bltv\b|lifetime value/i.test(t))return"Lifetime value is the estimated revenue or profit a customer generates over the full relationship with a business.";return"Finance decisions should be grounded in assumptions, cash-flow impact, unit economics, risk exposure, and clear decision thresholds.";}
function knowledgeDomainReply(knowledgeDomain,text,input={},routed={}){const k=normalizeKnowledgeDomain(knowledgeDomain);if(!k)return"";if(isKnowledgeDomainActivationRequest(text))return activateDomainReply(k);const registryReply=registryKnowledgeAnswer(k,text);if(registryReply)return registryReply;if(k==="english")return polishEnglishText(text);if(k==="psychology")return psychologyDomainAnswer(text,input);if(k==="ai")return aiDomainAnswer(text);if(k==="cyber")return cyberDomainAnswer(text);if(k==="law")return lawDomainAnswer(text);if(k==="finance")return financeDomainAnswer(text);return"";}
...
function hasNaturalDepth(reply,intent){const text=safeStr(reply);if(!text)return false;if(intent==="domain_question"&&sentenceCount(text)>=1&&text.length>=45&&!/domain engaged:/i.test(text))return true;const profile=toneProfile(intent);if(sentenceCount(text)<profile.minSentences)return false;if(text.length<110&&!["simple_chat","identity_query","music_query","news_query","roku_query"].includes(intent))return false;if(/^(technical response|loop diagnosis|state spine audit target|bridge audit target):/i.test(text)&&sentenceCount(text)<3)return false;return true;}
