...
function buildComposerVoiceMetadata(input = {}, routed = {}, response = {}) {
  ...
  const voice = firstObj(i.voice, r.voice, safeObj(r.routing).voice, resp.voice, i.voiceEnvelope, r.voiceEnvelope);
  const inputSource = firstText(i.inputChannel, i.source, i.inputSource, safeObj(i.voiceTextParity).source, voice.inputChannel, voice.source, "");
  const active = inputSource === "voice" || voice.source === "voice" || voice.inputChannel === "voice" || safeObj(i.voiceTextParity).source === "voice";
  ...
  if (!active && !Object.keys(voice).length) {
    return {
      active: false,
      inputChannel: firstText(inputSource, "text"),
      source: firstText(inputSource, "text"),
      audioStored: false,
