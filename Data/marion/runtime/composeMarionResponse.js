...
const VERSION = "composeMarionResponse v3.7.3 FINAL-LOOP-HARDLOCK-GREETING-AUTHORITY";
...
function detectIdentityIntent(text){const t=lower(text);return /\b(who are you|what are you|tell me who you are|how do you think|how marion helps|what is marion|who is marion|your identity|your role|what do you do)\b/i.test(t);}
...
function isGreetingOnly(text){
  ...
  return /^(hi|hello|hey|yo|good morning|good afternoon|good evening)(\s+nyx|\s+nix|\s+vera)?$/.test(t);
}
function buildMarionSafeGreetingReply(_text,input={}){
  const name=firstText(safeObj(input).preferredName,safeObj(input).userName,"Mac");
  return `Hi ${name}. Nyx is live, Marion is connected behind the response path, and I’ll keep the next command clean and grounded.`;
}
function buildFinalLoopRecoveryReply(intent,text,input={}){
  if(isGreetingOnly(text))return buildMarionSafeGreetingReply(text,input);
...
function identityReply(text,input={}){const t=lower(text);if(/marion helps|how marion|how do you think|marion.*think|think/i.test(t))return "I’m Nyx — the live interface you speak with. Marion is the cognitive layer behind me: it reads intent, context, memory pressure, and domain signals, then shapes the final answer I deliver. When we are working correctly, I handle the voice and interaction while Marion supplies the deeper reasoning.";return "I’m Nyx — the live interface for Sandblast. Marion is the deeper cognitive system behind me, helping interpret intent, preserve context, and shape responses so this feels like one coordinated intelligence instead of a loose chatbot.";}
function simpleChatReply(text,input={}){const t=lower(text);if(!t)return"";if(isGreetingOnly(t))return buildMarionSafeGreetingReply(text,input);if(/how are you/.test(t))return"I’m steady, focused, and ready to help. Send the target and I’ll keep it clean.";if(/thank/.test(t))return"You’re welcome. We’ll keep the thread moving without looping.";return"";}
function technicalReply(text,input={}){const memory=safeObj(input.previousMemory||input.memory),state=safeObj(input.conversationState||input.state||memory.stateSpine||memory.conversationState),noProgress=clampInt(safeObj(state.repetition).noProgressCount||memory.noProgressCount,0,0,99),t=lower(text);if(/state spine|statespine/i.test(t))return"State Spine audit target: confirm revision increment, user hash update, assistant hash update, final stage transition, and noProgress reset after a trusted Marion final.";if(/bridge|final envelope|signature/i.test(t))return"Bridge audit target: normalize once, route once, compose once, wrap through the single Marion final envelope, then return without re-entry.";if(/loop/i.test(t)||noProgress>=1)return`Loop diagnosis: verify inbound text, routed intent, composer reply, final envelope creation, and State Spine persistence. Current no-progress pressure: ${noProgress}.`;return"Technical response: verify the routed intent, composed reply, trusted final envelope, and State Spine persistence as one atomic turn. No greeting fallback should be allowed to become final output.";}
