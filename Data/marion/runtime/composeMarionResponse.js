...
function priority9EFreshContinuationReply(prompt,lastReply,ctx){var task=priority9EResolveTaskLabel(prompt,lastReply,ctx);if(/Priority 90\/9E/i.test(task))return "Run the Priority 90/9E sequence again: confirm the active lane, retest “Next steps,” retest “Run that again,” verify the response uses fresh wording, reject internal governor wording, and move forward only when the public answer stays conversational.";if(/Priority 9E/i.test(task))return "Run Priority 9E again: resolve the continuation to the active task, generate fresh wording, reject internal governor wording, confirm no repeated reply, then lock the loop governor before moving to the next layer.";return "Run the last valid Marion task again with fresh wording, keep the useful context intact, and return the next concrete action instead of describing the internal process.";}
...
function priority9ER2ConcreteContinuationReply(prompt,lastReply,ctx,packet){var source=safeStr([prompt,lastReply,priority9ER2LastTaskSource(packet,ctx),JSON.stringify(safeObj(ctx).progressionMemory||{})].join(" "));if(/priority\s*(?:90|9c|9d)|echo|fallback|suppression|five[-\s]?turn|lane[-\s]?lock|nyx route|handoff/i.test(source))return "Run the Priority 90/9E sequence again: confirm greeting stability, confirm lane-lock, test “Next steps,” test “Run that again,” verify fresh wording, reject meta-governor language, then mark the live continuation path passed only if the answer stays conversational.";if(/priority\s*9e|loop governor|meta[-\s]?recovery|continuation enforcement|concrete continuation/i.test(source))return "Run Priority 9E again: resolve the continuation to the active lane, generate fresh wording, block meta-governor language, confirm there is no exact replay, then keep the result as a normal conversational answer.";return "Repeat the last valid Marion sequence: restate the target in fresh wording, perform the next concrete step, verify no echo or meta-language is visible, then continue from the active lane.";}
