...
const VERSION = "composeMarionResponse v3.19.3 VALIDATION-SUPPRESSION-DEPTH-GOVERNOR + BACKEND-TECHNICAL-DOMAIN-LOCK";
...
function hasNaturalDepth(reply,intent){const text=safeStr(reply);if(!text)return false;if(intent==="domain_question"&&sentenceCount(text)>=2&&text.length>=120&&!/domain engaged:/i.test(text))return true;const profile=toneProfile(intent);if(sentenceCount(text)<profile.minSentences)return false;if(text.length<110&&!["simple_chat","identity_query","music_query","news_query","roku_query"].includes(intent))return false;if(/^(technical response|loop diagnosis|state spine audit target|bridge audit target):/i.test(text)&&sentenceCount(text)<3)return false;return true;}
...
function applyConversationQuality(reply,intent,text,input={},routed={}){if(isVoiceSystemCheckTurn(text,input))return buildVoiceSystemCheckReply(text,input);let out=sanitizeUserFacingReply(reply,intent,text,input);if(intent==="domain_question"&&out&&hasNaturalDepth(out,intent))return out.replace(/\s+/g," ").trim();out=deepenGeneralReply(out,intent,text,input,routed);out=out.replace(/\s+/g," ").trim();if(isInternalContractLeak(out)||isBlockedLoopReply(out))out=buildFinalLoopRecoveryReply(intent,text,input);if(!hasNaturalDepth(out,intent)&&!["simple_chat","identity_query","music_query","news_query","roku_query","domain_question"].includes(intent)){out=`${out} ${nextMoveForIntent(intent,text)}`.replace(/\s+/g," ").trim();}return sanitizeUserFacingReply(out,intent,text,input)||buildFinalLoopRecoveryReply(intent,text,input);}
