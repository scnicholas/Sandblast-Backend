...
function finalSurfaceReply(value,intent,text,input={}){
  const guarded=progressionShapingGuardReply(text,input,intent,{}); if(guarded)return guarded;
...
function domainConfidenceUserReply(input = {}, routed = {}) {
  ...
  return "The domain signal is too mixed to lock safely. I’ll keep the answer in the general lane instead of forcing a finance, psychology, legal, English, AI, or cyber route from weak evidence.";
}
...
function progressionShapingProfile(reply="",intent="",text="",input={},routed={}){
  const guard=progressionShapingGuardProfile(text,input,routed,intent);
  ...
  return{version:PROGRESSION_SHAPING_GOVERNOR_VERSION,active:!!(guard.active||parity||isolation||practical||contextual||directive||leak||thin),guard,parity,isolation,practical,contextual,directive,leak,thin,carry,updatedAt:Date.now()};
}
...
function progressionShapingReply(reply="",intent="",text="",input={},routed={}){
  const guarded=applyProgressionShapingGuard(reply,intent,text,input,routed); if(guarded&&guarded!==reply)return guarded;
...
const PROGRESSION_SHAPING_GUARD_VERSION = "nyx.marion.progressionShapingGuard/1.0";
function progressionTurnNumber(text=""){
  const m=lower(text).match(/\bturn\s*([1-5])\b/i);
  return m ? clampInt(m[1],0,0,5) : 0;
}
function progressionRequestedPhase(text=""){
  const t=lower(text), turn=progressionTurnNumber(text);
  if(turn===1)return "direct";
  if(turn===2)return "deepen";
  if(turn===3)return "refine";
  if(turn===4)return "action";
  if(turn===5)return "rule";
  if(/\b(go deeper|deepen|why|does not guarantee|mechanism)\b/i.test(t))return "deepen";
  if(/\b(refine|scenario|late[- ]?paying|invoices?|expenses?|business owner)\b/i.test(t))return "refine";
  if(/\b(practical next steps?|diagnose|correct|checklist|action steps?)\b/i.test(t))return "action";
  if(/\b(final operating rule|rule marion should preserve|summarize the operating rule|carry[- ]?forward rule)\b/i.test(t))return "rule";
  if(/\b(explain|define|what is|versus|vs\.?)\b/i.test(t))return "direct";
  return "";
}
function priorProgressionDomain(input={},routed={}){
  const i=safeObj(input), pm=safeObj(i.previousMemory||i.memory||i.turnMemory), st=safeObj(i.state||i.conversationState||pm.stateSpine||pm.conversationState), sb=safeObj(pm.stateBridge||st.stateBridge), routing=safeObj(routed.routing||i.routing), dc=safeObj(pm.domainConfidence||st.domainConfidence||routing.domainConfidence||safeObj(routed).domainConfidence);
  return normalizeKnowledgeDomain(firstText(pm.lastKnowledgeDomain,st.lastKnowledgeDomain,sb.lastKnowledgeDomain,dc.knowledgeDomain,routing.knowledgeDomain,routed.knowledgeDomain,i.knowledgeDomain)) || normalizeDomain(firstText(pm.lastDomain,st.domain,sb.lastDomain,dc.selectedDomain,dc.primaryDomain,routing.domain,routed.domain), "");
}
function progressionGuardFinanceEvidence(text="",input={},routed={}){
  const t=lower(text), prior=lower(priorProgressionDomain(input,routed)), phase=progressionRequestedPhase(text);
  const explicitFinance=/\b(finance|financial|cash[- ]?flow|profit|cash availability|receivables?|late[- ]?paying|invoices?|expenses?|working capital|cash[- ]?flow problem|operating rule)\b/i.test(t);
  const progressionFinance=/\bprogression compact finance test\b/i.test(t)||(/\bturn\s*[1-5]\b/i.test(t)&&explicitFinance)||(/\bturn\s*[2-5]\b/i.test(t)&&prior==="finance");
  return !!(phase&&(explicitFinance||progressionFinance||prior==="finance"));
}
function progressionShapingGuardProfile(text="",input={},routed={},intent=""){
  const phase=progressionRequestedPhase(text), turn=progressionTurnNumber(text), finance=progressionGuardFinanceEvidence(text,input,routed), prior=priorProgressionDomain(input,routed), active=!!(phase&&finance);
  return {version:PROGRESSION_SHAPING_GUARD_VERSION,active,domain:finance?"finance":prior,phase,turn,priorDomain:prior,reason:active?"finance_progression_guard":"inactive",updatedAt:Date.now()};
}
function financeProgressionGuardReply(phase="",text=""){
  if(phase==="direct")return "Cash flow is the timing of money moving in and out of the business, while profit is the surplus left after revenue is matched against expenses. A business can look profitable and still be under pressure if the cash has not arrived yet, if inventory or debt absorbs money, or if growth consumes working capital. Profit tells you whether the model creates value; cash flow tells you whether the business can keep operating.";
  if(phase==="deepen")return "Profit does not guarantee cash availability because accounting can recognize revenue before the money is actually collected. If invoices are unpaid, customers pay late, inventory is purchased upfront, payroll is due, or debt payments hit before collections arrive, the business may show profit on paper while the bank account is tight. The deeper rule is timing: profit measures economic surplus, but cash flow measures whether obligations can be met when they come due.";
  if(phase==="refine")return "For a business owner with late-paying clients, invoices, and expenses, the issue is not simply whether the work is profitable; it is whether cash arrives before bills must be paid. A strong invoice pipeline can still create stress if receivables age too long, expenses are fixed, and supplier, payroll, rent, or debt payments land first. The owner should watch the timing gap between invoiced revenue and collected cash, because that gap is where profitable businesses get squeezed.";
  if(phase==="action")return "Diagnose the cash-flow problem in this order: list every receivable by client, amount, due date, and days overdue; map fixed expenses and debt payments by due date; compare expected cash-in against required cash-out for the next 7, 14, and 30 days; identify the clients or cost categories creating the gap; tighten collections, shorten payment terms, pause nonessential spending, and build a rolling cash forecast so the business can see pressure before it becomes a crisis.";
  if(phase==="rule")return "Final operating rule: preserve the distinction between profitability and survivability. Profit tells Marion whether the business model creates surplus; cash flow tells Marion whether the business can meet obligations in real time. When a user continues the finance thread, Marion should carry the cash-timing context forward, deepen or operationalize the answer, and never replace the finance subject with generic progression language or unrelated business-positioning content.";
  return "";
}
function progressionShapingGuardReply(text="",input={},intent="",routed={}){
  const profile=progressionShapingGuardProfile(text,input,routed,intent);
  if(!profile.active)return "";
  if(profile.domain==="finance")return financeProgressionGuardReply(profile.phase,text);
  return "";
}
function isWeakProgressionSurface(reply="",text="",input={},routed={},intent=""){
  const r=lower(reply), profile=progressionShapingGuardProfile(text,input,routed,intent);
  if(!profile.active)return false;
  return /\b(continuing from .*the next move is|carry only the useful part|avoid generic|last accepted result|last weak reply|tell me the exact target|send the exact target|give me the target|what'?s next|commercially, the clean move|sponsor'?s audience|premium engagement layer|measurable outcomes)\b/i.test(r);
}
function applyProgressionShapingGuard(reply="",intent="",text="",input={},routed={}){
  const guarded=progressionShapingGuardReply(text,input,intent,routed);
  if(!guarded)return reply;
  if(!safeStr(reply)||isWeakProgressionSurface(reply,text,input,routed,intent)||isBlockedLoopReply(reply)||isInternalContractLeak(reply))return guarded;
  return guarded;
}


function highPriorityProgressionSurfaceReply(text="",input={},intent="",routed={}){
  const guarded=progressionShapingGuardReply(text,input,intent,routed); if(guarded)return guarded;
  const t=lower(normalizeVoiceTextParityText(text));
