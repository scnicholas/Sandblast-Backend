...
function priority90ContextRepairReply(prompt,intent){
  ...
  if(/\b(priority\s*9c|priority\s*9d|priority\s*90|echo|fallback|suppression|loop|looping|deep conversational|multi[-\s]?layer|continuity)\b/i.test(source)){
    return "Priority 9C/9D is the response-discipline layer: classify the user’s intent first, block prompt echo and generic fallback reuse, preserve the current prompt through recovery, and return one concrete final answer tied to the active Marion lane.";
  }
  ...
  if(source)return "I have the current request. Marion will answer from this prompt, keep the reply concrete, and avoid reusing a stale fallback.";
  return "Marion is ready, but there is no clean user prompt in this turn. The safe action is to suppress the reply instead of emitting a generic fallback.";
}
