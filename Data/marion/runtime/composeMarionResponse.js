...
const VERSION = "composeMarionResponse v3.9.0 USER-WARMTH-SOCIAL-GATE";
...
function resolveIntent(routed={},input={}){const r=safeObj(routed),i=safeObj(input),text=extractText(routed,input),t=lower(text);if(detectIdentityIntent(text))return "identity_query";if(/make the conversation|conversation quality|intelligent|layered|natural|response depth|tone profile|anti-flatness|phrase variation|context carry/i.test(t))return "technical_debug";if(/loop|looping|fallback|duplicate reply|backend reply unavailable|final envelope|finalenvelope|marion bridge|chat engine|state spine|api\/chat|transport path|pipeline/i.test(t))return "technical_debug";if(/given that setup|based on that|that setup|finalenvelope|final envelope|contract breaks|biggest risk/i.test(t))return "contextual_directive";if(detectArchitectureReasoning(text))return "domain_question";if(detectDirectiveIntent(text))return "directive_response";return normalizeIntent(r.intent||(r.marionIntent&&r.marionIntent.intent)||(r.routing&&r.routing.intent)||i.intent||(i.marionIntent&&i.marionIntent.intent)||(i.routing&&i.routing.intent)||(i.signals&&i.signals.technical&&i.signals.technical.detected?"technical_debug":"")||(i.signals&&i.signals.emotional&&i.signals.emotional.detected?"emotional_support":"")||"simple_chat");}
function resolveDomain(routed={},input={},intent="simple_chat"){const r=safeObj(routed),i=safeObj(input);return normalizeDomain(r.primaryDomain||r.domain||(r.routing&&r.routing.domain)||i.domain||i.requestedDomain||(i.routing&&i.routing.domain),intent);}
...
function isWarmSocialTurn(text){
  const t=lower(text);
  return isGreetingOnly(text)||isHowAreYouTurn(text)||/\b(good to see you|nice to meet you|what can you do|what can you help with|thanks|thank you)\b/i.test(t);
}
...
function buildWarmSocialReply(text,input={}){
  ...
  if(isGreetingOnly(text)){
    ...
    if(hashText(prior)===hashText(first)||stateHashText(prior)===stateHashText(first))return "Ready when you are. We can work on chat, media, radio, News Canada, Roku, diagnostics, or anything else you want to shape.";
    return first;
  }
  ...
  if(/\b(what can you do|what can you help with)\b/i.test(t))return "I can help with conversation, media, radio, News Canada, Roku, avatar flow, and backend diagnostics. Tell me where you want to start.";
  return "I’m here with you. Tell me what you want to work on, and I’ll keep it clear and useful.";
}
function buildFinalLoopRecoveryReply(intent,text,input={}){
  ...
  return "Nyx is live with Marion connected. Send the command and I’ll keep the response specific, grounded, and non-repetitive.";
}
