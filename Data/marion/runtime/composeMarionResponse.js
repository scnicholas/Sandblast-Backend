...
const RESPONSE_DEPTH_CALIBRATION_VERSION = "nyx.marion.responseDepthCalibration/1.0";
function responseDepthCalibrationKind(text=""){
  const t=lower(text);
  if(!t)return "normal";
  if(/\b(short version|quick version|brief version|compact|in short|short answer|bottom line|summarize briefly|give me the short)\b/i.test(t))return "compact";
  if(/\b(deeper|deep dive|architecture-level|strategic|fuller|more complete|expanded|detailed|covers|checklist that covers|sponsor-facing positioning argument)\b/i.test(t))return "deep";
  return "working";
}
function isBusinessDepthCalibrationTurn(text="",intent="",domain=""){
  const t=lower(text);
  return (intent==="business_strategy"||domain==="business"||/\b(advertisers?|sponsors?|sponsorship|campaign value|audience attention|brand recall|retention|commercially|media kit|ad placement)\b/i.test(t)) && !/\b(roku submission|developer account|legal advice|cybersecurity|cash flow|profit|rewrite this|autopsy|line[- ]?by[- ]?line)\b/i.test(t);
}
function businessDepthCalibrationReply(text="",kind="working"){
  const t=lower(text);
  const sandblast=/\bsandblast\b/i.test(t);
  const subject=sandblast?"Sandblast":"the platform";
  if(kind==="compact"){
    if(/\baudience attention\b/i.test(t))return `${subject} audience attention matters because it shows sponsors people are actually staying with the media surface instead of scrolling past it. Stronger attention creates better recall, better brand association, and a clearer reason to buy placement.`;
    return `The commercial point is simple: sponsors buy attention, memory, and measurable response. ${subject} should package those into one clear audience, one placement offer, and one proof point.`;
  }
  if(kind==="deep")return `A stronger sponsor-facing argument for ${subject} should connect four things: attention, retention, recall, and campaign value. Attention proves the audience is present; retention proves they stay long enough for the message to land; brand recall proves the sponsor is remembered; and campaign value turns that attention into measurable outcomes such as clicks, listening time, offer response, or repeat exposure. The pitch should package one audience segment, one sponsor placement, one measurable outcome, and one proof point so the advertiser sees a buying case instead of a vague branding claim.`;
  return `Advertisers should care about ${subject} audience attention because attention is the first proof that a campaign has room to work. Retention shows the audience stays long enough for the sponsor message to register, and brand recall shows the placement can create memory rather than just impressions. The practical sponsor value is to package attention, retention, and recall into one measurable offer with a clear audience, placement, and outcome.`;
}
function suppressDuplicateOpeners(reply=""){
  let out=safeStr(reply);
  if(!out)return "";
  out=out.replace(/\b(Commercially, the clean move)\s*:\s*\1\b[:,]?\s*/gi,"$1: ");
  out=out.replace(/\b(The stronger angle)\s*:\s*\1\b[:,]?\s*/gi,"$1: ");
  out=out.replace(/\b(The leverage point)\s*:\s*\1\b[:,]?\s*/gi,"$1: ");
  out=out.replace(/\b(The practical play)\s*:\s*\1\b[:,]?\s*/gi,"$1: ");
  out=out.replace(/\b(Commercially, the clean move)\s*:\s*Commercially,\s*/gi,"$1: ");
  return out.replace(/\s+/g," ").trim();
}
function applyResponseDepthCalibration(reply="",intent="",domain="",text="",input={},routed={}){
  let out=suppressDuplicateOpeners(reply);
  const kind=responseDepthCalibrationKind(text);
  if(isBusinessDepthCalibrationTurn(text,intent,domain)){
    const calibrated=businessDepthCalibrationReply(text,kind);
    if(kind==="compact")return calibrated;
    if(kind==="deep" && (!out || out.length<360 || /Commercially, the clean move\s*:\s*Commercially/i.test(out)))return calibrated;
    if(kind==="working" && (!out || out.length<190 || /Commercially, the clean move\s*:\s*Commercially/i.test(out)))return calibrated;
    return out;
  }
  if(kind==="compact" && out.length>520 && !/\b(autopsy|audit|line[- ]?by[- ]?line|technical|legal|cybersecurity)\b/i.test(lower(text))){
    return firstNSentences(out,2,360) || out;
  }
  return out;
}
