...
const VERSION = "composeMarionResponse v3.19.0 CREATIVE-SUGGESTION-COGNITIVE-LAYER";
...
function shouldSurfaceCreativeSuggestion(layer={},intent="",text="",reply=""){if(!safeObj(layer).enabled)return false;if(!safeStr(layer.suggestion))return false;if(!safeStr(reply)||isInternalContractLeak(reply)||isBlockedLoopReply(reply))return false;if(intent==="emotional_support"||intent==="identity_query"||intent==="simple_chat")return false;if(CREATIVE_SUPPRESS_RX.test(lower(text)))return false;return CREATIVE_TRIGGER_RX.test(lower(text))||["technical_debug","business_strategy","contextual_directive","directive_response"].includes(intent);}
