...
const VERSION = "composeMarionResponse v3.19.4 INTENT-SPECIFIC-DEPTH-GOVERNOR + VALIDATION-SUPPRESSION + BACKEND-TECHNICAL-DOMAIN-LOCK";
...
function shouldEmotionInfluenceIntent(intent,emotion,text=""){
  ...
  if(emotion.escalationNeeded)return true;
  if(emotion.carried&&emotion.primary!=="neutral"&&hasEmotionalContinuityCue(text))return true;
  if(emotion.primary!=="neutral"&&emotion.intensity>=0.45&&emotion.confidence>=0.45&&hasEmotionalContinuityCue(text))return true;
  return emotion.primary!=="neutral"&&emotion.intensity>=0.6&&emotion.confidence>=0.55;
}
...
function inputSourceKind(input={}){const i=safeObj(input),session=safeObj(i.session),payload=safeObj(i.payload),body=safeObj(i.body),client=safeObj(i.client);return lower(firstText(i.inputSource,i.source,i.triggerSource,payload.inputSource,payload.source,body.inputSource,body.source,session.inputSource,session.source,client.inputSource,client.source,"text"));}
...
function deepenBusinessReply(reply,text){const base=safeStr(reply)||"Let’s make this commercial and practical.";return hasNaturalDepth(base,"business_strategy")?base:`${phrasePick("strategic",text)}: ${base} First clarify the offer, then isolate the buyer psychology, then package the value into one action the audience can understand immediately. ${nextMoveForIntent("business_strategy",text)}`;}
...
function applyIntentSpecificDepthGovernor(reply,intent,text,input={},routed={}){let out=sanitizeUserFacingReply(reply,intent,text,input);if(isDepthGovernorSuppressed(text,input))return out||buildFinalLoopRecoveryReply(intent,text,input);if(intent==="simple_chat"||intent==="identity_query"||intent==="music_query"||intent==="news_query"||intent==="roku_query")return out||deepenGeneralReply(out,intent,text,input,routed);if(!out)out=deepenGeneralReply(out,intent,text,input,routed);if(!hasNaturalDepth(out,intent)){out=`${out} ${intentDepthExpansion(intent,text,input,routed)}`.replace(/\s+/g," ").trim();}const p=depthProfile(intent);if(p.allowNextMove&&!/\b(next move|next check|next step|practical fix|practical move)\b/i.test(out)&&!isExactTechnicalValidationTurn(text)){out=`${out} ${nextMoveForIntent(intent,text)}`.replace(/\s+/g," ").trim();}return sanitizeUserFacingReply(out,intent,text,input)||buildFinalLoopRecoveryReply(intent,text,input);}
...
function applyConversationQuality(reply,intent,text,input={},routed={}){if(isVoiceSystemCheckTurn(text,input))return buildVoiceSystemCheckReply(text,input);let out=sanitizeUserFacingReply(reply,intent,text,input);if(intent==="domain_question"&&out&&hasNaturalDepth(out,intent)&&isDepthGovernorSuppressed(text,input))return out.replace(/\s+/g," ").trim();out=deepenGeneralReply(out,intent,text,input,routed);out=applyIntentSpecificDepthGovernor(out,intent,text,input,routed);out=out.replace(/\s+/g," ").trim();if(isInternalContractLeak(out)||isBlockedLoopReply(out))out=buildFinalLoopRecoveryReply(intent,text,input);return sanitizeUserFacingReply(out,intent,text,input)||buildFinalLoopRecoveryReply(intent,text,input);}
...
const CREATIVE_TRIGGER_RX = /\b(creative suggestion|suggestion module|improve|enhance|refine|upgrade|next step|next steps|plan|strategy|feature|module|layer|cognitive|intelligence|commercial grade|elite|controlled enhancement|what should we add|what can we add|what would improve)\b/i;
const CREATIVE_SUPPRESS_RX = /\b(voice test|mic test|system check|can you hear me|legal advice|crisis|suicide|self[- ]?harm|kill myself|hurt myself)\b/i;
...
function cognitiveSignalScore(text,intent,input={},routed={}){const t=lower(text),carry=extractContextCarry(input,routed);let score=0;if(CREATIVE_TRIGGER_RX.test(t))score+=28;if(/\b(autopsy|audit|hardening|critical fix|script|backend|composer|state spine|widget|pipeline)\b/i.test(t))score+=18;if(/\b(monetiz|sponsor|audience|commercial|brand|conversion|offer)\b/i.test(t))score+=14;if(/\b(conversation|memory|continuity|reflective|prompt|creative|intelligence)\b/i.test(t))score+=14;if(carry)score+=10;if(["technical_debug","business_strategy","contextual_directive","directive_response","domain_question"].includes(intent))score+=12;return Math.max(0,Math.min(100,score));}
function cognitiveLayerMode(intent,text){const t=lower(text);if(/\b(autopsy|audit|hardening|critical fix|script|backend|composer|state spine|pipeline)\b/i.test(t)||intent==="technical_debug")return"architectural_hardening";if(/\b(monetiz|sponsor|audience|commercial|brand|conversion|offer|strategy)\b/i.test(t)||intent==="business_strategy")return"commercial_leverage";if(/\b(reflective|prompt|conversation|memory|continuity|cognitive|intelligence)\b/i.test(t))return"cognitive_scaffolding";if(/\b(creative|suggestion|feature|module|enhance|upgrade)\b/i.test(t))return"creative_expansion";return"adaptive_reasoning";}
function creativeSuggestionForMode(mode,intent,text,input={},routed={}){const topic=boundedWords(deriveTopic(text,input,routed)||text,18);if(mode==="architectural_hardening")return`Add a small suggestion pass after the main answer, not before it. It should propose one controlled improvement for ${topic||"the active system"} while leaving routing, state, and output authority untouched.`;if(mode==="commercial_leverage")return`Turn the next improvement into a visible user benefit: one sharper promise, one measurable outcome, and one proof point that makes ${topic||"the offer"} easier to sell.`;if(mode==="cognitive_scaffolding")return`Add a reflective prompt that asks one better question after the answer lands, so Marion helps the user think deeper without hijacking the conversation.`;if(mode==="creative_expansion")return`Prototype the idea as a sidecar layer first: generate one useful suggestion, store its reason, then let the main reply remain clean and user-facing.`;if(intent==="domain_question")return`Use one clarifying lens: define the concept, explain why it matters, then suggest one practical next move.`;return`Add one useful next-angle suggestion only when it improves the user’s task; keep it short, optional, and grounded in the current turn.`;}
function buildCognitiveIntelligenceLayer({intent="simple_chat",domain="general",knowledgeDomain="",text="",reply="",input={},routed={},recoveryRequired=false}={}){const distress=detectDistress(text),score=cognitiveSignalScore(text,intent,input,routed),mode=cognitiveLayerMode(intent,text),suppressed=!!(isVoiceSystemCheckTurn(text,input)||isExactTechnicalValidationTurn(text)||isGreetingPostureTurn(text)||CREATIVE_SUPPRESS_RX.test(lower(text))||distress.crisis||recoveryRequired);const enabled=!!(!suppressed&&score>=34&&reply);const suggestion=enabled?creativeSuggestionForMode(mode,intent,text,input,routed):"";return{version:COGNITIVE_LAYER_VERSION,creativeSuggestionVersion:CREATIVE_SUGGESTION_VERSION,enabled,mode,score,domain,knowledgeDomain:knowledgeDomain||"",triggered:score>=34,suppressed,reason:suppressed?"safety_or_surface_turn":(enabled?"task_improvement_signal":"insufficient_signal"),suggestion,updatedAt:Date.now()};}
function shouldSurfaceCreativeSuggestion(layer={},intent="",text="",reply=""){if(!safeObj(layer).enabled)return false;if(!safeStr(layer.suggestion))return false;if(!safeStr(reply)||isInternalContractLeak(reply)||isBlockedLoopReply(reply))return false;if(intent==="emotional_support"||intent==="identity_query"||intent==="simple_chat")return false;if(CREATIVE_SUPPRESS_RX.test(lower(text))||isExactTechnicalValidationTurn(text))return false;if(isNyxMarionBackendTechnicalContext(text)&&/\b(technical test only|exact code-level|not a generic|no generic|answer with the exact)\b/i.test(lower(text)))return false;return CREATIVE_TRIGGER_RX.test(lower(text))||["technical_debug","business_strategy","contextual_directive","directive_response"].includes(intent);}
function applyCreativeSuggestionModule(reply,layer={},intent="",text="",input={}){let base=sanitizeUserFacingReply(reply,intent,text,input);if(!base)return"";if(!shouldSurfaceCreativeSuggestion(layer,intent,text,base))return base;const suggestion=sanitizeUserFacingReply(safeObj(layer).suggestion,intent,text,input);if(!suggestion)return base;if(lower(base).includes(lower(suggestion).slice(0,48)))return base;const out=`${base} Creative suggestion: ${suggestion}`.replace(/\s+/g," ").trim();return sanitizeUserFacingReply(out,intent,text,input)||base;}
...
function directiveReply(text,input={}){const t=lower(text);if(/improving this system|improve this system|next best step/i.test(t))return "Enforce the Marion final-envelope contract end to end: every path must return a valid finalEnvelope.reply before Nyx renders anything.";if(/commercial grade|closer to commercial grade|single change/i.test(t))return "Lock the /api/chat path to MarionBridge only, then verify every response returns finalEnvelope.reply with sessionPatch continuity.";if(/what should (we|i) do|next step|one precise action|one concrete step/i.test(t))return "Run the next controlled test, capture the raw /api/chat response, and fix the first layer that fails the finalEnvelope.reply contract.";return "The next best step is to enforce one response authority: MarionBridge must call ComposeMarionResponse, return finalEnvelope.reply, and let Nyx render only that final reply.";}
function contextualDirectiveReply(text,input={},routed={}){const t=lower(text),carry=lower(extractContextCarry(input,routed));if(/what layer|harden first|that desynchronization risk|desynchronization risk/i.test(t))return "Harden the MarionBridge-to-finalEnvelope boundary first. That is the choke point where valid cognition becomes renderable output: require finalEnvelope.reply, sessionPatch, replySignature, and trusted final markers to survive together before Nyx is allowed to display anything.";if(/sessionpatch survives|session patch survives|reply is lost|finalenvelope\.reply is lost/i.test(t))return "If finalEnvelope.reply is lost but sessionPatch survives, the state layer may advance without a renderable answer. That creates a split-brain turn: memory believes Marion completed the step, while Nyx has no trusted reply to display. The fix is to reject the state advance unless the same packet carries both finalEnvelope.reply and sessionPatch.";if(/finalenvelope|final envelope|contract breaks|breaks/i.test(t+" "+carry))return "If the finalEnvelope contract breaks in this architecture, Nyx can render incomplete or invalid output while Marion’s reasoning continues separately, causing desynchronization and trust loss.";if(/biggest risk|risk/i.test(t))return "The biggest risk is desynchronization: Nyx may appear responsive while Marion’s reasoning layer is no longer providing a valid final answer.";if(/given that setup|based on that|that setup/i.test(t))return "Given that setup, preserve the Marion-to-Nyx contract first: Marion must complete cognition, then Nyx should render only the finalEnvelope.reply.";return directiveReply(text,input);}
...
function technicalReply(text,input={}){const memory=safeObj(input.previousMemory||input.memory),state=safeObj(input.conversationState||input.state||memory.stateSpine||memory.conversationState),noProgress=clampInt(safeObj(state.repetition).noProgressCount||memory.noProgressCount,0,0,99),t=lower(text);if(isVoiceSystemCheckTurn(text,input))return buildVoiceSystemCheckReply(text,input);if(/state spine|statespine/i.test(t)&&/code-level|carry-forward|carry forward|stability|continuity hardening|highest-value|hardening improvement/i.test(t))return"Highest-value State Spine code-level refinement: gate carry-forward updates behind completed Marion reply acceptance, then update turnDepth, lastUserHash, lastAssistantHash, noProgressCount, and creativeCognitiveCarry in one bounded state patch. That prevents stale continuity from being promoted when a turn is blocked, incomplete, or only a diagnostic echo. The practical fix area is the post-reply state mutation path: accept one completed Marion answer, write one compact carry summary, reset noProgressCount only after the assistant hash changes, and cap creativeCognitiveCarry so it informs the next turn without taking over routing.";if(/state spine|statespine/i.test(t))return"State Spine is the continuity ledger. Verify revision increment, user hash update, assistant hash update, final-stage transition, and noProgress reset after a trusted Marion final. If any one of those is missing, the dialogue may feel alive for one turn and then flatten on the next.";if(/bridge|final envelope|signature/i.test(t))return"Bridge audit target: normalize once, route once, compose once, wrap through one Marion final envelope, then return without re-entry. The reply is only safe when the final reply, final status, Marion authority, and session patch survive together.";if(/loop/i.test(t)||noProgress>=1)return`Loop diagnosis: the transport path must return one trusted final reply, not a local fallback or diagnostic echo. Check inbound text, routing, composition, final response wrapping, and State Spine update as one controlled turn. Current no-progress pressure: ${noProgress}.`;return"The technical path is live. The next check is to compare the raw backend final reply with the widget-rendered reply, then confirm the user-facing answer stays clean instead of exposing internal routing language.";}
function emotionalReply(text,input={}){
  ...
  if(/\b(exhausting|exhausted|mentally drained|drained|burned out|burnt out|worn down)\b/i.test(lower(text)))return "That makes sense. You’ve been carrying a lot of precision work here, and the hard part is that every fix exposes the next deeper layer. Let’s reduce the load: treat this as a system-stabilization pass, not a personal failure. The next move is one controlled test, one raw response capture, one gate at a time.";
  if(progression)return progression;
  if(alreadySupported&&!distress.high)return"I hear the weight in that. Let’s move deeper instead of circling it: what is the real pressure underneath this right now?";
  if(distress.high)return"That sounds heavy, and I’m not going to flatten it into a generic line. What part of it is pressing on you the hardest right now?";
  return"I hear you. Let’s not flatten this into a generic answer. What part of this actually matters most underneath the surface?";
}
...
function continuationCue(text=""){const t=lower(text);return /\b(given that|given that answer|based on that|based on your answer|from that|from there|now explain|now make|turn that into|turn this into|make it sound|make it more|prioriti[sz]e|next three|those upgrades|that answer|for a sponsor|to a sponsor|investor[- ]?facing|premium|pitch|say out loud|short pitch|continue|carry that forward)\b/i.test(t);}
...
function continuityContext(input={},routed={},text=""){const prevReply=previousAssistantText(input),prevUser=previousUserText(input),carry=previousCarrySummary(input,routed),active=!!(continuationCue(text)||prevReply||carry);const topic=deriveTopic(text,input,routed);return{active,prevReply:compactSummary(prevReply,520),prevUser:compactSummary(prevUser,260),carry:compactSummary(carry,520),topic};}
...
function continuationAwareBusinessReply(text,input={},routed={}){const ctx=continuityContext(input,routed,text);const t=lower(text);if(/short pitch|say out loud|turn that into|turn this into/.test(t))return"Here’s the pitch: we’re building an AI media interface that does more than answer questions — it guides users through intelligent, emotionally aware, domain-specific conversations that make the platform feel alive, useful, and premium. For a sponsor, that means stronger attention, deeper engagement, better brand association, and a media experience people remember instead of scrolling past.";if(/premium|investor[- ]?facing/.test(t))return"Investor-facing version: this is a differentiated AI media layer that turns Sandblast from a passive content surface into an interactive intelligence platform. The value is defensibility: proprietary conversation flow, domain-aware guidance, emotional continuity, and sponsor-ready engagement data can create a product experience that is harder to copy than a standard radio stream or static media site.";if(/sponsor|business value|commercial value/.test(t))return"For a sponsor, the business value is attention quality. A smarter Nyx/Marion interface keeps users engaged longer, gives the brand a premium innovation halo, and creates more natural paths into campaigns, offers, music, news, and interactive media moments. The sponsor is not just buying placement; they are attaching themselves to a living AI-powered experience with higher memorability and better audience connection.";if(/prioriti[sz]e|next three|three upgrades/.test(t))return"I’d prioritize three upgrades: first, context carry so Nyx remembers the user’s goal across turns; second, emotional calibration so the interface responds with the right level of warmth without becoming generic; third, domain synthesis so Marion can connect technical, business, media, and user-experience reasoning in one answer. That order improves usefulness first, then trust, then commercial differentiation.";if(ctx.active)return`Carrying the thread forward: ${ctx.carry||ctx.prevReply||"the interface needs to feel intelligent, emotionally aware, and useful"} The business move is to turn that capability into a sponsor-facing value stack: attention, trust, differentiation, and measurable engagement.`;return"Commercially, the clean move is to define the offer, identify the sponsor’s audience, package the AI interface as a premium engagement layer, and tie it to measurable outcomes like retention, clicks, listening time, or campaign recall.";}
function continuationAwareReply(intent,text,input={},routed={}){const posture=extractGreetingPosture(input,routed,text);if(posture&&posture.active&&!continuationCue(text))return"";const ctx=continuityContext(input,routed,text);if(!ctx.active)return"";const t=lower(text);if(intent==="business_strategy"||/sponsor|business value|investor|premium|pitch/.test(t))return continuationAwareBusinessReply(text,input,routed);if(/prioriti[sz]e|next three|three upgrades/.test(t))return"Given that direction, prioritize the upgrades in this order: context carry first, because the system must remember what the user is building; emotional calibration second, because tone determines trust; domain synthesis third, because that is what makes Marion feel like a cognitive partner instead of a single-answer bot. That sequence improves continuity, then human feel, then intelligence depth.";if(/turn that into|short pitch|say out loud/.test(t))return"We’re building an AI media interface that feels alive: it remembers the conversation, understands the user’s intent, adjusts its tone, and gives practical answers across media, business, technology, and support. That makes the experience more useful for users and more valuable for sponsors.";if(/premium|investor[- ]?facing/.test(t))return"Premium framing: Nyx and Marion turn the media interface into an intelligent engagement layer, not just a chat widget. The strategic value is deeper user retention, stronger brand differentiation, and a scalable foundation for sponsor campaigns, guided discovery, and interactive media experiences.";return"I’m keeping the current thread intact. The next move is to name the exact output you want, then I’ll shape it into the clean user-facing version.";}

function hotFallbackReply(intent,text,input={}){const distress=detectDistress(text),emotion=resolveEffectiveEmotion(input,normalizeResolvedEmotion(input));if((intent==="emotional_support"||distress.emotional||(emotion.present&&emotion.primary!=="neutral"&&hasEmotionalContinuityCue(text)))&&!distress.crisis)return emotionalReply(text,input);if(intent==="contextual_directive")return contextualDirectiveReply(text,input);if(intent==="directive_response")return directiveReply(text,input);if(intent==="identity_query")return identityReply(text,input);if(intent==="technical_debug")return buildFinalLoopRecoveryReply(intent,text,input);if(intent==="emotional_support"||distress.emotional){if(distress.crisis)return"Your safety comes first. If you might hurt yourself or you’re in immediate danger, contact emergency services or a local crisis line now.";return"I’m with you. Let’s slow it down and stay with the specific pressure point instead of repeating a flat support line.";}if(intent==="business_strategy")return"Let’s keep this commercial: define the offer, identify the buyer psychology, package the value, and turn it into the next execution step.";if(intent==="music_query")return"Music lane is ready. Give me the year, artist, chart, or story angle and I’ll route it cleanly.";if(intent==="news_query")return"News Canada lane is ready. Give me the story, headline, or feed issue and I’ll keep the source path clean.";if(intent==="roku_query")return"Roku lane is ready. Tell me whether we’re checking the app path, live TV lane, content feed, or deployment issue.";return"";}
...
function buildReply(intent,text,input={},routed={}){if(isVoiceSystemCheckTurn(text,input))return buildVoiceSystemCheckReply(text,input);const emotion=resolveEffectiveEmotion(input,normalizeResolvedEmotion(input)),knowledgeDomain=resolveKnowledgeDomain(routed,input,text);const continuationReply=continuationAwareReply(intent,text,input,routed);if(continuationReply)return continuationReply;if(knowledgeDomain&&!(knowledgeDomain==="psychology"&&intent==="emotional_support")){const kdReply=knowledgeDomainReply(knowledgeDomain,text,input,routed);if(kdReply)return kdReply;}if(isCapabilityQuestion(text))return registryCapabilityIntro();if(intent==="simple_chat"&&isWarmSocialTurn(text))return buildWarmSocialReply(text,input);if(isRecoveryRequested(input,routed))return recoveryReply(intent,text,input);if(intent==="simple_chat"&&emotion.present&&emotion.primary!=="neutral"&&hasEmotionalContinuityCue(text))return emotionalReply(text,input);switch(intent){case"contextual_directive":return contextualDirectiveReply(text,input,routed);case"directive_response":return directiveReply(text,input);case"identity_query":return identityReply(text,input);case"technical_debug":return technicalReply(text,input);case"emotional_support":return emotionalReply(text,input);case"business_strategy":return continuationAwareBusinessReply(text,input,routed);case"music_query":return"Music lane is ready. Give me the year, artist, chart, or story angle and I’ll route it cleanly.";case"news_query":return"News Canada lane is ready. Give me the story, headline, or feed issue and I’ll keep the source path clean.";case"roku_query":return"Roku lane is ready. Tell me whether we’re checking the app path, live TV lane, content feed, or deployment issue.";case"identity_or_memory":return buildFinalLoopRecoveryReply(intent,text,input);case"domain_question":return domainQuestionReply(text,input,routed);default:return simpleChatReply(text,input);}}
function safeReply(value,intent,text,input={}){if(isVoiceSystemCheckTurn(text,input))return buildVoiceSystemCheckReply(text,input);const reply=sanitizeUserFacingReply(value,intent,text,input);let candidate="";if(isWarmSocialTurn(text))candidate=buildWarmSocialReply(text,input);else if(reply)candidate=reply;else if(intent==="technical_debug"||/\b(loop|looping|debug|fallback|technical|route|bridge|composer|chat engine|state spine|api|backend|frontend)\b/i.test(lower(text)))candidate=technicalReply(text,input);else{const emotion=resolveEffectiveEmotion(input,normalizeResolvedEmotion(input));if(intent==="emotional_support"||detectDistress(text).emotional||(emotion.present&&emotion.primary!=="neutral")){const emotional=emotionalReply(text,{...input,forceRecovery:false,recoveryRequired:false,lastLoopReasons:["blocked_or_empty_reply_emotion_preserved"]});candidate=finalSurfaceReply(emotional,intent,text,input);}else candidate=buildFinalLoopRecoveryReply(intent,text,input);}return applyConversationQuality(candidate,intent,text,input,{});}
...
function buildMemoryPatch({intent,domain,knowledgeDomain,text,reply,previousMemory,state,recoveryRequired,turnId,input={},routed={},contextCarry=""}){const replySignature=hashText(reply),replyStateSignature=stateHashText(reply),userSignature=hashText(text),stateUserHash=stateHashText(text),priorIntent=lower(firstText(previousMemory.lastIntent,state.lastIntent)),sameIntent=!!(priorIntent&&priorIntent===intent),priorUser=firstText(previousMemory.userSignature,previousMemory.lastUserSignature,state.lastUserHash),sameUser=!!(priorUser&&(priorUser===userSignature||priorUser===stateUserHash)),previousNoProgress=clampInt(safeObj(state.repetition).noProgressCount||previousMemory.noProgressCount,0,0,99),noProgressCount=recoveryRequired?0:(sameUser&&sameIntent?previousNoProgress+1:0),loopCount=recoveryRequired?Math.max(1,clampInt(state.loopCount||previousMemory.loopCount,0,0,99)):0,stage="final";const priorDepth=clampInt(previousMemory.turnDepth||state.turnDepth||safeObj(state.continuityThread).depthLevel,0,0,999);const isContinuation=continuationCue(text)||!!contextCarry;const turnDepth=isContinuation?Math.max(2,priorDepth+1):1;const lastTopic=deriveTopic(text,{...safeObj(input),previousMemory,state},routed);const carryForwardSummary=buildCarryForwardSummary({intent,domain,knowledgeDomain,text,reply,input:{...safeObj(input),previousMemory,state,contextCarry},routed});const conversationSummary=compactSummary(carryForwardSummary,700);return{turnId,lastIntent:intent,lastDomain:domain,lastKnowledgeDomain:knowledgeDomain||"",lastTopic,lastUserText:safeStr(text),lastAssistantReply:reply,conversationSummary,carryForwardSummary,turnDepth,userSignature,lastUserSignature:userSignature,stateUserHash,lastUserHash:stateUserHash,replySignature,replyStateSignature,lastReplySignature:replySignature,lastAssistantHash:replyStateSignature,noProgressCount,loopCount,recoveryRequired:!!recoveryRequired,duplicateUserTurn:sameUser,duplicateIntentTurn:sameIntent,supportUsedLastTurn:intent==="emotional_support",composedOnce:true,nextMove:chooseNextMove(intent,recoveryRequired),stateStage:stage,stage,stateBridge:{shouldAdvanceState:true,expectedStateMutation:"finalizeTurn",source:"composeMarionResponse",stateSchema:STATE_SPINE_SCHEMA,stateSchemaCompat:STATE_SPINE_SCHEMA_COMPAT,composedOnce:true,recoveryRequired:!!recoveryRequired,nextStateStage:"final",recoveryDetected:!!recoveryRequired,userSignature,replySignature,replyStateSignature,stateUserHash,noProgressCount,loopCount,turnDepth,lastTopic,carryForwardSummary},marionCohesion:{composerObserved:true,shouldAdvanceState:true,loopPhraseRejected:false,lastComposerIntent:intent,lastComposerDomain:domain,lastComposerUserSignature:userSignature,lastComposerReplySignature:replySignature,updatedAt:Date.now()},updatedAt:Date.now()};}
function buildFinalEnvelope(reply,intent,domain,turnId,knowledge={}){const replySignature=hashText(reply),replyStateSignature=stateHashText(reply),knowledgeDomain=normalizeKnowledgeDomain(safeObj(knowledge).knowledgeDomain||"");return{reply,displayReply:reply,spokenText:reply,intent,domain,knowledgeDomain,domainHints:safeObj(knowledge).domainHints||null,domainRoute:safeObj(knowledge).domainRoute||null,turnId,source:"composeMarionResponse",authority:"marionFinalEnvelope",contract:"MARION_FINAL_ENVELOPE_V1",contractVersion:"nyx.marion.final/1.0",final:true,marionFinal:true,handled:true,replySignature,replyStateSignature,cognitionComplete:true,createdAt:Date.now()};}
