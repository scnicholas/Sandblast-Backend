...
const VERSION = "composeMarionResponse v3.11.0 KNOWLEDGE-DOMAIN-HINT-SHAPING-GATE";
...
function knowledgeUtilCandidates(domain){
  const key=lower(domain).replace(/[^a-z0-9]+/g,"_").replace(/^_+|_+$/g,"");
  const map={psychology:"psychologyKnowledge",english:"englishKnowledge",ai:"aiKnowledge",cyber:"cyberKnowledge",law:"lawKnowledge",finance:"financeKnowledge"};
  const base=map[key]||"";
  if(!base)return[];
  return [`./Utils/${base}.js`,`./Utils/${base}`,`./utils/${base}.js`,`./utils/${base}`,`./${base}.js`,`./${base}`];
}
function loadKnowledgeUtil(domain){return tryRequireOptional(knowledgeUtilCandidates(domain));}
function normalizeDomainTokens(text,input={},routed={}){
  const supplied=safeArray(input.tokens||routed.tokens||safeObj(input.features).tokens||safeObj(routed.features).tokens);
  const fromText=lower(text).replace(/[^a-z0-9_\-\s]+/g," ").split(/\s+/).filter(Boolean).slice(0,24);
  const out=[],seen=new Set();
  for(const t of [...supplied,...fromText]){const v=safeStr(t).toLowerCase().slice(0,40);if(!v||seen.has(v))continue;seen.add(v);out.push(v);if(out.length>=28)break;}
  return out;
}
function resolveKnowledgeDomainForCompose(intent,domain,input={},routed={},text=""){
  const routing=safeObj(routed.routing||input.routing);
  const direct=firstText(input.knowledgeDomain,routed.knowledgeDomain,routing.knowledgeDomain,input.primaryDomain,routed.primaryDomain,routing.primaryDomain,domain);
  const key=lower(direct).replace(/[^a-z0-9]+/g,"_").replace(/^_+|_+$/g,"");
  if(["psychology","english","ai","cyber","law","finance"].includes(key))return key;
  const t=lower(text);
  if(/\b(psychology|attachment|trauma|cognitive distortion|overwhelm|spiral|shame|panic)\b/i.test(t))return"psychology";
  if(/\b(grammar|syntax|semantics|phonology|morphology|english|writing|clarity|polish|make this sound)\b/i.test(t))return"english";
  if(/\b(artificial intelligence|\bai\b|agent|agents|rag|llm|embedding|orchestration|prompt injection)\b/i.test(t))return"ai";
  if(/\b(cyber|cybersecurity|phishing|ransomware|breach|mfa|iam|endpoint|network security|defensive)\b/i.test(t))return"cyber";
  if(/\b(law|legal|contract|tort|criminal|charter|statute|case law|jurisdiction)\b/i.test(t))return"law";
  if(/\b(finance|pricing|unit economics|ltv|cac|runway|valuation|capital markets|risk management)\b/i.test(t))return"finance";
  return"";
}
function getKnowledgeHints(domain,text,input={},routed={}){
  const key=resolveKnowledgeDomainForCompose("",domain,input,routed,text);
  if(!key)return{enabled:false,domain:"",reason:"no_knowledge_domain"};
  if(isObj(input.domainHints)&&safeStr(input.domainHints.domain||input.domainHints.focus))return{...input.domainHints,domain:key,source:"input.domainHints"};
  if(isObj(routed.domainHints)&&safeStr(routed.domainHints.domain||routed.domainHints.focus))return{...routed.domainHints,domain:key,source:"routed.domainHints"};
  const mod=loadKnowledgeUtil(key);
  if(!mod||typeof mod.getMarionHints!=="function")return{enabled:false,domain:key,reason:"knowledge_util_unavailable"};
  try{
    const features={...safeObj(input.features),...safeObj(routed.features),lane:key,domain:key,intent:firstText(input.intent,routed.intent,safeObj(routed.routing).intent)};
    const hints=mod.getMarionHints({features,tokens:normalizeDomainTokens(text,input,routed),queryKey:firstText(input.queryKey,routed.queryKey,hashText(text).slice(0,18))}, {source:"composeMarionResponse",domain:key});
    return {...safeObj(hints),enabled:hints&&hints.enabled!==false,domain:key,source:"knowledgeUtil"};
  }catch(err){return{enabled:false,domain:key,reason:"knowledge_hint_fault",error:safeStr(err&&err.message||err,120)};}
}
function summarizeHintList(value,max=3){return safeArray(value).map(v=>safeStr(v,80)).filter(Boolean).slice(0,max).join(", ");}
function domainKnowledgeReply(domain,text,input={},routed={}){
  const key=resolveKnowledgeDomainForCompose("",domain,input,routed,text);
  if(!key)return"";
  const hints=getKnowledgeHints(key,text,input,routed);
  const focus=firstText(hints.focus,hints.stance,hints.reason,key);
  const principles=summarizeHintList(hints.principles,3);
  const guardrails=summarizeHintList(hints.guardrails,3);
  const frameworks=summarizeHintList(hints.frameworks,2);
  if(key==="psychology")return `Psychology lane active: start with safety and regulation before interpretation. ${focus?`Focus: ${focus}. `:""}${principles?`Use: ${principles}. `:""}Keep the response bounded, non-shaming, and final-envelope safe.`;
  if(key==="english")return `English lane active: shape the answer for clarity, flow, register, and clean language. ${focus?`Focus: ${focus}. `:""}${principles?`Use: ${principles}. `:""}The response should sound natural, polished, and easy to follow.`;
  if(key==="ai")return `AI lane active: explain the architecture, constraints, and implementation path. ${focus?`Focus: ${focus}. `:""}${frameworks?`Framework: ${frameworks}. `:""}${guardrails?`Guardrails: ${guardrails}. `:""}Keep it practical and contract-aware.`;
  if(key==="cyber")return `Cyber lane active: keep this defensive-only. ${focus?`Focus: ${focus}. `:""}${principles?`Use: ${principles}. `:""}Avoid exploit detail and steer toward hardening, detection, mitigation, and safe verification.`;
  if(key==="law")return `Law lane active: keep this educational, Canada-aware where relevant, and source-ladder driven. ${focus?`Focus: ${focus}. `:""}${guardrails?`Guardrails: ${guardrails}. `:""}Do not present this as legal advice.`;
  if(key==="finance")return `Finance lane active: use scenario reasoning and disclose assumptions. ${focus?`Focus: ${focus}. `:""}${principles?`Use: ${principles}. `:""}Do not make investment recommendations; separate cash flow, risk, and tradeoffs.`;
  return"";
}
...
function isWarmSocialTurn(text){
  const t=lower(text);
  return isGreetingOnly(text)||isHowAreYouTurn(text)||isCapabilityQuestion(text)||/\b(good to see you|nice to meet you|thanks|thank you)\b/i.test(t);
}
...
function buildFinalLoopRecoveryReply(intent,text,input={}){
  ...
  if(intent==="technical_debug"||/\b(loop|looping|debug|test|fallback|technical|route|bridge|composer|chat engine|state spine|api|backend|frontend|final envelope)\b/i.test(lower(text))){
    return "Technical response: the Marion path must return one trusted final reply only. I’m blocking generic placeholder language and keeping the reply bound to the routed intent, composed answer, final envelope, and session-state update.";
  }
  ...
  return "Tell me what you want to work on, and I’ll keep the response specific, grounded, and easy to build from.";
}
function finalSurfaceReply(value,intent,text,input={}){
  const reply=safeStr(value);
  if(reply&&!isBlockedLoopReply(reply))return reply;
...
function hasNaturalDepth(reply,intent){const text=safeStr(reply);if(!text)return false;const profile=toneProfile(intent);if(sentenceCount(text)<profile.minSentences)return false;if(text.length<110&&!["simple_chat","identity_query","music_query","news_query","roku_query"].includes(intent))return false;if(/^(technical response|loop diagnosis|state spine audit target|bridge audit target):/i.test(text)&&sentenceCount(text)<3)return false;return true;}
...
function applyConversationQuality(reply,intent,text,input={},routed={}){let out=safeStr(reply);out=deepenGeneralReply(out,intent,text,input,routed);out=out.replace(/\s+/g," ").trim();if(isBlockedLoopReply(out))out=buildFinalLoopRecoveryReply(intent,text,input);if(!hasNaturalDepth(out,intent)&&!["simple_chat","identity_query","music_query","news_query","roku_query"].includes(intent)){out=`${out} ${nextMoveForIntent(intent,text)}`.replace(/\s+/g," ").trim();}return out;}
...
function technicalReply(text,input={}){const memory=safeObj(input.previousMemory||input.memory),state=safeObj(input.conversationState||input.state||memory.stateSpine||memory.conversationState),noProgress=clampInt(safeObj(state.repetition).noProgressCount||memory.noProgressCount,0,0,99),t=lower(text);if(/state spine|statespine/i.test(t))return"State Spine is the continuity ledger. Verify revision increment, user hash update, assistant hash update, final-stage transition, and noProgress reset after a trusted Marion final. If any one of those is missing, the dialogue may feel alive for one turn and then flatten on the next.";if(/bridge|final envelope|signature/i.test(t))return"Bridge audit target: normalize once, route once, compose once, wrap through one Marion final envelope, then return without re-entry. The reply is only safe when finalEnvelope.reply, final:true, marionFinal:true, and the session patch survive together.";if(/loop/i.test(t)||noProgress>=1)return`Loop diagnosis: the transport path must return one trusted final reply, not a local fallback or diagnostic echo. Check inbound text, routed intent, composer reply, final envelope creation, and State Spine persistence as one atomic turn. Current no-progress pressure: ${noProgress}.`;return"Technical read: verify the routed intent, composed reply, trusted final envelope, and State Spine persistence as one atomic turn. The important shift is quality now: Marion should explain the layer, carry context, and give a forward move without letting a greeting fallback become final output.";}
...
function buildReply(intent,text,input={},routed={}){const emotion=resolveEffectiveEmotion(input,normalizeResolvedEmotion(input));const routedDomain=resolveDomain(routed,input,intent);const knowledgeDomain=resolveKnowledgeDomainForCompose(intent,routedDomain,input,routed,text);if(isCapabilityQuestion(text))return registryCapabilityIntro();if(knowledgeDomain&&intent!=="technical_debug"&&!isWarmSocialTurn(text)){const domainReply=domainKnowledgeReply(knowledgeDomain,text,input,routed);if(domainReply)return domainReply;}if(intent==="simple_chat"&&isWarmSocialTurn(text))return buildWarmSocialReply(text,input);if(isRecoveryRequested(input,routed))return recoveryReply(intent,text,input);if(intent==="simple_chat"&&emotion.present&&emotion.primary!=="neutral"&&hasEmotionalContinuityCue(text))return emotionalReply(text,input);switch(intent){case"contextual_directive":return contextualDirectiveReply(text,input,routed);case"directive_response":return directiveReply(text,input);case"identity_query":return identityReply(text,input);case"technical_debug":return technicalReply(text,input);case"emotional_support":return domainKnowledgeReply("psychology",text,input,routed)||emotionalReply(text,input);case"business_strategy":return"Let’s make this commercial and practical: clarify the offer, sharpen the buyer psychology, define the package, then move it into a clean execution path.";case"music_query":return"Music lane is ready. Give me the year, artist, chart, or story angle and I’ll route it cleanly.";case"news_query":return"News Canada lane is ready. Give me the story, headline, or feed issue and I’ll keep the source path clean.";case"roku_query":return"Roku lane is ready. Tell me whether we’re checking the app path, live TV lane, content feed, or deployment issue.";case"identity_or_memory":return buildFinalLoopRecoveryReply(intent,text,input);case"domain_question":return domainKnowledgeReply(knowledgeDomain||routedDomain,text,input,routed)||domainQuestionReply(text,input,routed);default:return simpleChatReply(text,input);}}
function safeReply(value,intent,text,input={}){const reply=safeStr(value);let candidate="";if(isWarmSocialTurn(text))candidate=buildWarmSocialReply(text,input);else if(reply&&!isBlockedLoopReply(reply))candidate=reply;else if(intent==="technical_debug"||/\b(loop|looping|debug|fallback|technical|route|bridge|composer|chat engine|state spine|api|backend|frontend)\b/i.test(lower(text)))candidate=buildFinalLoopRecoveryReply(intent,text,input);else{const emotion=resolveEffectiveEmotion(input,normalizeResolvedEmotion(input));if(intent==="emotional_support"||detectDistress(text).emotional||(emotion.present&&emotion.primary!=="neutral")){const emotional=emotionalReply(text,{...input,forceRecovery:false,recoveryRequired:false,lastLoopReasons:["blocked_or_empty_reply_emotion_preserved"]});candidate=finalSurfaceReply(emotional,intent,text,input);}else candidate=buildFinalLoopRecoveryReply(intent,text,input);}return applyConversationQuality(candidate,intent,text,input,{});}
...
function ensureFinalReply(packet={},ctx={}){const p=safeObj(packet),intent=ctx.intent||normalizeIntent(p.intent),domain=ctx.domain||normalizeDomain(p.domain,intent),turnId=ctx.turnId||firstText(p.turnId,p.finalEnvelope&&p.finalEnvelope.turnId),text=ctx.text||"",input=safeObj(ctx.input);let reply=firstText(p.finalEnvelope&&p.finalEnvelope.reply,p.reply,p.text,p.answer,p.output,p.response,p.message,p.spokenText);if(!isUsableFinalReply(reply)){return buildAwaitingMarionContract("compose_reply_missing_or_blocked",{replyPreview:safeStr(reply).slice(0,160),intent,domain}, {intent,domain,turnId,text,input});}const finalEnvelope=assertFinalEnvelope(buildFinalEnvelope(reply,intent,domain,turnId));const resolvedEmotion=normalizeResolvedEmotion(input);const presence=emotionPresenceProfile(intent,resolvedEmotion);const hint=emotionNyxHint(intent,resolvedEmotion);return{...p,ok:true,final:true,marionFinal:true,reply,text:reply,answer:reply,output:reply,response:reply,message:reply,spokenText:reply,finalEnvelope,displayReply:reply,hotFallbackApplied:false,finalAuthorityGuaranteed:true,speech:{...safeObj(p.speech),enabled:true,silent:false,silentAudio:false,textDisplay:reply,textSpeak:reply,presenceProfile:presence,nyxStateHint:hint},ui:{...safeObj(p.ui),nyxStateHint:hint,presenceProfile:presence,openOverlay:false},meta:{...safeObj(p.meta),finalEnvelopePresent:true,replyAuthority:"composeMarionResponse.ensureFinalReply",cognitionComplete:true,syntheticFallbackSuppressed:true},diagnostics:{...safeObj(p.diagnostics),hardFinalCognitionComplete:true,hotFallbackApplied:false,finalAuthorityGuaranteed:true,syntheticFallbackSuppressed:true}};}
function composeMarionResponse(routed={},input={}){const text=extractText(routed,input);const contextCarry=extractContextCarry(input,routed);const enrichedInput={...safeObj(input),contextCarry};let resolvedEmotion=normalizeResolvedEmotion(enrichedInput);resolvedEmotion=resolveEffectiveEmotion(enrichedInput,resolvedEmotion);let intent=resolveIntent(routed,enrichedInput);intent=protectsContextFromOverride(intent,text,enrichedInput,routed);if(shouldEmotionInfluenceIntent(intent,resolvedEmotion,text))intent="emotional_support";const domain=resolveDomain(routed,enrichedInput,intent),turnId=resolveTurnId(routed,enrichedInput),{previousMemory,state}=resolvePreviousMemory(enrichedInput),routing=safeObj(routed.routing||enrichedInput.routing),recoveryRequested=isRecoveryRequested(enrichedInput,routed);let rawReply=buildReply(intent,text,enrichedInput,routed),reply=safeReply(rawReply,intent,text,enrichedInput),duplicateDetected=false;const priorReplySig=firstText(previousMemory.replySignature,previousMemory.lastReplySignature,state.lastAssistantHash),replySignature=hashText(reply),replyStateSignature=stateHashText(reply),deepContinuityTurn=isDeepContinuityTurn(text,enrichedInput,routed);if(!recoveryRequested&&priorReplySig&&(priorReplySig===replySignature||priorReplySig===replyStateSignature)){duplicateDetected=true;if(deepContinuityTurn){const progressed=forceCognitionCompleteReply(intent,text,{...enrichedInput,forceRecovery:false,recoveryRequired:false,previousDuplicateReply:reply});if(isUsableFinalReply(progressed)&&hashText(progressed)!==priorReplySig&&stateHashText(progressed)!==priorReplySig){rawReply=progressed;reply=progressed;}else{duplicateDetected=false;}}else{rawReply=recoveryReply(intent,text,{...enrichedInput,forceRecovery:true,lastLoopReasons:["duplicate_composer_reply"]});reply=safeReply(rawReply,intent,text,{...enrichedInput,forceRecovery:true});}}reply=applyConversationQuality(reply,intent,text,enrichedInput,routed);const recoveryRequired=(recoveryRequested||duplicateDetected)&&!deepContinuityTurn,memoryPatch=buildMemoryPatch({intent,domain,text,reply,previousMemory,state,recoveryRequired,turnId});memoryPatch.resolvedEmotion=resolvedEmotion.present?resolvedEmotion.state:null;memoryPatch.emotionRuntimeObserved=!!resolvedEmotion.present;memoryPatch.emotionalContinuity=resolvedEmotion.present?{primary:resolvedEmotion.primary,secondary:resolvedEmotion.secondary,intensity:resolvedEmotion.intensity,confidence:resolvedEmotion.confidence,carried:!!resolvedEmotion.carried,continuityPreserved:!!resolvedEmotion.continuityPreserved,updatedAt:Date.now()}:null;memoryPatch.lastEmotionState=resolvedEmotion.present?resolvedEmotion.state:null;const progressionProfile=emotionalProgressionProfile(text,enrichedInput,resolvedEmotion);memoryPatch.emotionalProgression=progressionProfile.active?{phase:progressionProfile.phase,momentum:progressionProfile.momentum,carryDepth:progressionProfile.carryDepth,pressure:!!progressionProfile.pressure,needsDeepening:progressionProfile.phase==="deepen",updatedAt:Date.now()}:null;memoryPatch.conversationVector={emotionalContinuityActive:!!progressionProfile.active,contextCarryPresent:!!contextCarry,replyAuthority:"composeMarionResponse",finalEnvelopeRequired:true};const presenceProfile=emotionPresenceProfile(intent,resolvedEmotion),nyxStateHint=emotionNyxHint(intent,resolvedEmotion),domainConfig=registryDomainConfig(domain),domainLabel=registryDomainLabel(domain),knowledgeDomain=resolveKnowledgeDomainForCompose(intent,domain,enrichedInput,routed,text),domainHints=getKnowledgeHints(knowledgeDomain||domain,text,enrichedInput,routed);const base={ok:true,composedOnce:true,finalizedBy:"composeMarionResponse",version:VERSION,composerVersion:VERSION,domain,intent,stateStage:memoryPatch.stateStage,routing:{...routing,domain,intent,endpoint:routing.endpoint||"marion://routeMarion.primary"},reply,text:reply,answer:reply,output:reply,response:reply,message:reply,spokenText:reply,replySignature:memoryPatch.replySignature,replyStateSignature:memoryPatch.replyStateSignature,memoryPatch,sessionPatch:memoryPatch,resolvedEmotion:resolvedEmotion.present?resolvedEmotion.state:null,emotionRuntimeObserved:!!resolvedEmotion.present,speech:{enabled:true,silent:false,silentAudio:false,textDisplay:reply,textSpeak:reply,presenceProfile,nyxStateHint,timingProfile:resolvedEmotion.timingProfile||{}},ui:{nyxStateHint,presenceProfile,openOverlay:false,domainLabel},meta:{domainRegistryLoaded:!!domainRegistryMod,domainLabel,domainMode:firstText(domainConfig.mode,""),domainDepth:firstText(domainConfig.depth,""),knowledgeDomain,domainHintsEnabled:!!domainHints.enabled,domainHintSource:firstText(domainHints.source,domainHints.reason)},domainHints,diagnostics:{composerVersion:VERSION,singleEmission:true,finalAuthority:"marionFinalEnvelope",composerDoesFinalize:true,hotFallbackGenerator:true,hardFinalCognitionComplete:true,identityAnchor:intent==="identity_query",directiveExecution:intent==="directive_response"||intent==="contextual_directive",contextualDirective:intent==="contextual_directive",contextCarryPresent:!!contextCarry,contextCarryHash:contextCarry?stateHashText(contextCarry):"",domainRegistryLoaded:!!domainRegistryMod,domainLabel,domainMode:firstText(domainConfig.mode,""),domainDepth:firstText(domainConfig.depth,""),knowledgeDomain,domainHintsEnabled:!!domainHints.enabled,domainHintReason:firstText(domainHints.reason,""),emotionRuntimeObserved:!!resolvedEmotion.present,emotionPrimary:resolvedEmotion.primary,emotionSecondary:resolvedEmotion.secondary,emotionIntensity:resolvedEmotion.intensity,emotionSafeToContinue:resolvedEmotion.safeToContinue,testSuiteHardened:true,blockedLoopReplySanitized:isBlockedLoopReply(rawReply),recoveryRequired,duplicateBroken:duplicateDetected,replySignature:memoryPatch.replySignature,replyStateSignature:memoryPatch.replyStateSignature,stateSchema:STATE_SPINE_SCHEMA,stateSchemaCompat:STATE_SPINE_SCHEMA_COMPAT}};return ensureFinalReply(base,{intent,domain,text,input:{...enrichedInput,resolvedEmotion:resolvedEmotion.present?resolvedEmotion.state:null,emotionalContinuity:memoryPatch.emotionalContinuity},turnId});}
async function run(routed={},input={}){return composeMarionResponse(routed,input);}
module.exports={VERSION,STATE_SPINE_SCHEMA,STATE_SPINE_SCHEMA_COMPAT,VALID_INTENTS,INTENT_TO_DOMAIN,composeMarionResponse,run,default:composeMarionResponse,ensureFinalReply,assertFinalEnvelope,_internal:{normalizeResolvedEmotion,resolveEffectiveEmotion,hasEmotionalContinuityCue,previousEmotionalContinuity,emotionalProgressionProfile,emotionalProgressionReply,shouldEmotionInfluenceIntent,extractText,detectIdentityIntent,detectDirectiveIntent,extractContextCarry,hasContextCarry,protectsContextFromOverride,resolveIntent,resolveDomain,isBlockedLoopReply,isGreetingOnly,isHowAreYouTurn,isWarmSocialTurn,isCapabilityQuestion,buildWarmSocialReply,registryCapabilityIntro,registryDomainConfig,getKnowledgeHints,resolveKnowledgeDomainForCompose,domainKnowledgeReply,isRecoveryRequested,isUsableFinalReply,forceCognitionCompleteReply,identityReply,directiveReply,contextualDirectiveReply,domainQuestionReply,detectArchitectureReasoning,hotFallbackReply,buildReply,safeReply,buildMemoryPatch,ensureFinalReply,assertFinalEnvelope,applyConversationQuality,toneProfile,contextCarrySummary,hasNaturalDepth}};
<<<<<<< HEAD
=======
"use strict";
...
async function run(routed={},input={}){return composeMarionResponse(routed,input);}
module.exports={VERSION,STATE_SPINE_SCHEMA,STATE_SPINE_SCHEMA_COMPAT,VALID_INTENTS,INTENT_TO_DOMAIN,composeMarionResponse,run,default:composeMarionResponse,ensureFinalReply,assertFinalEnvelope,_internal:{normalizeResolvedEmotion,resolveEffectiveEmotion,hasEmotionalContinuityCue,previousEmotionalContinuity,emotionalProgressionProfile,emotionalProgressionReply,shouldEmotionInfluenceIntent,extractText,detectIdentityIntent,detectDirectiveIntent,extractContextCarry,hasContextCarry,protectsContextFromOverride,resolveIntent,resolveDomain,isBlockedLoopReply,isGreetingOnly,isHowAreYouTurn,isWarmSocialTurn,isCapabilityQuestion,buildWarmSocialReply,registryCapabilityIntro,registryDomainConfig,getKnowledgeHints,resolveKnowledgeDomainForCompose,domainKnowledgeReply,isRecoveryRequested,isUsableFinalReply,forceCognitionCompleteReply,identityReply,directiveReply,contextualDirectiveReply,domainQuestionReply,detectArchitectureReasoning,hotFallbackReply,buildReply,safeReply,buildMemoryPatch,ensureFinalReply,assertFinalEnvelope,applyConversationQuality,toneProfile,contextCarrySummary,hasNaturalDepth}};
>>>>>>> a6112a3d (Integrate Marion domain runtime knowledge lanes)
