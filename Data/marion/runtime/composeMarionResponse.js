...
function knowledgeDomainReply(knowledgeDomain,text,input={},routed={}){const k=normalizeKnowledgeDomain(knowledgeDomain);if(!k)return"";if(isKnowledgeDomainActivationRequest(text))return activateDomainReply(k);const shortConcept=isShortConceptFollowup(text);const q=shortConcept?conceptPromptForDomain(k,text):text;const direct=directDomainDepthAnswer(k,q,input);if(domainDepthFloor(k,q,direct))return direct;const registryReply=registryKnowledgeAnswer(k,q);if(domainDepthFloor(k,q,registryReply))return registryReply;const fallback=shortConcept?domainConceptFallbackAnswer(k,text):"";return direct||registryReply||fallback||"";}
...
function domainConfidenceUserReply(input = {}, routed = {}) {
  ...
  const activeDomain=activeKnowledgeDomainFromContext(input,routed,text);if(activeDomain&&isShortConceptFollowup(text)){const ans=knowledgeDomainReply(activeDomain,text,input,routed);if(ans)return ans;}
