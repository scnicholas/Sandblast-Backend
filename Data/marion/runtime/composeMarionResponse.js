...
function buildAsterPublicAnswer(text="",input={}){
  ...
  const hasName=/\b(?:aster|environmental pathway|environmental gateway)\b/i.test(source);
  const hasIntent=/\b(?:explain|what|does|do|clear sentence|one sentence|environmental|climate|weather|signal|pathway|gateway|risk|regional)\b/i.test(source);
  if(hasName&&hasIntent){
