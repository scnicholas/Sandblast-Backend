...
/**
 * LingoLinkResponseEnvelope
 *
 * Standard output contract from LingoLink back to Marion.
 */
...
function createLingoLinkResponseEnvelope(input = {}) {
  ...
  if (!finalText) {
    warnings.push('No final text was produced by LingoLink.');
  }
  ...
  return {
    ...
    provider: input.provider || 'lingolink-core',
    metadata: {
...
function createLingoLinkFallbackResponse(input = {}) {
  ...
  return createLingoLinkResponseEnvelope({
    ...
    fallbackUsed: true,
    warnings: [
      input.reason || 'LingoLink fallback response created.'
    ],
    provider: 'lingolink-fallback'
  });
}

function validateLingoLinkResponseEnvelope(envelope = {}) {
  const errors = [];
...
module.exports = {
  ...
  validateLingoLinkResponseEnvelope
};
