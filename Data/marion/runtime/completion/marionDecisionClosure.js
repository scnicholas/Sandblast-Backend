...
/* MARION_NUANCE_PHASE_B_DecisionClosureCohesion_V1_START */
(function marionNuancePhaseBDecisionClosureCohesionV1(){
  "use strict";
  const api=module.exports&&typeof module.exports==="object"?module.exports:null;
  if(!api||api.__marionNuancePhaseBDecisionClosureCohesionV1)return;
  ...
  const original=api.analyze;if(typeof original==="function")api.analyze=function(input={}){const b=phaseB(input),s=summary(b),p=s.primaryPragmaticIntent,correction=["direct_correction","indirect_correction","explicit_disagreement","polite_disagreement"].includes(p)||s.interactionState==="correction",forbidden=["request_for_reassurance","request_for_acknowledgement","tentative_commitment","reluctant_acceptance_possible","rhetorical_question_possible","sarcasm_possible"].includes(p),out=original.call(this,input);let result={...obj(out),phaseBControls:{correctionOverride:correction,primaryStance:s.primaryStance,primaryPragmaticIntent:p,literalIntentPreserved:true},conversationArchitectureHardStop:HARD_STOP_LAYER,completionDecisionLayer:20,stanceMayAuthorizeClosure:false,pragmaticIntentMayAuthorizeClosure:false,figurativeLanguageMayAuthorizeClosure:false};if((correction||forbidden)&&result.closureStatus==="closed")result={...result,closureStatus:"open",finalDecision:"",hardStopAtLayer20:false,phaseBClosureBlocked:true};return result;};

  api.MARION_NUANCE_PHASE_B_CONTRACT=PHASE_B_CONTRACT;
  api.MARION_LAYER_HARD_STOP=HARD_STOP_LAYER;
  api.__marionNuancePhaseBDecisionClosureCohesionV1=true;
})();
/* MARION_NUANCE_PHASE_B_DecisionClosureCohesion_V1_END */
