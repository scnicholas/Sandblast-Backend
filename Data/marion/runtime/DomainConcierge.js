...
/* MARION_ROUND4_SINGLE_PASS_CONCIERGE_V2_START */
(function(){"use strict";const api=module.exports;if(!api||api.__marionRound4SinglePassConciergeV2)return;let md=null;try{md=require("./marionRound4DomainIntegration.js")}catch(_){return}const V="nyx.marion.round4.singlePassConcierge/2.0",cache=new WeakMap();function wrap(fn){if(typeof fn!=="function"||fn.__round4single)return fn;if(cache.has(fn))return cache.get(fn);const w=function(){const args=arguments,input=args&&args[0],plan=md.build(input,args&&args[1]);const r=fn.apply(this,args),done=v=>{if(!v||typeof v!=="object"||plan.domains.length<2)return v;return Object.assign({},v,{action:"route",needsClarifier:false,clarify:false,answerOnly:true,domain:plan.primaryDomain,knowledgeDomain:plan.primaryDomain,secondaryDomains:plan.secondaryDomains,multiDomainIntegration:plan,round4RoutingFinalized:true,routingReentryAllowed:false,recomputeProhibited:true,executionAuthorized:false,noUserFacingDiagnostics:true});};return r&&typeof r.then==="function"?r.then(done):done(r)};w.__round4single=true;cache.set(fn,w);return w}for(const n of["route","run","process","decide","handle","orchestrate","concierge"]){if(typeof api[n]==="function")api[n]=wrap(api[n])}api.buildRound4SinglePassPlan=(input,options)=>md.build(input,options);api.MARION_ROUND4_SINGLE_PASS_CONCIERGE_VERSION=V;api.__marionRound4SinglePassConciergeV2=true;
})();
