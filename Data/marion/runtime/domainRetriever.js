...
const VERSION = "PUBLIC-MARION-IDENTITY-NONRETRIEVAL-COHESION-R1 + domainRetriever v1.5.0 PRIORITY2-RETRIEVER-COHESION-HARDENING + BUSINESS-STRATEGY-ALIAS-COMPAT + INDEX-COHESION + REGISTRY-PATH-ALIGNMENT + DATA-DOMAINS-ROOT-LOCK + DOMAIN-ISOLATION-GUARD";
...
/* MARION_ROUND4_BOUNDED_RETRIEVAL_V2_START */
(function(){"use strict";const api=module.exports;if(!api||api.__marionRound4BoundedRetrievalV2)return;let md=null;try{md=require("./marionRound4DomainIntegration.js")}catch(_){return}const V="nyx.marion.round4.boundedRetrieval/2.0";
function timeout(ms,label){return new Promise(resolve=>setTimeout(()=>resolve({__timeout:true,error:label||"domain_budget_exceeded"}),ms));}
api.retrieveDomains=async function(input,options={}){const plan=md.build(input,options),started=Date.now(),fn=typeof api.retrieveDomain==="function"?api.retrieveDomain:api.retrieve;if(typeof fn!=="function")return {version:V,...plan,results:[],degraded:true,error:"retriever_unavailable",elapsedMs:Date.now()-started};const jobs=plan.domains.map(domain=>Promise.race([Promise.resolve().then(()=>fn.call(api,domain,input,Object.assign({},options,{domain,allowGeneralFallback:false,maxResults:Math.min(4,Number(options.maxResults)||4),maxWalkDepth:Math.min(2,Number(options.maxWalkDepth)||2)}))).then(result=>({domain,ok:true,result})).catch(err=>({domain,ok:false,error:err&&err.message?String(err.message):"retrieval_failed"})),timeout(plan.perDomainBudgetMs).then(v=>({domain,ok:false,timedOut:true,error:v.error}))]));const aggregate=Promise.all(jobs);const results=await Promise.race([aggregate,timeout(plan.totalBudgetMs,"total_retrieval_budget_exceeded")]);const finalResults=Array.isArray(results)?results:plan.domains.map(domain=>({domain,ok:false,timedOut:true,error:results.error}));return {version:V,primaryDomain:plan.primaryDomain,secondaryDomains:plan.secondaryDomains,domains:plan.domains,results:finalResults,successfulDomains:finalResults.filter(x=>x.ok).map(x=>x.domain),failedDomains:finalResults.filter(x=>!x.ok).map(x=>x.domain),degraded:finalResults.some(x=>!x.ok),partialFailureAllowed:true,domainIsolationRequired:true,noCrossDomainBleed:true,recomputeProhibited:true,executionAuthorized:false,noUserFacingDiagnostics:true,elapsedMs:Date.now()-started,budgetMs:plan.totalBudgetMs};};
api.MARION_ROUND4_BOUNDED_RETRIEVAL_VERSION=V;api.__marionRound4BoundedRetrievalV2=true;
})();
