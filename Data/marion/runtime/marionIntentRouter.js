...
/* MARION_PRIVATE_CONTEXTUAL_ENGINEERING_ROUTER_HARDLOCK_V5_START */
(function marionPrivateContextualEngineeringRouterHardlockV5(){
  "use strict";
  const VERSION="nyx.marion.privateContextualEngineeringRouter/5.0";
  ...
  function sid(v){const x=O(v),b=O(x.body),p=O(x.payload),m=O(x.meta);return T(x.sessionId||x.conversationId||x.threadId||b.sessionId||b.conversationId||p.sessionId||p.conversationId||m.sessionId||"private-marion",180);}
  ...
  function stamp(node,sub){if(!node||typeof node!=="object"||Array.isArray(node))return node;node.domain="technical";node.primaryDomain="technical";node.selectedDomain="technical";node.knowledgeDomain="technical";node.activeFeatureLane="technical";node.activeTask=sub;node.activeSubject=sub;node.r18CLawRealWorldAssessment=false;node.lawTechnicalSurgeryGuard=true;node.staleLawCarrySuppressed=true;node.legalCategory="";node.legalCategories=[];return node;}
  function project(value,input){if(!privateCtx(input)||!value||typeof value!=="object"||Array.isArray(value))return value;const q=prompt(input),c=classify(q),key=sid(input);if(O(input).newSession===true||O(input).firstTurn===true||O(input).resetSession===true||O(input).freshSession===true)sessions.delete(key);const prev=sessions.get(key)||{},tc=techCarry(input)||techCarry(value);let lane=prev.lane||"";if(c.social)lane="";else if(c.legal)lane="law";else if(c.tech||tc)lane="technical";const sub=subject(input,q);if(c.social)sessions.delete(key);else if(lane)sessions.set(key,{lane,subject:prev.subject||sub,updatedAt:Date.now()});prune();const force=!c.social&&!c.legal&&(c.tech||(c.contextual&&(lane==="technical"||prev.lane==="technical"||tc)));if(!force)return value;const out={...value};stamp(out,prev.subject||sub);out.marionIntent=stamp({...O(out.marionIntent),intent:"technical_debug",subIntent:"contextual_engineering_followup",reason:"private_contextual_engineering_precedence",routeLock:true,requiresComposer:true,requiresFinalEnvelope:true},prev.subject||sub);out.routing=stamp({...O(out.routing),intent:"technical_debug",mode:"debug",depth:"forensic",routeLock:true,routeAmbiguous:false,routeFailClosed:false,answerMode:"implementation_grade",preferredStyle:"direct_forensic",effectivePrompt:q,rawTurnText:q},prev.subject||sub);out.routing.domainConfidence={...O(out.routing.domainConfidence),confidence:0.98,confidenceScore:0.98,band:"high",confidenceBand:"high",primaryDomain:"technical",knowledgeDomain:"technical",routeLocked:true,ambiguous:false,reason:"private_contextual_engineering_precedence"};out.meta={...O(out.meta),privateContextualEngineeringRouterVersion:VERSION,expectedDomain:"technical",staleLawCarrySuppressed:true,activeSubject:prev.subject||sub};return out;}
  function wrap(fn,name){if(typeof fn!=="function"||fn.__marionPrivateContextualEngineeringV5)return fn;const w=function(){const input=arguments&&arguments.length?arguments[0]:{},r=fn.apply(this,arguments),p=v=>project(v,input);return r&&typeof r.then==="function"?r.then(p):p(r);};try{Object.keys(fn).forEach(k=>w[k]=fn[k]);}catch(_){}w.__marionPrivateContextualEngineeringV5=true;w.__wrappedName=name;return w;}
  try{const api=module.exports&&typeof module.exports==="object"?module.exports:null;if(!api)return;for(const n of["routeMarionIntent","route","run","handle","default","classifyIntent"])if(typeof api[n]==="function")api[n]=wrap(api[n],n);api.MARION_PRIVATE_CONTEXTUAL_ENGINEERING_ROUTER_VERSION=VERSION;api.projectMarionPrivateContextualEngineeringRoute=project;}catch(_){}
})();
/* MARION_PRIVATE_CONTEXTUAL_ENGINEERING_ROUTER_HARDLOCK_V5_END */
