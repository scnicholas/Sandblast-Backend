...
(function(){
  "use strict";
  const V="nyx.guideOrchestration.marionIntentRouter/4.0-steps10-11-12",PC="nyx.guideActionPlan/1.1",EC="nyx.guideExecution/1.0";
  function o(v){return v&&typeof v==="object"&&!Array.isArray(v)?v:{}}function x(v,n=120){return String(v==null?"":v).replace(/[\u0000-\u001f\u007f]/g,"").replace(/\s+/g," ").trim().slice(0,n)}function h(v){let n=2166136261,s=String(v==null?"":v);for(let i=0;i<s.length;i++){n^=s.charCodeAt(i);n=Math.imul(n,16777619)}return(n>>>0).toString(16)}
  function prompt(v,args){const a=[],seen=new Set();function w(q,d){if(q==null||d>4)return;if(typeof q==="string"){a.push(x(q,1800));return}if(typeof q!=="object"||seen.has(q))return;seen.add(q);q=o(q);for(const k of["userText","rawUserText","message","text","prompt","query","effectivePrompt","normalizedUserIntent"])if(typeof q[k]==="string")a.push(x(q[k],1800));for(const k of["payload","body","meta","routing","guideContext","runtimeState","state","session"])w(q[k],d+1)}for(const q of Array.from(args||[]).concat([v]))w(q,0);return a.filter(Boolean).join(" ").slice(0,2800)}
  function context(v,args){let c={};const seen=new Set();function w(q,d){if(!q||typeof q!=="object"||d>4||seen.has(q))return;seen.add(q);q=o(q);if(!Object.keys(c).length)c=o(q.guideContext||q.publicGuideContinuity);for(const k of["payload","body","meta","routing","runtimeState","state","session"])w(q[k],d+1)}for(const q of Array.from(args||[]).concat([v]))w(q,0);return c}
  function infer(t,c){t=x(t,2800).toLowerCase();const current=x(o(c).currentLane||"home",32),a=[];function add(type,target,label,lane){if(a.some(q=>q.type===type&&q.target===target))return;const id=`act_${h(`${type}|${target}|${label}`)}`;a.push({contract:"nyx.guideAction/1.2",id,type,target,targetKey:target,lane,expectedLane:current,rollbackLane:current,label,requiresUserGesture:true,autoExecute:false,idempotent:true,idempotencyKey:h(`${id}|${current}`),serverExecutionAllowed:false,symbolicTargetOnly:true})}const go="(?:open|show|watch|play|start|go to|take me to|continue to|launch)";if(new RegExp(`\\b${go}\\b.{0,42}\\b(radio|live stream|music)\\b`).test(t))add("play_radio","sandblast_radio","Play Radio","live");if(/\b(stop|pause|turn off)\b.{0,30}\b(radio|stream|music)\b/.test(t))add("stop_radio","sandblast_radio","Stop Radio","live");if(new RegExp(`\\b${go}\\b.{0,46}\\broku\\b`).test(t))add("open_roku","sandblast_roku","Open Sandblast on Roku","roku");if(new RegExp(`\\b${go}\\b.{0,46}\\b(cartoon|cartoons)\\b`).test(t))add("open_tv","sandblast_cartoons","Open Cartoons","watch");if(new RegExp(`\\b${go}\\b.{0,46}\\b(classic|classics|classic movies)\\b`).test(t))add("open_tv","sandblast_classics","Open Classics","watch");if(new RegExp(`\\b${go}\\b.{0,46}\\b(sandblast tv|television|tv)\\b`).test(t))add("open_tv","sandblast_tv","Open Sandblast TV","watch");if(new RegExp(`\\b${go}\\b.{0,42}\\b(synapse|news)\\b`).test(t))add("open_synapse","synapse","Open Synapse","news");if(/\b(open|show|ask|use)\b.{0,30}\b(nyx|guide|chat)\b/.test(t))add("open_guide","nyx_guide","Ask Nyx","search");if(/\b(go|return|take me)\b.{0,24}\b(home|ecosystem)\b/.test(t))add("navigate","sandblast_home","Open Home","home");return a.slice(0,6)}
  function project(v,args){if(!v||typeof v!=="object"||Array.isArray(v))return v;const c=context(v,args),actions=infer(prompt(v,args),c);if(!actions.length)return v;const planId=`plan_${h(`${x(c.currentLane||"home",32)}|${actions.map(a=>a.id).join("|")}`)}`,plan={contract:PC,version:V,planId,actionCount:actions.length,actions,requiresUserGesture:true,autoExecute:false,idempotencyRequired:true,rollbackRequired:true,executionAuthority:"client_user_gesture",serverExecutionAllowed:false,expiresAt:Date.now()+300000},out={...v,guideActionPlan:plan,guideActions:actions,nyxGuideExecution:{contract:EC,version:V,planId,status:"pending_user_gesture",currentLane:x(c.currentLane||"home",32),previousLane:x(c.previousLane||c.currentLane||"home",32),revision:Math.max(0,Number(c.revision||0)||0),requiresUserGesture:true,autoExecute:false,idempotencyRequired:true,rollbackRequired:true,publicSessionOnly:true,privateMemoryAccess:false}};out.routing={...o(out.routing),guideNavigation:true,guideRouteLocked:true,guideTarget:actions[0].target,guideTargetLane:actions[0].lane,guideActionPlan:plan,noKnowledgeDomainHijack:true};out.marionIntent={...o(out.marionIntent),guideNavigation:true,guideActionPlan:plan,finalReplyAuthority:false};return out}
  function wrap(fn){if(typeof fn!=="function"||fn.__nyx101112Intent)return fn;const w=function(){const a=arguments,r=fn.apply(this,a);return r&&typeof r.then==="function"?r.then(v=>project(v,a)):project(r,a)};try{Object.keys(fn).forEach(k=>w[k]=fn[k])}catch(_){}w.__nyx101112Intent=true;return w}
  try{if(typeof module.exports==="function")module.exports=wrap(module.exports);const api=module.exports&&typeof module.exports==="object"?module.exports:null;if(api){for(const n of["routeMarionIntent","route","classifyIntent","detectKnowledgeDomain","run","handle","default"])if(typeof api[n]==="function")api[n]=wrap(api[n]);api.NYX_GUIDE_STEPS_10_11_12_INTENT_VERSION=V;api.buildNyxGuideIntentActions=infer;api.attachNyxGuideIntentExecution=(v,i)=>project(v,[i||{}])}}catch(_){}
})();
...
(function(){
  ...
  try{
    ...
    if(api){
      for(const name of ["routeMarionIntent", "route", "classifyIntent", "default"])if(typeof api[name]==="function")api[name]=wrap(api[name],name);
...
/* MARION_PRIVATE_RUNTIME_CONTINUITY_ROUTER_V6_START */
(function marionPrivateRuntimeContinuityRouterV6(){
  "use strict";
  const VERSION="nyx.marion.intentRouter.privateRuntimeContinuity/6.0";
  const sessions=new Map(),TTL=45*60*1000,MAX=1000;
  function O(v){return v&&typeof v==="object"&&!Array.isArray(v)?v:{};}
  function T(v,n=6000){try{return String(v==null?"":v).replace(/[\u0000-\u001f\u007f]/g," ").replace(/\s+/g," ").trim().slice(0,n);}catch(_){return"";}}
  function N(v){return T(v).toLowerCase().replace(/[’‘]/g,"'").replace(/[^a-z0-9]+/g," ").replace(/\s+/g," ").trim();}
  function prompt(v){const x=O(v),b=O(x.body),p=O(x.payload),m=O(x.meta);return T(x.rawUserText||x.userText||x.prompt||x.query||x.text||x.message||b.rawUserText||b.userText||b.prompt||b.query||b.text||b.message||p.rawUserText||p.userText||p.prompt||p.query||p.text||p.message||m.rawUserText||m.userText||m.prompt||m.text);}
  function privateCtx(v){const x=O(v),b=O(x.body),p=O(x.payload),m=O(x.meta);const path=T(x.route||x.path||x.originalUrl||x.url||m.route).toLowerCase();return x.privateAdminConversation===true||x.marionAdminConversation===true||x.directMarionAdminInterface===true||x.adminVerified===true||x.passwordFreeTestChat===true||b.marionAdminConversation===true||p.marionAdminConversation===true||/\/(?:api\/)?private\/marion|marion_admin|admin_conversation/.test(path+" "+T(x.source||x.inputChannel||x.adminInterfaceScope).toLowerCase());}
  function sid(v){const x=O(v),b=O(x.body),p=O(x.payload),m=O(x.meta),s=O(x.session);return T(x.sessionId||x.conversationId||x.threadId||b.sessionId||b.conversationId||p.sessionId||p.conversationId||m.sessionId||s.sessionId||"private-marion",180);}
  ...
  function classify(q){const n=N(q);const social=/^(?:hi|hello|hey|good morning|good afternoon|good evening|thanks|thank you|okay|ok|got it)(?: marion)?$/.test(n);const contextual=/^(?:go deeper|continue|keep going|next|next step|next steps|what next|what now|then what|what should be fixed first|what should we fix first|why is that the first priority|why first|what is the safest implementation order|safest implementation order|how do we validate the repair|how should we validate the repair|how do we test the repair|what is the main risk|what happens after that|what is the first concrete defect|what should i inspect first)$/.test(n)||/^(?:what could break|what could go wrong|how do we validate|how should we validate|how do we test)(?:\s+.*)?$/.test(n);const tech=/\b(?:surgical autopsy|autopsy|javascript|index js|chatengine|chat engine|intent router|domain router|routing file|runtime|backend|frontend|bridge|final envelope|state spine|module|function|payload|syntax|patch|hotfix)\b/.test(n);const legal=!tech&&/\b(?:legal advice|legal risk|lawyer|attorney|court|tribunal|lawsuit|liability|negligence|damages|contract|agreement|clause|breach|copyright|licensing rights|trademark|patent|jurisdiction|statute|regulation|compliance)\b/.test(n);return{n,social,contextual,tech,legal};}
  function activeTechnical(input){const x=O(input),nodes=[x,O(x.routing),O(x.marionIntent),O(x.meta),O(x.sessionPatch),O(x.memoryPatch),O(x.previousMemory),O(x.payload)];for(const node of nodes){const d=N(node.domain||node.primaryDomain||node.selectedDomain||node.knowledgeDomain||node.activeFeatureLane);if(d==="technical"||d==="ai"||d==="cyber")return true;const subject=N(node.activeTask||node.currentTask||node.lastValidTask||node.activeSubject||node.continuationAnchor||node.technicalTarget||node.targetFile||node.targetPath);if(/\b(?:javascript|routing|router|runtime|backend|frontend|bridge|index js|chatengine|state spine|final envelope|module|function|payload|patch|hotfix)\b/.test(subject))return true;}return false;}
  function technicalSubject(v){return T(v,700).replace(/\blaw[-\s]?routing\b/ig,"domain-routing").replace(/\blegal[-\s]?routing\b/ig,"domain-routing").replace(/\blaw routing\b/ig,"domain routing");}
  function subject(input,q){const x=O(input),nodes=[x,O(x.routing),O(x.marionIntent),O(x.meta),O(x.sessionPatch),O(x.memoryPatch),O(x.previousMemory),O(x.payload)],fields=["activeTask","currentTask","lastValidTask","activeSubject","continuationAnchor","technicalTarget","targetFile","targetPath"];for(const node of nodes)for(const f of fields){const z=T(node[f],700);if(z&&!classify(z).contextual&&!classify(z).social)return technicalSubject(z);}return technicalSubject(T(q,700)||"the active JavaScript routing repair");}
  function stamp(node,sub,q){if(!node||typeof node!=="object"||Array.isArray(node))return node;const qualified=`Technical debugging follow-up for ${sub}: ${T(q,1200)}`;node.domain="technical";node.primaryDomain="technical";node.selectedDomain="technical";node.knowledgeDomain="technical";node.activeFeatureLane="technical";node.activeTask=sub;node.activeSubject=sub;node.effectivePrompt=qualified;node.normalizedUserIntent=qualified;node.r18CLawRealWorldAssessment=false;node.lawTechnicalSurgeryGuard=true;node.staleLawCarrySuppressed=true;node.legalCategory="";node.legalCategories=[];return node;}
  function project(value,input){if(!privateCtx(input)||!value||typeof value!=="object"||Array.isArray(value))return value;const q=prompt(input),c=classify(q),key=sid(input),x=O(input);if(x.newSession===true||x.firstTurn===true||x.resetSession===true||x.freshSession===true||x.clearMemory===true)sessions.delete(key);let prior=sessions.get(key)||{};if(c.social){sessions.delete(key);return value;}if(c.legal){sessions.set(key,{lane:"law",subject:T(q,700),updatedAt:Date.now()});prune();const out={...value,domain:"law",primaryDomain:"law",selectedDomain:"law",knowledgeDomain:"law",activeFeatureLane:"law",r18CLawRealWorldAssessment:true,lawTechnicalSurgeryGuard:false,staleLawCarrySuppressed:false};out.marionIntent={...O(out.marionIntent),intent:"domain_question",subIntent:"legal_information",domain:"law",primaryDomain:"law",knowledgeDomain:"law",routeLock:true,requiresComposer:true,requiresFinalEnvelope:true};out.routing={...O(out.routing),intent:"domain_question",domain:"law",primaryDomain:"law",selectedDomain:"law",knowledgeDomain:"law",mode:"educational_law_information",depth:"balanced",routeLock:true,routeAmbiguous:false,routeFailClosed:false,rawTurnText:q};out.meta={...O(out.meta),privateRuntimeContinuityRouterVersion:VERSION,expectedDomain:"law",explicitCurrentLawRequest:true,routerSessionCache:true};return out;}if(c.tech){prior={lane:"technical",subject:subject(input,q),updatedAt:Date.now()};sessions.set(key,prior);}prune();const force=c.tech||(c.contextual&&(activeTechnical(input)||prior.lane==="technical"));if(!force)return value;const sub=prior.subject||subject(input,q),out={...value};sessions.set(key,{lane:"technical",subject:sub,updatedAt:Date.now()});stamp(out,sub,q);out.marionIntent=stamp({...O(out.marionIntent),intent:"technical_debug",subIntent:c.contextual?"contextual_engineering_followup":"technical_debug",reason:"private_current_state_engineering_precedence",routeLock:true,requiresComposer:true,requiresFinalEnvelope:true},sub,q);out.routing=stamp({...O(out.routing),intent:"technical_debug",mode:"debug",depth:"forensic",routeLock:true,routeAmbiguous:false,routeFailClosed:false,answerMode:"implementation_grade",preferredStyle:"direct_forensic",rawTurnText:q},sub,q);out.routing.domainConfidence={...O(out.routing.domainConfidence),confidence:0.98,confidenceScore:0.98,band:"high",confidenceBand:"high",primaryDomain:"technical",knowledgeDomain:"technical",routeLocked:true,ambiguous:false,reason:"private_current_state_engineering_precedence"};out.meta={...O(out.meta),privateRuntimeContinuityRouterVersion:VERSION,expectedDomain:"technical",staleLawCarrySuppressed:true,activeSubject:sub,routerSessionCache:true};return out;}
  function wrap(fn,name){if(typeof fn!=="function"||fn.__marionPrivateRuntimeContinuityRouterV6)return fn;const w=function(){const input=arguments&&arguments.length?arguments[0]:{},r=fn.apply(this,arguments),p=v=>project(v,input);return r&&typeof r.then==="function"?r.then(p):p(r);};try{Object.keys(fn).forEach(k=>w[k]=fn[k]);}catch(_){}w.__marionPrivateRuntimeContinuityRouterV6=true;w.__wrappedName=name;return w;}
  try{const api=module.exports&&typeof module.exports==="object"?module.exports:null;if(!api)return;for(const n of["routeMarionIntent","route","run","handle","default","classifyIntent"])if(Object.prototype.hasOwnProperty.call(api,n)&&typeof api[n]==="function")api[n]=wrap(api[n],n);api.MARION_PRIVATE_RUNTIME_CONTINUITY_ROUTER_V6=Object.freeze({version:VERSION,sessionCache:true,sessionCacheBounded:true,currentTurnRouting:true,activeStateContinuity:true,staleLawSuppression:true,nyxPublicArchitecturePreserved:true});api.projectMarionPrivateRuntimeContinuityRoute=project;}catch(_){}
})();
...
/* MARION_OUTCOME_FLOW_LAYERS_12_13_14_ROUTER_V14_START */
...
/* MARION_REALWORLD_AI_ROUTE_LOCK_V1_START */
(function(){"use strict";const a=module.exports;if(!a||a.__marionRealworldAiRouteLockV1)return;const V="nyx.marion.realworldAiRouteLock/1.0",H=28;
function o(v){return v&&typeof v==="object"&&!Array.isArray(v)?v:{}}
function s(v){try{return String(v==null?"":v).replace(/\s+/g," ").trim()}catch(_){return""}}
function p(v){const x=o(v),b=o(x.body),q=o(x.payload);return s(x.text||x.prompt||x.userText||x.userQuery||x.query||x.message||b.text||b.prompt||q.text||q.prompt)}
function match(v){const t=p(v).toLowerCase();return /\b(?:real[- ]world|physical environment|sensors?|cameras?|external systems?|responsibly interact)\b/.test(t)&&/\b(?:cognitive|marion|system|architecture|application|deploy|interaction)\b/.test(t)}
function locked(v){const q=p(v);return{ok:true,version:V,contract:"nyx.marion.intent/2.5",intent:"domain_question",domain:"ai",knowledgeDomain:"ai",mode:"ai_architecture_reasoning",depth:"forensic",preferredStyle:"implementation_grade",needsClarifier:false,clarifier:"",answerOnly:true,actionRequired:false,currentTurnAuthority:true,hardStopLayer:H,userQuery:q,text:q,prompt:q,routing:{intent:"domain_question",domain:"ai",knowledgeDomain:"ai",mode:"ai_architecture_reasoning",depth:"forensic",preferredStyle:"implementation_grade",needsClarifier:false,answerOnly:true,currentTurnAuthority:true}}}
const c=new WeakMap;for(const n of["routeMarionIntent","route","run","default","classifyIntent"]){const f=a[n];if(typeof f!=="function")continue;let w=c.get(f);if(!w){w=function(input){if(match(input))return locked(input);return f.apply(this,arguments)};c.set(f,w)}a[n]=w}a.MARION_REALWORLD_AI_ROUTE_LOCK_VERSION=V;a.MARION_LAYER_HARD_STOP=H;a.__marionRealworldAiRouteLockV1=true})();
/* MARION_REALWORLD_AI_ROUTE_LOCK_V1_END */
...
(function marionPublicSecurityBoundaryRouteLockR3(){
  "use strict";
  const api=module.exports&&typeof module.exports==="object"?module.exports:null;
  if(!api||api.__marionPublicSecurityBoundaryRouteLockR3)return;
  const VERSION="nyx.marion.publicSecurityBoundaryRouteLock/3.0";
  function obj(v){return v&&typeof v==="object"&&!Array.isArray(v)?v:{};}
  function text(v){try{return String(v==null?"":v).replace(/[\u0000-\u001f\u007f]/g," ").replace(/\s+/g," ").trim();}catch(_){return"";}}
  ...
  function lock(result,input,kind){if(!kind)return result;const base=obj(result),routing=obj(base.routing),subIntent="public_"+kind,reason=subIntent+"_terms";return{...base,ok:base.ok!==false,contract:base.contract||"nyx.marion.intent/2.5",intent:"identity_query",domain:"identity",knowledgeDomain:"",mode:"identity",depth:"identity_boundary",preferredStyle:"identity_clear",subIntent,reason,confidence:0.995,needsClarifier:false,clarifier:"",answerOnly:true,actionRequired:false,currentTurnAuthority:true,publicSurfaceSafe:true,securityBoundary:true,routing:{...routing,intent:"identity_query",domain:"identity",knowledgeDomain:"",mode:"identity",depth:"identity_boundary",preferredStyle:"identity_clear",subIntent,reason,confidence:0.995,needsClarifier:false,answerOnly:true,actionRequired:false,currentTurnAuthority:true,publicSurfaceSafe:true,securityBoundary:true},meta:{...obj(base.meta),publicSecurityBoundaryRouteLocked:true,publicSecurityBoundaryRouteLockVersion:VERSION,noUserFacingDiagnostics:true}};}
  const names=["routeMarionIntent","route","run","handle","default","classifyIntent","classify"];
  const cache=new WeakMap();
  for(const name of names){const fn=api[name];if(typeof fn!=="function")continue;let wrapped=cache.get(fn);if(!wrapped){wrapped=function(){const input=arguments[0],kind=classify(input),out=fn.apply(this,arguments),apply=v=>lock(v,input,kind);return out&&typeof out.then==="function"?out.then(apply):apply(out);};try{Object.keys(fn).forEach(k=>wrapped[k]=fn[k]);}catch(_){}cache.set(fn,wrapped);}api[name]=wrapped;}
  api.classifyPublicSecurityBoundary=function(input){const kind=classify(input);return kind?{intent:"identity_query",subIntent:"public_"+kind,reason:"public_"+kind+"_terms",confidence:0.995,securityBoundary:true}:null;};
  api.MARION_PUBLIC_SECURITY_BOUNDARY_ROUTE_LOCK_VERSION=VERSION;
...
/* MARION_ROUTER_IDENTITY_SECURITY_TERMINAL_R6_START */
(function marionRouterIdentitySecurityTerminalR6(){
  "use strict";
  const VERSION="nyx.marion.routerIdentitySecurityTerminal/6.0";
  ...
  const api=module.exports&&typeof module.exports==="object"?module.exports:null;
  if(!api||api.__marionRouterIdentitySecurityTerminalR6)return;
  function routePacket(input){
    const kind=classify(input);
    if(!kind)return null;
    const packet=build(kind,"marionIntentRouter");
    return{contract:"nyx.marion.intent/2.5",intent:"identity_query",domain:"identity",knowledgeDomain:"",mode:"identity",depth:kind==="marion_identity"?"identity_baseline":"identity_boundary",preferredStyle:"identity_clear",subIntent:packet.subIntent,reason:packet.reason,confidence:0.999,needsClarifier:false,clarifier:"",answerOnly:true,actionRequired:false,currentTurnAuthority:true,publicSurfaceSafe:true,securityBoundary:kind!=="marion_identity",identitySecurityHandled:true,identitySecurityTerminal:true,terminalReply:packet.reply,finalEnvelope:packet.finalEnvelope,routing:{intent:"identity_query",domain:"identity",knowledgeDomain:"",mode:"identity",depth:kind==="marion_identity"?"identity_baseline":"identity_boundary",preferredStyle:"identity_clear",subIntent:packet.subIntent,reason:packet.reason,confidence:0.999,needsClarifier:false,answerOnly:true,actionRequired:false,currentTurnAuthority:true,publicSurfaceSafe:true,securityBoundary:kind!=="marion_identity",identitySecurityHandled:true,identitySecurityTerminal:true},meta:{replyAuthority:"nyx_public_identity_security_terminal_contract",identitySecurityHandled:true,identitySecurityTerminal:true,noUserFacingDiagnostics:true}};
  }
  for(const name of["routeMarionIntent","route","run","handle","default","classifyIntent","classify"]){
    if(typeof api[name]==="function"){
      const original=api[name];
      api[name]=function(){
        const terminal=routePacket(arguments[0]);
        if(terminal)return original.constructor&&original.constructor.name==="AsyncFunction"?Promise.resolve(terminal):terminal;
        return original.apply(this,arguments);
      };
    }
  }
  api.classifyPublicIdentitySecurityTerminal=classify;
  api.buildPublicIdentitySecurityTerminalRoute=routePacket;
  api.MARION_ROUTER_IDENTITY_SECURITY_TERMINAL_VERSION=VERSION;
  api.__marionRouterIdentitySecurityTerminalR6=true;
})();
/* MARION_ROUTER_IDENTITY_SECURITY_TERMINAL_R6_END */
...
/* MARION_PUBLIC_KNOWLEDGE_FAST_ROUTE_R13_START
 * Live canonical current-turn route lock for public six-domain questions.
 * This terminal layer is intentionally narrow: it never composes replies and
 * never intercepts authenticated/private Marion traffic.
 */
(function marionPublicKnowledgeFastRouteR13(){
  "use strict";
  const api=module.exports&&typeof module.exports==="object"?module.exports:null;
  if(!api||api.__marionPublicKnowledgeFastRouteR13)return;
  const V="nyx.marion.publicKnowledgeFastRoute/1.3-live-canonical";
  ...
  function wrap(name){const f=prior[name];return function(input){const x=build(input);return x||(f?f.apply(this,arguments):null)}}
  for(const n of names)api[n]=wrap(n);
  api.buildPublicKnowledgeFastRoute=build;
  api.classifyPublicKnowledgeFastDomain=function(input){if(priv(input))return"";const q=P(input);return Q(q)?D(q):""};
  api.MARION_PUBLIC_KNOWLEDGE_FAST_ROUTE_VERSION=V;
  api.__marionPublicKnowledgeFastRouteR13=true;
})();
