...
} = require("./LingoSentinelGlossaryRefiner");

const {
  recordTranslationTelemetry,
  getTranslationTelemetrySnapshot,
  clearTranslationTelemetry,
} = require("./LingoSentinelTranslationTelemetry");

const {
  detectLikelyLanguage,
} = require("./LingoSentinelLanguageDetector");
...
function detectLikelyLanguageLegacy(text = "") {
...
function normalizeRequest(input = {}) {
  ...
  const target = normalizeLanguageCode(
    explicitTarget ||
      memory.preferredTargetLanguage ||
      getDefaultTargetLanguage()
  );
  ...
  if ((!source || source === "auto") && input.speakerId && memory.speakerLanguages) {
    const rememberedSpeakerLanguage = memory.speakerLanguages[String(input.speakerId)];
    if (rememberedSpeakerLanguage) {
      source = normalizeLanguageCode(rememberedSpeakerLanguage);
    }
  }
...
async function routeTranslationRequestWithTelemetry(input = {}) {
  const startedAt = Date.now();

  try {
    const rawResult = await routeTranslationRequest(input);
    const result = refineTranslationResult(rawResult);
    ...
    recordTranslationTelemetry({
      ...
      result: {
        ...
        translationRequired: true,
        provider: "argos",
        source: input.source || input.from || input.sourceLanguage || null,
        target: input.target || input.to || input.targetLanguage || null,
        warnings: ["TRANSLATION_EXCEPTION"],
        error: error.message || "TRANSLATION_EXCEPTION",
      },
      startedAt,
      endedAt: Date.now(),
      error: error.message || "TRANSLATION_EXCEPTION",
...
module.exports = {
  ...
  detectLikelyLanguage,
  shouldrouteTranslationRequest: routeTranslationRequestWithTelemetry,
  routeTranslationRequest: routeTranslationRequestWithTelemetry,
  getArgosHealth,
  getArgosLanguages,
  normalizeLanguageCode,
  validateLanguagePair,
...
// PHASE_3D_EXPORT_COMPATIBILITY_PATCH
// Keeps older gateway imports stable after telemetry wrapping.
try {
  if (typeof routeTranslationRequestWithTelemetry === "function") {
    module.exports.routeTranslationRequest = routeTranslationRequestWithTelemetry;
  } else if (typeof routeTranslationRequest === "function") {
    module.exports.routeTranslationRequest = routeTranslationRequest;
  }

  if (typeof shouldRouteTranslationRequest === "function") {
    module.exports.shouldRouteTranslationRequest = shouldRouteTranslationRequest;
  }

  if (typeof detectLikelyLanguage === "function") {
    module.exports.detectLikelyLanguage = detectLikelyLanguage;
  }

  if (typeof getArgosHealth === "function") {
    module.exports.getArgosHealth = getArgosHealth;
  }

  if (typeof getArgosLanguages === "function") {
    module.exports.getArgosLanguages = getArgosLanguages;
  }

  if (typeof getTranslationTelemetrySnapshot === "function") {
    module.exports.getTranslationTelemetrySnapshot = getTranslationTelemetrySnapshot;
  }

  if (typeof clearTranslationTelemetry === "function") {
    module.exports.clearTranslationTelemetry = clearTranslationTelemetry;
  }

  if (typeof normalizeLanguageCode === "function") {
    module.exports.normalizeLanguageCode = normalizeLanguageCode;
  }

  if (typeof validateLanguagePair === "function") {
    module.exports.validateLanguagePair = validateLanguagePair;
  }
  ...
  if (typeof getSupportedLanguageCodes === "function") {
    module.exports.getSupportedLanguageCodes = getSupportedLanguageCodes;
  }

  if (typeof getSupportedLanguages === "function") {
    module.exports.getSupportedLanguages = getSupportedLanguages;
  }
} catch (error) {
  // Do not throw during module load; tests will expose missing exports.
}
