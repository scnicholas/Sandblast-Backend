...
const VERSION = '2.1.0-spontaneous-context-memory';
...
class LingoSentinelContextMemory {
  ...
  normalizeSessionId(sessionId) {
    const value = clamp(sessionId || 'default', 128).replace(/[^a-z0-9_.:-]/gi, '_');
  ...
  snapshot(sessionId, maxTurns = this.maxTurns) {
    const session = this.getSession(sessionId);
    return session.turns.slice(-Math.max(1, maxTurns)).map(turn => ({ ...turn }));
  }

  summarize(sessionId, maxTurns = this.maxTurns) {
    const turns = this.snapshot(sessionId, maxTurns);
