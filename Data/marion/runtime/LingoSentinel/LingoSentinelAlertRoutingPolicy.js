...
/**
 * LingoSentinelAlertRoutingPolicy
 *
 * Purpose:
 * Decides how Marion should route LingoSentinel unknown-language alerts.
 *
 * Scope:
 ...
 *
 * Authority Rule:
 * LingoSentinel signals.
 * Policy classifies.
 * Marion authorizes.
 */

const ALERT_ROUTING_POLICY_VERSION = "nyx.lingosentinel.alertRoutingPolicy/0.1";
...
function extractAlertSurface(payload = {}) {
  ...
  const alert = safeObject(
    source.unknownLanguageAlert ||
      ...
      safeObject(source.lingoSentinel).unknownLanguageAlert
  );

  const gatewayMeta = safeObject(source.gatewayMeta || safeObject(source.lingoSentinel).gatewayMeta);
  const dormantScanner = safeObject(source.dormantScanner || safeObject(source.lingoSentinel).dormantScanner);
  const scannerHeartbeat = safeObject(source.scannerHeartbeat || safeObject(source.lingoSentinel).scannerHeartbeat);
  ...
  return {
    ...
    correlationId,
    source: "LingoSentinelAlertRoutingPolicy"
  };
}
...
function buildAlertRoutingDecision(payload = {}, options = {}) {
  ...
  return {
    ...
    metadata: {
      ...
      gateway: safeString(surface.gatewayMeta.gateway || "LingoSentinel"),
      scanner: safeString(surface.scannerHeartbeat.scanner || "LingoSentinelDormantScanner")
    },
