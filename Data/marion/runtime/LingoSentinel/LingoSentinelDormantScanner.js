...
const { detectLanguage } = (() => {
  try {
    return require("./LingoSentinelLanguageDetect");
  } catch (_) {
    return {
      detectLanguage: null
    };
...
const { normalizeInput } = (() => {
  try {
    return require("./LingoSentinelNormalizer");
  } catch (_) {
    return {
      normalizeInput: null
    };
...
const SCANNER_VERSION = "nyx.lingosentinel.dormantScanner/0.1";
...
function buildScannerHeartbeat(options = {}) {
  ...
  return {
    ...
    source: "LingoSentinelDormantScanner"
  };
}
...
function scanDormantInput(input, options = {}) {
  ...
  if (config.enabled === false) {
    return {
      version: SCANNER_VERSION,
      scanId: `lingosentinel_scan_disabled_${timestamp}`,
      enabled: false,
      scanned: false,
      inputHash: "",
      lingoInput: {
      ...
      ? buildUnknownLanguageAlert(
          {
            ...
            languageMeta,
            gateway: "LingoSentinelDormantScanner"
          },
          {
  ...
  return {
    version: SCANNER_VERSION,
    scanId: `lingosentinel_scan_${timestamp}_${hashText(normalization.originalText)}`,
    enabled: true,
    scanned: true,
