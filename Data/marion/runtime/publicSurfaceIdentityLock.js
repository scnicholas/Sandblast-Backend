 ...
 * Phase 1+2B+3C Public Surface Identity + Loop/Fallback Surface Purge.
 *
 * Purpose:
 ...
 * - Phase 3C: public identity questions are answered directly and safely instead
 *   of being flattened into generic presence-check replies.
 */
const VERSION = "nyx.publicSurfaceIdentityLock/1.3-phase3c-public-identity-question-refinement";
...
const PUBLIC_WHO_PROMPT_RE = /\b(?:who\s+am\s+i\s+talking\s+to|who\s+are\s+you|what\s+are\s+you|what\s+is\s+nyx)\b/i;
...
function isPublicSurfaceContext(input={}){
  ...
  if(isPrivateOperatorContext(src)||isPrivateOperatorContext(body)||isPrivateOperatorContext({body,headers,payload})) return false;
...
function cleanPublicWhoReply(){ return identityRefinement && identityRefinement.PUBLIC_WHO_REPLY ? identityRefinement.PUBLIC_WHO_REPLY : "You’re speaking with Nyx, the Sandblast guide for media, radio, TV, discovery, and business tools."; }
function isInternalPublicLeak(value=""){ return INTERNAL_PUBLIC_LEAK_RE.test(safeStr(value)); }
function extractPrompt(context={}){
  if(identityRefinement && identityRefinement.extractPrompt){ try { const p=identityRefinement.extractPrompt(context); if(p) return p; } catch(_){} }
...
function isPublicIdentityQuestionPrompt(value=""){
  if(identityRefinement && identityRefinement.isPublicIdentityQuestionPrompt){ try { return identityRefinement.isPublicIdentityQuestionPrompt(value); } catch(_){} }
  return isPublicWhoPrompt(value);
}
function cleanPublicIdentityReply(prompt=""){
  if(identityRefinement && identityRefinement.cleanPublicIdentityReply){ try { return identityRefinement.cleanPublicIdentityReply(prompt); } catch(_){} }
  return cleanPublicWhoReply();
}
function sanitizePublicReply(value="", prompt=""){
  let t=safeStr(value);
  const p=safeStr(prompt);
  if(p && isPublicIdentityQuestionPrompt(p)) return cleanPublicIdentityReply(p);
  if(!t) return "";
  if(identityRefinement && identityRefinement.resolvePublicReply){ try { t=identityRefinement.resolvePublicReply(p,t); } catch(_){} }
  ...
  t=t.replace(/\bMarion is connected behind the response path\b/gi,"private system routing is not exposed here");
...
function projectPublicReplyFields(value,context={},depth=0){
  ...
  if(typeof value==="string") return sanitizePublicReply(value,prompt);
  ...
  for(const [key,child] of Object.entries(value)){
    ...
    if(REPLY_KEYS.has(key)){ out[key]=sanitizePublicReply(child,prompt); continue; }
    out[key]=projectPublicReplyFields(child,context,depth+1);
  }
  if(prompt && isPublicIdentityQuestionPrompt(prompt)){
    const direct=cleanPublicIdentityReply(prompt);
    ["reply","text","answer","response","message","output","spokenText","displayReply","publicReply","visibleReply","finalReply"].forEach(k=>{ out[k]=direct; });
...
function projectPublicPayload(payload={},context={}){
  ...
  let reply=sanitizePublicReply(extractReply(base),prompt);
  if(isPublicIdentityQuestionPrompt(prompt)) reply=cleanPublicIdentityReply(prompt);
  else if(isPublicWhoPrompt(prompt)) reply=cleanPublicWhoReply();
  ...
  return {
    ...
    publicIdentityQuestionRefinement:isPublicIdentityQuestionPrompt(prompt),
    operatorPersonalization:false,
    allowPersonalName:false,
    authenticatedOperator:false,
    revealBackendAgent:false,
    payload:{reply,text:reply,message:reply,spokenText:reply,publicAgent:"Nyx",surfaceAgent:"nyx",audience:"public",publicSurfaceOnly:true,publicSurfaceIdentityLock:true,publicLoopFallbackSurfacePurge:true,publicIdentityQuestionRefinement:isPublicIdentityQuestionPrompt(prompt)},
    ui:{renderReady:true,connectionState:"ready",publicSurfaceOnly:true,surfaceAgent:"nyx"},
    meta:{publicSurfaceIdentityLock:true,publicLoopFallbackSurfacePurge:true,publicIdentityQuestionRefinement:isPublicIdentityQuestionPrompt(prompt),publicProjection:"Nyx",privateOperatorContext:false,version:VERSION}
  };
}
module.exports={VERSION,isPublicSurfaceContext,isPrivateOperatorContext,isInternalPublicLeak,isPublicPresencePrompt,isPublicWhoPrompt,isPublicIdentityQuestionPrompt,cleanPublicPresenceReply,cleanPublicWhoReply,cleanPublicIdentityReply,sanitizePublicReply,extractPrompt,extractReply,projectPublicReplyFields,projectPublicPayload};
