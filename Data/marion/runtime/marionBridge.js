...
function isExcessExpressionReply(value=""){const t=safeStr(value);return /(stop the echo|switching from invitation to execution|recovery line has already served its purpose|next line must carry progress|public knowledge topic|useful answer should|six-domain layer|final envelope|state spine|progression shaping|runtimeTelemetry|replyAuthority|diagnostic packet)/i.test(t);}
...
async function handleMarionAdminConversation(input = {}, context = {}) {
  const source = safeObj(input);
  const ctx = safeObj(context);
  return processWithMarion({
    ...ctx,
    ...source,
    inputChannel: cleanText(source.inputChannel || ctx.inputChannel || "text"),
