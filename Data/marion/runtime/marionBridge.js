...
function bridgeBroadPublicTerm(text=""){
  ...
  for(const rx of patterns){const m=raw.match(rx);if(m&&m[1])return safeStr(m[1]).replace(/\b(in|for|within)\s+(psychology|english|ai|artificial intelligence|finance|cyber|law)\s*$/i,"").trim();}
  return "";
}
function capTerm(s){s=safeStr(s);return s?s.charAt(0).toUpperCase()+s.slice(1):"";}
function bridgePublicDomainAnswer(text=""){
  const term=bridgeBroadPublicTerm(text),k=lower(term),label=capTerm(term);
  if(!k)return"";
  if(/\baudit(?:ing)?\b/.test(k))return"Auditing is a structured review process that checks records, systems, finances, operations, or decisions against a standard. It helps reveal gaps, confirm evidence, reduce risk, and show where controls or processes need to be corrected.";
  if(/\bcognitive bias\b/.test(k))return"Cognitive bias is a repeated pattern in how people think or judge that can distort interpretation, memory, risk assessment, or decision-making. In psychology, it matters because people often feel certain even when their reasoning has been nudged by emotion, habit, fear, or incomplete information.";
  if(/\bcognitive\b/.test(k))return"Cognitive refers to the mental processes involved in understanding, learning, remembering, reasoning, and making decisions. In psychology, it explains how people interpret information; in AI, it describes systems that imitate parts of that reasoning process.";
  if(/\bnegligence\b/.test(k))return"Negligence is a legal concept where someone fails to take reasonable care and that failure causes harm or loss. At a general-information level, it usually involves a duty of care, a breach of that duty, causation, and resulting damage.";
  if(/\bphishing\b/.test(k))return"Phishing is a cyberattack where someone tries to trick a person into giving up information, credentials, money, or access. It usually works through fake emails, messages, websites, or urgent instructions that look legitimate.";
  if(/\bsyntax\b/.test(k))return"Syntax is the structure that controls how words, phrases, or symbols are arranged so meaning is clear. In English it shapes sentence flow; in code it determines whether instructions are written in a form the system can understand.";
  if(/\bmachine learning\b/.test(k))return"Machine learning is a branch of AI where systems learn patterns from data and use those patterns to make predictions, classifications, or decisions. The value is that the system improves from examples instead of relying only on fixed rules.";
  if(/\bemotional regulation\b/.test(k))return"Emotional regulation is the ability to notice, manage, and respond to emotions without letting them completely control behavior. In psychology, it supports clearer thinking, better decisions, and steadier relationships under pressure.";
  if(/\brevenue\b/.test(k))return"Revenue is the money a business earns from selling products, services, subscriptions, ads, sponsorships, or other income streams before expenses are removed. It shows how money enters the business, but not whether the business is profitable.";
  if(/\bleast privilege\b/.test(k))return"Least privilege is a cybersecurity principle where a person, service, or system gets only the minimum access needed to do its job. It reduces damage if an account, app, or credential is misused or compromised.";
  if(/\bcash flow\b/.test(k))return"Cash flow is the movement of money into and out of a business. Positive cash flow means enough money is coming in to cover expenses, while weak cash flow can create pressure even when sales look strong.";
  if(/\bgrammar\b/.test(k))return"Grammar is the rule system that shapes how words are arranged so meaning is clear. It covers sentence structure, agreement, tense, punctuation, and the flow that makes communication understandable.";
  if(/\bconsideration\b/.test(k)&&/\bcontract|law\b/.test(lower(text)))return"Consideration in contract law is the value exchanged between parties, such as money, a service, a promise, or a benefit. At a general-information level, it helps show that an agreement is more than a casual promise.";
  if(/\b(llm|model|embedding|agent|artificial intelligence|ai|neural|prompt|inference|tool routing|rag)\b/i.test(k))return`${label} is an AI concept. At a practical level, it helps explain how systems interpret information, use patterns, make predictions, or coordinate tools to produce an answer.`;
  if(/\b(threat|malware|ransomware|mfa|iam|zero trust|access|credential|security|privacy|encryption|vulnerability)\b/i.test(k))return`${label} is a cybersecurity concept. The useful way to understand it is by asking what risk it creates, what access or data it touches, and what control reduces the exposure.`;
  if(/\b(contract|liability|compliance|jurisdiction|statute|tort|damages|privacy|policy|rights|duty)\b/i.test(k))return`${label} is a legal concept. At a general-information level, the key is to identify the rule, the parties affected, the duty or obligation involved, and the consequence if that duty is not met.`;
  if(/\b(cash|profit|margin|pricing|cost|budget|capital|asset|debt|equity|runway|valuation|investment)\b/i.test(k))return`${label} is a finance concept. The practical angle is how it affects money movement, risk, pricing, resources, or the strength of a decision.`;
  if(/\b(grammar|word|sentence|tone|writing|rewrite|language|meaning|phrase|clarity)\b/i.test(k))return`${label} is an English-language concept. The core issue is how wording, structure, tone, or meaning changes the way a message is understood.`;
  if(/\b(emotion|behavior|cognition|bias|memory|learning|attention|trauma|attachment|anxiety|decision)\b/i.test(k))return`${label} is a psychology concept. It is useful because it explains how people think, feel, react, learn, or make decisions in real situations.`;
  return `${label} is a concept Marion can break down by defining what it means, why it matters, and how it applies in context. The useful answer is not just a label; it should connect the idea to decisions, risks, communication, or action.`;
}
function isDomainClarifierReply(value=""){
  const t=lower(value);
  return /are you asking about the interface|which file or runtime layer|radio\/media|backend technical work|what do you want me to focus on|are you aiming this at interface buyers|cash flow, pricing, revenue, cost, or investment risk|access control, threat risk, implementation, or policy|jurisdiction and legal issue/.test(t);
}
...
function createLocalFinalEnvelope({normalized={},routed={},contract={},reason="local_final_fallback",loopGuardResult={}}={}){
  ...
  let reply=firstText(extractReply(contract));const domainAnswer=bridgePublicDomainAnswer(firstText(normalized.userQuery,normalized.text,normalized.rawUserQuery));if(domainAnswer&&(!reply||isDomainClarifierReply(reply)))reply=domainAnswer;
...
function isDefinitionQuery(text=""){const t=lower(text);return!!t&&(/\b(what\s+is|what\s+are|tell me about|define|definition\s+of|meaning\s+of|explain|explain\s+the\s+term|explain\s+the\s+word|describe)\b/i.test(t)||/\?$/.test(t));}
...
function crossDomainSecondaryLaneProfile(text=""){
  ...
  if(/\b(full autopsy|line[-\s]?by[-\s]?line|critical fix|critical fixes|patch|debug|backend|frontend|widget|script|file|api\/chat|render|deploy|syntax|node --check)\b/i.test(t))return null;
