...
const VERSION = "marionBridge v4.1.0 SINGLE-INTERPRETER";
...
function _normalizeSupportFlags(flags = {}) {
  ...
  return {
    crisis: _bool(src.crisis),
    needsContainment: _bool(src.needsContainment),
    needsStabilization: _bool(src.needsStabilization),
    needsClarification: _bool(src.needsClarification),
    needsConnection: _bool(src.needsConnection),
    ...
    guardedness: _bool(src.guardedness),
    suppressed: _bool(src.suppressed),
    forcedPositivity: _bool(src.forcedPositivity),
    minimization: _bool(src.minimization)
  };
}
...
function _normalizeEmotion(raw = {}, text = "", behavior = {}) {
  ...
  if (["sadness", "fear"].includes(primaryEmotion)) {
  ...
  return {
    primaryEmotion,
    secondaryEmotion: _lower(primary.secondaryEmotion || src.secondaryEmotion || ""),
    intensity: Number(intensity.toFixed(3)),
    valence: primaryEmotion === "joy" ? 0.7 : (primaryEmotion === "neutral" ? 0 : -0.7),
    confidence: _clamp01(primary.confidence != null ? primary.confidence : src.confidence, 0.72),
    supportFlags,
...
async function _callRetriever(mod, preferredName, payload) {
  ...
  if (!fn) return null;
  try { return await Promise.resolve(fn(payload)); } catch (_e) { return null; }
}
function _extractDatasets(inputDatasets = [], sections = {}) {
  const out = [];
  for (const item of _safeArray(inputDatasets)) out.push(item);
  for (const value of Object.values(_safeObj(sections))) if (Array.isArray(value)) out.push(...value);
  return out;
}
function _resolveCanonicalRoute(text, requestedDomain, previousMemory = {}) {
  if (typeof routeMarion === "function") {
    try {
      return _safeObj(routeMarion({ text, query: text, requestedDomain, domain: requestedDomain, previousMemory }));
    } catch (_e) {}
  }
  return {
    ok: true,
    primaryDomain: _canonicalDomain(requestedDomain),
    secondaryDomains: [],
    supportFlags: {},
    domains: { emotion: {}, psychology: { matched: false, matches: [] } },
    primaryEmotion: { emotion: "neutral", intensity: 0 },
    blendProfile: { weights: { neutral: 1 }, dominantAxis: "neutral" },
    stateDrift: { previousEmotion: "", currentEmotion: "neutral", trend: "stable", stability: 1 },
    classified: { classifications: {}, supportFlags: {}, domainCandidates: [_canonicalDomain(requestedDomain)] },
    diagnostics: { domainCandidates: [_canonicalDomain(requestedDomain)], usedPsychology: false }
  };
}
function _synthesizeReply(contract = {}, layer2 = {}) {
  ...
  if (_safeObj(contract.supportFlags).crisis) {
    return [
      line1,
      "Your safety comes first. Reach out to immediate human support right now or local emergency help if you might act on this."
    ].join("\n");
  }

  if (layer2.domain !== "psychology") {
    const top = _safeArray(layer2.evidence)[0];
    if (top && _trim(top.summary)) return top.summary;
    return line1;
  }

  if (directive.pacing === "slow" && directive.followupStyle === "soft_probe") {
    return [line1, "You do not have to force it. Give me the piece that feels easiest to say first."].join("\n");
  }
  if (directive.followupStyle === "ground_then_narrow") {
    return [line1, "Let’s keep it to one piece at a time. Tell me the part that feels most immediate."].join("\n");
  }
  if (directive.followupStyle === "action_gate") {
    return [line1, "Stay with the immediate next safe step only."].join("\n");
  }
  if (directive.followupStyle === "direct_answer_then_one_question") {
    return [line1, "What is the next move you want to stabilize first?"].join("\n");
  }
  return line1;
}
function _buildPacket(result, evidence) {
  return {
    synthesis: {
      ...
      blendProfile: result.contract.blendProfile,
      stateDrift: result.contract.stateDrift,
...
async function retrieveLayer2Signals(input = {}) {
  ...
  const supportFlags = _normalizeSupportFlags({
    ...
    ..._safeObj(routing.supportFlags),
    ..._safeObj(_safeObj(routing.classified).supportFlags)
  });
  ...
  const psychology = _safeObj(_safeObj(routing.domains).psychology).matched
    ? _safeObj(_safeObj(routing.domains).psychology)
    : _safeObj(await _callRetriever(PsychologyRetriever, "retrievePsychology", {
        text, query: text, userQuery: text, supportFlags,
  ...
  const directDatasetEvidence = _safeArray(await _callRetriever(DatasetRetriever, "retrieveDataset", {
    text, query: text, userQuery: text, domain, intent, datasets,
    emotion: { primaryEmotion: emotion.primaryEmotion, intensity: emotion.intensity },
    psychology: { supportMode: _trim(_safeObj(_safeObj(psychology.primary).record).supportMode || _safeObj(psychology.route).supportMode || "") },
    maxMatches: 6
  }));

  const evidence = _normalizeEvidence(
    []
      .concat(normalized.memoryEvidence)
      .concat(normalized.generalEvidence)
      .concat(normalized.domainEvidence)
      .concat(normalized.datasetEvidence)
      .concat(directDomainEvidence)
      .concat(directDatasetEvidence),
    domain,
    "bridge"
  ...
  return {
    ...
    diagnostics: {
      ...
      layer2EvidenceCounts: {
        total: evidenceValidation.items.length,
...
async function processWithMarion(input = {}) {
  ...
  const contract = typeof composeMarionResponse === "function"
    ? composeMarionResponse({
        primaryDomain: layer2.domain,
        domains: layer2.routing.domains || { emotion: {}, psychology: {} },
        classified: layer2.routing.classified || {},
        supportFlags: layer2.supportFlags,
        blendProfile: layer2.routing.blendProfile,
        stateDrift: layer2.routing.stateDrift,
        previousTurn: layer2.routing.previousTurn,
        routeBias: _trim(_safeObj(_safeObj(layer2.psychology).route).routeBias || "")
      }, input)
    : {
        interpretation: FALLBACK_REPLY,
        supportMode: "clarify_and_sequence",
        routeBias: "clarify",
        riskLevel: layer2.supportFlags.crisis ? "critical" : (layer2.supportFlags.highDistress ? "high" : "low"),
        responsePlan: { pacing: "steady", followupStyle: "reflective" },
        nyxDirective: { followupStyle: "reflective", pacing: "steady", responseLength: "medium" },
        supportFlags: layer2.supportFlags,
        blendProfile: layer2.routing.blendProfile,
        stateDrift: layer2.routing.stateDrift
      };
  ...
  const continuityState = {
    ...
    responseMode: contract.supportMode || "clarify_and_sequence",
    continuityHealth: layer2.behavior.repeatQuery ? "stressed" : "stable",
    recoveryMode: !!layer2.supportFlags.recoveryPresent,
    timestamp: Date.now()
  };
  const turnMemory = {
  ...
  const result = {
    ...
    meta: {
      version: VERSION,
      endpoint: layer2.endpoint,
      evidenceCount: layer2.evidence.length,
      mode: contract.supportMode || "clarify_and_sequence",
