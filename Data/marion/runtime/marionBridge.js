...
function bridgeLoopGovernorReply(normalized={},routed={},reason=""){
  ...
  if(/\b(loop|looping|duplicate|repeated|fallback)\b/i.test(source)&&/\b(nyx|nix|nicks|marion|bridge|composer|index|chat)\b/i.test(source)){
    return "The loop is being contained at the bridge layer. MarionBridge should accept only one clean Marion final, reject repeated generic support text, and return a bounded recovery from the current user prompt instead of letting Nyx render another recycled fallback.";
  }
  return "I stopped the repeated fallback and preserved the current turn. Marion stays as final authority, while the supporting lanes remain advisory instead of taking over the conversation.";
}
...
function bridgeRecoveryReply(normalized={},routed={},reason="bridge_recovery"){
  ...
  return bridgeLoopGovernorReply(normalized,routed,reason);
}
