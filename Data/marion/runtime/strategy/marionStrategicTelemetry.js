"use strict";
const VERSION="nyx.marion.layer27.strategicTelemetry/1.1-baseline-freeze";
...
function collect(input) { const src=safeObject(input); const plan=safeObject(safeRead(src,"plan",src)); const events=[]; if(safeRead(plan,"correctionOverride",false)===true)events.push("current_turn_correction_preserved"); if(safeArray(safeRead(plan,"steps",[])).length)events.push("execution_plan_created"); if(safeArray(safeRead(plan,"opportunities",[])).length)events.push("opportunities_detected"); return bounded({version:VERSION,layer:27,events,metrics:{stepCount:safeArray(safeRead(plan,"steps",[])).length,priorityCount:safeArray(safeRead(plan,"priorities",safeRead(plan,"ranked",[]))).length},advisoryOnly:true}); }
