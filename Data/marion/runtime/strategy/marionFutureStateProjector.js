"use strict";
const VERSION="nyx.marion.layer27.futureStateProjector/1.1-baseline-freeze";
...
function project(input) {
  ...
  const constraints=[...new Set(safeArray(safeRead(src,"constraints",[])).map(v=>safeText(v,"",300)).filter(Boolean))].slice(0,12);
  const states=[
    {horizon:"immediate",condition:"current-turn objective is preserved",desiredState:goal||"stable response"},
    {horizon:"short",condition:"dependencies and tests pass",desiredState:"additive Layer 27 integration"},
    {horizon:"medium",condition:"Layer 28 remains internal and bounded",desiredState:"reflective quality control without reply takeover"},
    {horizon:"long",condition:"production telemetry remains stable",desiredState:"cohesive Layers 1 through 28"}
  ];
  return bounded({version:VERSION,layer:27,goal,states,constraints,assumptions:[],advisoryOnly:true});
}
