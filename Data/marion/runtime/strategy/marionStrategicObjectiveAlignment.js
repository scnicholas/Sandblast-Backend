...
/* MARION_NUANCE_PHASE_B_StrategicAlignmentCohesion_V1_START */
(function marionNuancePhaseBStrategicAlignmentCohesionV1(){
  "use strict";
  const api=module.exports&&typeof module.exports==="object"?module.exports:null;
  if(!api||api.__marionNuancePhaseBStrategicAlignmentCohesionV1)return;
  ...
  const original=api.analyze;if(typeof original==="function")api.analyze=function(input={}){const b=phaseB(input),s=summary(b),explicit=/\b(?:our|the|my)\s+(?:new\s+)?(?:governing\s+|program\s+|project\s+|current\s+)?(?:objective|goal|priority)\s+(?:is|will be|becomes)\b|\b(?:change|shift|realign|replace|update|reset)\s+(?:the\s+)?(?:objective|goal|priority)\s+(?:to|toward|towards)\b/i.test(clean(input.prompt,6000)),out=original.call(this,input);let result={...obj(out),phaseBPrimaryPragmaticIntent:s.primaryPragmaticIntent,subtextMayChangeGoverningObjective:false,stanceMayChangeGoverningObjective:false,literalIntentPreserved:true};if(!explicit&&result.objectiveChanged===true)result={...result,objectiveChanged:false,phaseBObjectiveChangeBlocked:true};return result;};

  api.MARION_NUANCE_PHASE_B_CONTRACT=PHASE_B_CONTRACT;
  api.MARION_LAYER_HARD_STOP=HARD_STOP_LAYER;
  api.__marionNuancePhaseBStrategicAlignmentCohesionV1=true;
})();
/* MARION_NUANCE_PHASE_B_StrategicAlignmentCohesion_V1_END */
