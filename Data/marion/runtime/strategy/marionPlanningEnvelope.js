"use strict";
const VERSION="nyx.marion.layer27.planningEnvelope/1.0";
...
function build(input){ const src=safeObject(input); const base=safeObject(safeRead(src,"baseEnvelope",{})); const reply=safeString(safeRead(base,"reply",safeRead(base,"displayReply",""))); const displayReply=safeString(safeRead(base,"displayReply",reply)); const layer27={version:VERSION,plan:safeObject(src.plan),priorities:safeObject(src.priorities),internalOnly:true,executionAuthorized:false}; return bounded({...base,reply,displayReply,final:safeRead(base,"final",true)!==false,handled:safeRead(base,"handled",true)!==false,noUserFacingDiagnostics:true,executionAuthorized:false,layer27,planning:layer27}); }
