...
function buildSafetyEnvelope({
  ...
  const driftTrend = emotion.stateDrift && emotion.stateDrift.trend ? String(emotion.stateDrift.trend) : "steady";
  ...
  if (suppressionSignals.length) {
    checks.push("Treat guarded language as soft signal, not confession.");
    checks.push("Avoid overprobing when suppression markers are active.");
    warnings.push(`Suppression markers: ${suppressionSignals.join(", ")}`);
  }

  if (driftTrend && driftTrend !== "steady") {
    checks.push(`Respect emotional movement trend: ${driftTrend}.`);
    warnings.push(`State drift trend: ${driftTrend}.`);
  }
  ...
  if (
    ...
    responseMode.mode === "soft-probe"
  ) {
    safeToElaborate = intensity < 0.88 && evidenceKept >= 1 && !suppressionSignals.includes("shutdown");
  }
