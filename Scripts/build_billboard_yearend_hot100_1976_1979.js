/* Scripts/build_billboard_yearend_hot100_1976_1979.js
   Builds:
...
function pickBestTable(allTables) {
  ...
  // Fallback: pick the largest table and infer indices
  let best = null;
  for (const tbl of allTables) {
    const rows = extractRowsFromTable(tbl);
...
async function buildYear(year) {
  ...
  if (out.length < 80) {
    return { year, ok: false, error: `TOO_FEW_ROWS(${out.length})`, url, rows: out.length };
  }
