...
function createDefaultStore() {
  return {
    slots: {
      editors_pick: createEmptySlot("editors_pick"),
      top_story: createEmptySlot("top_story"),
      news_canada_1: createEmptySlot("news_canada_1"),
      news_canada_2: createEmptySlot("news_canada_2")
    }
  };
}
  ...
  try {
    ...
    for (const slotId of Object.keys(SLOT_CONFIG)) {
      if (!parsed.slots[slotId] || typeof parsed.slots[slotId] !== "object") {
        ...
        parsed.slots[slotId].id = slotId;
        parsed.slots[slotId].chipLabel = SLOT_CONFIG[slotId];
      }
    }

    return parsed;
  } catch (err) {
    console.log("[manualNewsCanadaRoutes][readStore:error]", err.message);
    const fallback = createDefaultStore();
    fs.writeFileSync(DATA_FILE, JSON.stringify(fallback, null, 2), "utf8");
    return fallback;
  }
...
router.post("/manual/save", (req, res) => {
  try {
    ...
    console.log("[manualNewsCanadaRoutes][save:error]", err.message);
    return res.status(500).json({
      ok: false,
...
router.post("/manual/clear", (req, res) => {
  try {
    ...
    console.log("[manualNewsCanadaRoutes][clear:error]", err.message);
    return res.status(500).json({
      ok: false,
      error: "manual_clear_failed"
    });
  }
});
