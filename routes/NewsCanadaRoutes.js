...
router.get("/manual/diagnostics", (req, res) => {
  try {
    const store = readStore();
    return res.json({
      ok: true,
      dataFile: DATA_FILE,
      slotIds: Object.keys((store && store.slots) || {}),
      activeSlotCount: Object.values((store && store.slots) || {}).filter((slot) => slot && slot.isActive).length,
      slots: store.slots
    });
  } catch (err) {
    return res.status(500).json({ ok: false, error: clean(err && err.message) || "manual_diagnostics_failed" });
  }
});
