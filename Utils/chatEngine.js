...
;(function(){
  "use strict";
  const PATCH_VERSION="chatEngine ecosystem continuity + TV adapter v1.0-r1";
  ...
  const WRAP_NAMES=["run", "handleChat", "chat", "handle", "reply", "processWithMarion", "route", "maybeResolve", "ask", "handleMessage", "default"];
...
(function(){
  "use strict";
  const V="nyx.guideOrchestration.chatEngine/4.0-steps10-11-12",EC="nyx.guideExecution/1.0",SC="nyx.guideStateTransition/1.0";
  ...
  function find(v,args){let plan={},exec={},ctx={};const seen=new Set();function w(q,d){if(!q||typeof q!=="object"||d>6||seen.has(q))return;seen.add(q);q=o(q);if(!Object.keys(plan).length&&Array.isArray(o(q.guideActionPlan).actions))plan=o(q.guideActionPlan);if(!Object.keys(exec).length&&o(q.nyxGuideExecution).contract)exec=o(q.nyxGuideExecution);if(!Object.keys(ctx).length)ctx=o(q.guideContext||q.publicGuideContinuity);for(const k of["payload","body","meta","result","finalEnvelope","routing","runtimeState","state","session","composerContext","marionIntent"])w(q[k],d+1)}for(const q of Array.from(args||[]).concat([v]))w(q,0);return{plan,exec,ctx}}
  function project(v,args){if(!v||typeof v!=="object"||Array.isArray(v))return v;const f=find(v,args);if(!Array.isArray(f.plan.actions)||!f.plan.actions.length)return v;const current=x(f.ctx.currentLane||f.exec.currentLane||"home",32),execution={...f.exec,contract:EC,version:V,planId:x(f.plan.planId||f.exec.planId,80),status:x(f.exec.status||"pending_user_gesture",32),currentLane:current,previousLane:x(f.ctx.previousLane||f.exec.previousLane||current,32),revision:Math.max(0,Number(f.exec.revision||f.ctx.revision||0)||0),requiresUserGesture:true,autoExecute:false,idempotencyRequired:true,rollbackRequired:true,serverExecutionAllowed:false,publicSessionOnly:true,privateMemoryAccess:false},transition={contract:SC,version:V,status:"awaiting_user_gesture",currentLane:execution.currentLane,previousLane:execution.previousLane,revision:execution.revision,planId:execution.planId,rollbackRequired:true};const out={...v,guideActionPlan:f.plan,guideActions:f.plan.actions,nyxGuideExecution:execution,nyxGuideStateTransition:transition};if(o(out.payload)){out.payload={...out.payload,guideActionPlan:f.plan,nyxGuideExecution:execution,nyxGuideStateTransition:transition}}if(o(out.finalEnvelope)){out.finalEnvelope={...out.finalEnvelope,guideActionPlan:f.plan,nyxGuideExecution:execution,nyxGuideStateTransition:transition}}out.meta={...o(out.meta),nyxGuideTransport:{version:V,coordinatorOnly:true,finalReplyAuthority:false,actionExecutionAuthority:"client_user_gesture",diagnosticsRedacted:true}};return out}
  function wrap(fn){if(typeof fn!=="function"||fn.__nyx101112Chat)return fn;const w=function(){const a=arguments,r=fn.apply(this,a);return r&&typeof r.then==="function"?r.then(v=>project(v,a)):project(r,a)};try{Object.keys(fn).forEach(k=>w[k]=fn[k])}catch(_){}w.__nyx101112Chat=true;return w}
  try{if(typeof module.exports==="function")module.exports=wrap(module.exports);const api=module.exports&&typeof module.exports==="object"?module.exports:null;if(api){for(const n of["processWithMarion","run","route","handle","ask","handleMessage","finalize","safeResponse","buildResponse","createResponse","default"])if(typeof api[n]==="function")api[n]=wrap(api[n]);api.NYX_GUIDE_STEPS_10_11_12_CHAT_VERSION=V;api.attachNyxGuideExecutionBoundary=(v,i)=>project(v,[i||{}])}}catch(_){}
})();
/* NYX_GUIDE_ORCHESTRATION_STEPS_10_11_12_R1_END */

/* NYX_PUBLIC_LOOP_LATENCY_FIX_R1_START */
(function nyxPublicLoopLatencyFixR1(){
  "use strict";
  const V="nyx.chatEngine.publicLoopLatencyFix/1.0";
  const recent=new Map();
  const MAX=500,TTL=15000;
  function obj(v){return v&&typeof v==="object"&&!Array.isArray(v)?v:{};}
  ...
  function source(args){for(const v of Array.from(args||[])){if(v&&typeof v==="object")return v;}return{};}
  function deepFlag(s,k){const x=obj(s),list=[x,obj(x.body),obj(x.payload),obj(x.meta),obj(x.client),obj(x.options)];return list.some(q=>q[k]===true);}
  function isPublic(s){const x=obj(s),aud=low(x.audience||obj(x.body).audience||obj(x.payload).audience),lane=low(x.lane||obj(x.body).lane||obj(x.payload).lane),profile=low(x.presentationProfile||obj(x.body).presentationProfile||obj(x.payload).presentationProfile);return aud==="public"||lane==="public_interface"||profile==="public"||deepFlag(x,"publicSurfaceOnly")||deepFlag(x,"publicIdentityLock");}
  function prompt(s){const x=obj(s),p=obj(x.payload),b=obj(x.body),m=obj(x.meta);return txt(x.text||x.userText||x.message||x.query||x.userQuery||x.prompt||p.text||p.message||p.query||b.text||b.message||b.query||m.text||m.message);}
  function session(s){const x=obj(s),p=obj(x.payload),b=obj(x.body);return txt(x.sessionId||p.sessionId||b.sessionId||"anon",120);}
  function reply(v){const x=obj(v),p=obj(x.payload),f=obj(x.finalEnvelope);return txt(x.publicReply||x.visibleReply||x.finalReply||x.reply||x.text||x.answer||x.output||x.response||x.message||f.publicReply||f.visibleReply||f.finalReply||f.reply||f.text||p.publicReply||p.visibleReply||p.finalReply||p.reply||p.text||p.message);}
  function blocked(v){const t=low(v);return !t||/\b(i(?:’|')?m here|i am here|still here|fully online|what(?:’|')?s next|what are we working on|give me the (?:exact |specific )?target|send the next target|run that one more time|final envelope missing|marion did not return|diagnostic packet|recovery path engaged|loop detected|same prompt,? new requirement)\b/i.test(t);}
  function generic(v){const t=norm(v);return t.length<190&&/\b(ready|here|continue|next|target|try again|send|working|online|connected|help)\b/.test(t);}
  function echo(r,p){const a=norm(r),b=norm(p);return !!(a&&b&&(a===b||(a.length>18&&b.includes(a))||(b.length>18&&a.includes(b))));}
  function fast(s){if(!isPublic(s))return null;const p=prompt(s),t=norm(p);if(!t||t.length>360)return null;
    if(/^(?:who|what)\s+is\s+marion$|^(?:what\s+does\s+marion\s+do|explain\s+marion|tell\s+me\s+about\s+marion)$/.test(t))return{intent:"public_marion_identity",reply:"Marion is Sandblast’s private cognitive coordination layer. She supports deeper reasoning, context continuity, routing, and response shaping behind the scenes, while I remain Nyx, the public-facing Sandblast assistant. Private operator functions and owner-only information are not exposed through this interface."};
    if(/\b(?:are you|is this) marion\b/.test(t))return{intent:"identity_boundary",reply:"No. I’m Nyx, the public Sandblast guide. Marion is reserved for the private operator environment and is not exposed through this public interface."};
    if(/^(?:who are you|what are you|tell me who you are|who is nyx|who is nix)$/.test(t))return{intent:"identity",reply:"I’m Nyx, the public Sandblast assistant. I help you explore Sandblast Radio, TV, Roku, Synapse, LingoSentinel, media, AI, and business tools."};
    if(/^(?:what is sandblast|tell me about sandblast|explain sandblast|what is the sandblast ecosystem)$/.test(t))return{intent:"ecosystem_identity",reply:"Sandblast is a connected media and AI ecosystem that brings together live radio, television, Roku viewing, Synapse news, LingoSentinel language tools, entertainment, and business experiences through one guided interface."};
    if(/\b(?:what can you do|what can you help with|how can you help|show me your capabilities)\b/.test(t))return{intent:"capabilities",reply:"I can guide you through the Sandblast ecosystem, answer questions, and help you find radio, TV, Roku, Synapse, LingoSentinel, cartoons, classics, AI, and business tools."};
    return null;
  }
  function fastPacket(s,d){const r=d.reply;return{ok:true,handled:true,final:true,finalized:true,marionFinal:false,awaitingMarion:false,suppressUserFacingReply:false,emit:true,blocked:false,reply:r,text:r,answer:r,output:r,response:r,message:r,displayReply:r,publicReply:r,visibleReply:r,finalReply:r,spokenText:r,payload:{reply:r,text:r,message:r,spokenText:r,final:true,publicFastPath:true},finalEnvelope:{contractVersion:"nyx.public.final/1.0",authority:"nyx_public_fast_path",reply:r,text:r,displayReply:r,spokenText:r,final:true,handled:true,publicFastPath:true},meta:{replyAuthority:"nyx_public_fast_path",semanticAuthority:"nyx",intent:d.intent,publicFastPath:true,loopLatencyFixVersion:V,noUserFacingDiagnostics:true}};}
  function suppress(v,reason){const x={...obj(v)},p={...obj(x.payload)},f={...obj(x.finalEnvelope)},m={...obj(x.meta)};for(const k of["reply","text","answer","output","response","message","displayReply","publicReply","visibleReply","finalReply","spokenText"])x[k]="";for(const k of["reply","text","answer","output","response","message","displayReply","publicReply","visibleReply","finalReply","spokenText"])p[k]="";for(const k of["reply","text","answer","output","response","message","displayReply","publicReply","visibleReply","finalReply","spokenText"])f[k]="";x.payload={...p,final:false,marionFinal:false,awaitingMarion:true,suppressUserFacingReply:true};x.finalEnvelope={...f,final:false,marionFinal:false,handled:false,canEmit:false};x.ok=false;x.final=false;x.finalized=false;x.marionFinal=false;x.awaitingMarion=true;x.suppressUserFacingReply=true;x.emit=false;x.blocked=true;x.meta={...m,loopLatencyFixVersion:V,loopSuppressed:true,loopSuppressionReason:reason,noUserFacingDiagnostics:true};return x;}
  function guard(v,args){if(!v||typeof v!=="object"||Array.isArray(v))return v;const s=source(args),p=prompt(s),r=reply(v);if(!isPublic(s))return v;const key=session(s),now=Date.now(),prev=recent.get(key);if(blocked(r)||echo(r,p))return suppress(v,echo(r,p)?"prompt_echo":"blocked_loop_phrase");if(prev&&now-prev.at<TTL&&prev.replyHash===hash(r)&&prev.promptHash!==hash(p)&&(generic(r)||blocked(r)))return suppress(v,"stale_reply_across_different_input");if(r){recent.delete(key);recent.set(key,{at:now,replyHash:hash(r),promptHash:hash(p)});while(recent.size>MAX)recent.delete(recent.keys().next().value);}const x={...obj(v),meta:{...obj(v.meta),loopLatencyFixVersion:V,loopGuardChecked:true}};return x;}
  function wrap(fn,name,allowFast){if(typeof fn!=="function"||fn.__nyxPublicLoopLatencyFixR1)return fn;const asyncLike=fn.constructor&&fn.constructor.name==="AsyncFunction";const w=function(){const a=arguments,s=source(a),d=allowFast?fast(s):null;if(d){const out=fastPacket(s,d);return asyncLike?Promise.resolve(out):out;}const r=fn.apply(this,a);return r&&typeof r.then==="function"?r.then(v=>guard(v,a)):guard(r,a);};try{Object.keys(fn).forEach(k=>w[k]=fn[k]);}catch(_){}w.__nyxPublicLoopLatencyFixR1=true;w.__nyxWrappedName=name;return w;}
  try{const api=module.exports&&typeof module.exports==="object"?module.exports:null;if(api){for(const n of["handleChat","run","chat","handle","reply","default"])if(typeof api[n]==="function")api[n]=wrap(api[n],n,true);for(const n of["normalizeCoordinatorOutputForPipeline","normalizeVisibleFinalReplyFields"])if(typeof api[n]==="function")api[n]=wrap(api[n],n,false);api.NYX_PUBLIC_LOOP_LATENCY_FIX_VERSION=V;api.buildNyxPublicFastReply=(input)=>{const d=fast(input);return d?fastPacket(input,d):null;};api.guardNyxPublicCoordinatorOutput=(value,input)=>guard(value,[input||{}]);}}
  catch(_){}
})();
...
/* MARION_STRATEGIC_FLOW_LAYERS_15_16_17_CHATENGINE_TRANSPORT_V17_START */
...
(function marionRound2UnifiedTrustedFinalV1(){
  ...
  const cache=new WeakMap();for(const name of names){const fn=api[name];if(typeof fn!=="function")continue;let w=cache.get(fn);if(!w){w=function(){const r=resolve(promptOf(arguments[0]));if(r)return Promise.resolve(packet(arguments[0],r));return fn.apply(this,arguments)};cache.set(fn,w)}api[name]=w;}
...
/* NYX_PUBLIC_IDENTITY_SECURITY_MATRIX_FINAL_AUTHORITY_R4_START */
(function nyxPublicIdentitySecurityMatrixFinalAuthorityR4(){
  "use strict";
  const api=module.exports&&typeof module.exports==="object"?module.exports:null;
  if(!api||api.__nyxPublicIdentitySecurityMatrixFinalAuthorityR4)return;
  const VERSION="nyx.chatEngine.publicIdentitySecurityMatrixFinalAuthority/4.0";
  ...
  function build(input,kind){
    const reply=REPLIES[kind],subIntent="public_"+kind,reason=subIntent+"_terms",x=obj(input),turnId=text(x.turnId||x.traceId||obj(x.payload).turnId);
    const finalEnvelope={contractVersion:"nyx.public.final/1.0",authority:"nyx_public_identity_security_matrix_final_authority",intent:"identity_query",subIntent,reason,reply,text:reply,displayReply:reply,visibleReply:reply,finalReply:reply,spokenText:reply,final:true,handled:true,publicFastPath:true,securityBoundary:kind!=="marion_identity",noUserFacingDiagnostics:true};
    return{ok:true,handled:true,final:true,finalized:true,marionFinal:false,awaitingMarion:false,suppressUserFacingReply:false,emit:true,blocked:false,intent:"identity_query",subIntent,reason,reply,text:reply,answer:reply,output:reply,response:reply,message:reply,displayReply:reply,publicReply:reply,visibleReply:reply,finalReply:reply,spokenText:reply,turnId,payload:{reply,text:reply,message:reply,spokenText:reply,final:true,publicFastPath:true,securityBoundary:kind!=="marion_identity",intent:"identity_query",subIntent},finalEnvelope,meta:{replyAuthority:"nyx_public_identity_security_matrix_final_authority",semanticAuthority:"nyx",intent:"identity_query",subIntent,reason,publicFastPath:true,securityBoundary:kind!=="marion_identity",version:VERSION,noUserFacingDiagnostics:true}};
  }
  const names=["handleChat","run","chat","handle","reply","default","processWithMarion","route","ask"];
  const cache=new WeakMap();
  for(const name of names){const fn=api[name];if(typeof fn!=="function")continue;let wrapped=cache.get(fn);if(!wrapped){wrapped=function(){const input=arguments[0],kind=classify(input);if(kind){const result=build(input,kind);return fn.constructor&&fn.constructor.name==="AsyncFunction"?Promise.resolve(result):result;}return fn.apply(this,arguments);};try{Object.keys(fn).forEach(k=>wrapped[k]=fn[k]);}catch(_){}cache.set(fn,wrapped);}api[name]=wrapped;}
  api.classifyNyxPublicIdentitySecurityMatrix=classify;
  api.buildNyxPublicIdentitySecurityReply=function(input){const kind=classify(input);return kind?build(input,kind):null;};
  api.NYX_PUBLIC_IDENTITY_SECURITY_MATRIX_FINAL_AUTHORITY_VERSION=VERSION;
...
/* NYX_LONG_SESSION_CONTINUITY_BRIDGE_R7_START */
(function(){"use strict";const api=module.exports&&typeof module.exports==="object"?module.exports:null;if(!api||api.__nyxLongSessionContinuityBridgeR7)return;const path=require("path"),prog=(()=>{try{return require(path.join(__dirname,"..","Data","marion","runtime","marionConversationProgression.js"))}catch(_){return null}})(),pivot=(()=>{try{return require(path.join(__dirname,"..","Data","marion","runtime","marionContextPivot.js"))}catch(_){return null}})(),VERSION="nyx.chatEngine.longSessionContinuityBridge/7.0";function t(v){try{return String(v==null?"":v).replace(/[\u0000-\u001f\u007f]/g," ").replace(/\s+/g," ").trim()}catch(_){return""}}function src(a){for(const v of Array.from(a||[])){if(typeof v==="string")return{text:v};if(v&&typeof v==="object")return v}return{}}function build(i){if(!prog||typeof prog.buildProgressionPacket!=="function")return null;const p=prog.buildProgressionPacket(i);if(!p||!p.active||!p.resolved)return null;const c=pivot&&pivot.buildContextPivot?pivot.buildContextPivot(i):{},reply=t(p.reply),action=t(p.action);if(!reply)return null;const fe={contractVersion:"nyx.marion.final/1.0",signature:"MARION_FINAL_AUTHORITY",authority:"nyx_long_session_continuity_bridge",source:"chatEngine",intent:"contextual_directive",subIntent:"continuation_"+action,reason:"long_session_continuity_resolved",reply,text:reply,displayReply:reply,visibleReply:reply,finalReply:reply,spokenText:reply,final:true,marionFinal:true,handled:true,continuityHandled:true,continuityTerminal:true,noUserFacingDiagnostics:true};return{ok:true,handled:true,final:true,finalized:true,terminal:true,marionFinal:true,awaitingMarion:false,transportSafe:true,suppressUserFacingReply:false,emit:true,blocked:false,canEmit:true,continuityHandled:true,continuityTerminal:true,intent:"contextual_directive",subIntent:"continuation_"+action,reason:"long_session_continuity_resolved",reply,text:reply,answer:reply,output:reply,response:reply,message:reply,displayReply:reply,visibleReply:reply,finalReply:reply,directReply:reply,authoritativeReply:reply,spokenText:reply,payload:{reply,text:reply,message:reply,answer:reply,output:reply,response:reply,displayReply:reply,visibleReply:reply,finalReply:reply,directReply:reply,authoritativeReply:reply,spokenText:reply,finalEnvelope:fe,final:true,marionFinal:true,handled:true,awaitingMarion:false,suppressUserFacingReply:false,emit:true,blocked:false,canEmit:true,continuityHandled:true,continuityTerminal:true,intent:"contextual_directive",subIntent:"continuation_"+action,progression:p,contextPivot:c},finalEnvelope:fe,meta:{replyAuthority:"nyx_long_session_continuity_bridge",continuityHandled:true,continuityTerminal:true,intent:"contextual_directive",subIntent:"continuation_"+action,version:VERSION,noUserFacingDiagnostics:true},progression:p,contextPivot:c}}function wrap(fn){if(typeof fn!=="function"||fn.__nyxLongSessionContinuityBridgeR7)return fn;const w=function(){const terminal=build(src(arguments));if(terminal)return fn.constructor&&fn.constructor.name==="AsyncFunction"?Promise.resolve(terminal):terminal;return fn.apply(this,arguments)};try{Object.keys(fn).forEach(k=>w[k]=fn[k])}catch(_){}w.__nyxLongSessionContinuityBridgeR7=true;return w}for(const n of["handleChat","run","chat","handle","reply","default","processWithMarion","route","ask"])if(typeof api[n]==="function")api[n]=wrap(api[n]);if(typeof api.ChatEngine==="function"&&api.ChatEngine.prototype)for(const n of["processInput","process","handle","run","chat","reply","ask"])if(typeof api.ChatEngine.prototype[n]==="function")api.ChatEngine.prototype[n]=wrap(api.ChatEngine.prototype[n]);api.buildNyxLongSessionContinuityReply=build;api.NYX_LONG_SESSION_CONTINUITY_BRIDGE_VERSION=VERSION;api.__nyxLongSessionContinuityBridgeR7=true})();
/* NYX_LONG_SESSION_CONTINUITY_BRIDGE_R7_END */

/* NYX_SESSION_CONTINUITY_R8_START */
(function(){"use strict";const api=module.exports&&typeof module.exports==="object"?module.exports:null;if(!api||api.__sessionContinuityR8)return;
const path=require("path"),root=path.join(__dirname,"..","Data","marion","runtime"),state=(()=>{try{return require(path.join(root,"marionContinuityState.js"))}catch(_){return null}})(),prog=(()=>{try{return require(path.join(root,"marionConversationProgression.js"))}catch(_){return null}})(),pivot=(()=>{try{return require(path.join(root,"marionContextPivot.js"))}catch(_){return null}})();
const o=v=>v&&typeof v==="object"&&!Array.isArray(v)?v:{},t=v=>{try{return String(v??"").replace(/[\u0000-\u001f\u007f]/g," ").replace(/\s+/g," ").trim()}catch(_){return""}};
function q(i){if(typeof i==="string")return t(i);const x=o(i),p=o(x.payload),b=o(x.body);return t(x.prompt||x.rawUserText||x.userText||x.query||x.message||x.text||p.prompt||p.userText||p.text||b.prompt||b.userText||b.text)}
function replyOf(r){const x=o(r),p=o(x.payload),f=o(x.finalEnvelope);for(const v of[x.directReply,x.visibleReply,x.displayReply,x.finalReply,x.reply,x.answer,x.response,x.text,x.message,f.reply,f.text,p.reply,p.text]){const z=t(v);if(z)return z}return""}
function build(i){if(!state||!prog)return null;const h=state.hydrate(i),p=prog.buildProgressionPacket(h);if(!p||!p.active||!p.resolved)return null;const c=pivot?pivot.buildContextPivot(h):{},r=t(p.reply),a=t(p.action);if(!r)return null;const f={contractVersion:"nyx.marion.final/1.0",signature:"MARION_FINAL_AUTHORITY",authority:"nyx_session_continuity_terminal",intent:"contextual_directive",subIntent:"continuation_"+a,reply:r,text:r,displayReply:r,visibleReply:r,finalReply:r,spokenText:r,final:true,marionFinal:true,handled:true,continuityHandled:true,continuityTerminal:true};const packet={ok:true,handled:true,final:true,finalized:true,terminal:true,marionFinal:true,awaitingMarion:false,emit:true,blocked:false,continuityHandled:true,continuityTerminal:true,intent:"contextual_directive",subIntent:"continuation_"+a,reply:r,text:r,answer:r,response:r,message:r,displayReply:r,visibleReply:r,finalReply:r,directReply:r,spokenText:r,payload:{reply:r,text:r,message:r,displayReply:r,visibleReply:r,finalReply:r,spokenText:r,finalEnvelope:f,final:true,handled:true,continuityHandled:true,continuityTerminal:true,subIntent:"continuation_"+a,progression:p,contextPivot:c},finalEnvelope:f,meta:{replyAuthority:"nyx_session_continuity_terminal",continuityHandled:true,continuityTerminal:true,noUserFacingDiagnostics:true},progression:p,contextPivot:c};state.recordAssistant(h,r,c.activeTopic);return packet}
function wrap(fn){if(typeof fn!=="function"||fn.__sessionContinuityR8)return fn;const w=function(){const raw=arguments[0],h=state?state.hydrate(raw):raw,qq=q(h);if(state&&qq)state.recordUser(h,qq);const terminal=build(h);if(terminal)return fn.constructor&&fn.constructor.name==="AsyncFunction"?Promise.resolve(terminal):terminal;const args=[...arguments];args[0]=h;const out=fn.apply(this,args),done=r=>{const rr=replyOf(r);if(state&&rr)state.recordAssistant(h,rr);return r};return out&&typeof out.then==="function"?out.then(done):done(out)};w.__sessionContinuityR8=true;return w}
for(const n of ["handleChat", "run", "chat", "handle", "reply", "default", "processWithMarion", "route", "ask"])if(typeof api[n]==="function")api[n]=wrap(api[n]);
if(typeof api.ChatEngine==='function'&&api.ChatEngine.prototype){for(const n of ['processInput','process','handle','run','chat','reply','ask'])if(typeof api.ChatEngine.prototype[n]==='function')api.ChatEngine.prototype[n]=wrap(api.ChatEngine.prototype[n]);}
api.buildNyxSessionContinuityReply=build;api.__sessionContinuityR8=true;})();
/* NYX_SESSION_CONTINUITY_R8_END */
/* NYX_E2E_CONTINUITY_COHESION_R9_START */
(function(){"use strict";const api=module.exports&&typeof module.exports==="object"?module.exports:null;if(!api||api.__e2eContinuityR9)return;const path=require("path"),root=path.join(__dirname,"..","Data","marion","runtime"),__e2eLoad=(name)=>{try{return require(path.join(root,name))}catch(_){return null}},__e2eState=__e2eLoad("marionContinuityState.js"),__e2eProgression=__e2eLoad("marionConversationProgression.js"),__e2ePivot=__e2eLoad("marionContextPivot.js");if(!__e2eState||!__e2eProgression||!__e2ePivot){api.NYX_E2E_CONTINUITY_COHESION_STATUS={version:"nyx.chatEngine.e2eContinuityCohesion/9.1-fail-open",active:false,degraded:true,missing:{state:!__e2eState,progression:!__e2eProgression,pivot:!__e2ePivot},noUserFacingDiagnostics:true};api.__e2eContinuityR9=true;return;}
...
function __E2E_CONT_PACKET(input,source){const h=__e2eState.hydrate(input),p=__e2eProgression.buildProgressionPacket(h);if(!p||!p.active||!p.resolved)return null;const c=__e2ePivot.buildContextPivot(h);if(!c.preservePriorSubject)return null;const reply=__E2E_CONT_TEXT(p.reply),subIntent="continuation_"+p.action,finalEnvelope={contractVersion:"nyx.marion.final/1.0",signature:"MARION_FINAL_AUTHORITY",authority:source,intent:"contextual_directive",subIntent,reason:"e2e_continuity_resolved",reply,text:reply,displayReply:reply,visibleReply:reply,finalReply:reply,spokenText:reply,final:true,marionFinal:true,handled:true,continuityHandled:true,continuityTerminal:true,noUserFacingDiagnostics:true};return{ok:true,handled:true,final:true,finalized:true,terminal:true,marionFinal:true,awaitingMarion:false,transportSafe:true,suppressUserFacingReply:false,emit:true,blocked:false,canEmit:true,continuityHandled:true,continuityTerminal:true,suppressGenericIdentityFallback:true,intent:"contextual_directive",subIntent,reason:"e2e_continuity_resolved",reply,text:reply,answer:reply,output:reply,response:reply,message:reply,displayReply:reply,visibleReply:reply,finalReply:reply,directReply:reply,authoritativeReply:reply,spokenText:reply,payload:{reply,text:reply,message:reply,answer:reply,output:reply,response:reply,displayReply:reply,visibleReply:reply,finalReply:reply,directReply:reply,authoritativeReply:reply,spokenText:reply,finalEnvelope,final:true,marionFinal:true,handled:true,continuityHandled:true,continuityTerminal:true,suppressGenericIdentityFallback:true,intent:"contextual_directive",subIntent,progression:p,contextPivot:c},finalEnvelope,meta:{replyAuthority:source,finalAuthority:source,continuityHandled:true,continuityTerminal:true,suppressGenericIdentityFallback:true,intent:"contextual_directive",subIntent,noUserFacingDiagnostics:true},progression:p,contextPivot:c}}

function prompt(i){if(typeof i==="string")return __E2E_CONT_TEXT(i);const x=__E2E_CONT_OBJ(i),p=__E2E_CONT_OBJ(x.payload),b=__E2E_CONT_OBJ(x.body),r=__E2E_CONT_OBJ(x.turn);return __E2E_CONT_TEXT(x.prompt||x.rawUserText||x.originalUserText||x.userText||x.query||x.message||x.text||p.prompt||p.userText||p.text||b.prompt||b.userText||b.text||r.prompt||r.userText||r.text)}
function wrap(fn){if(typeof fn!=="function"||fn.__e2eContinuityR9)return fn;const w=function(){const raw=arguments[0],h=__e2eState.hydrate(raw),q=prompt(h);if(q)__e2eState.recordUser(h,q);const term=__E2E_CONT_PACKET(h,"nyx_e2e_continuity_cohesion");if(term){__e2eState.recordAssistant(h,term.reply,{activeTopic:term.contextPivot.activeTopic,intent:term.intent,subIntent:term.subIntent});return fn.constructor&&fn.constructor.name==="AsyncFunction"?Promise.resolve(term):term}const a=Array.from(arguments);a[0]=h;const out=fn.apply(this,a),project=r=>{const reply=__E2E_CONT_REPLY(r);if(reply)__e2eState.recordAssistant(h,reply,{intent:r&&r.intent,subIntent:r&&r.subIntent});return r};return out&&typeof out.then==="function"?out.then(project):project(out)};w.__e2eContinuityR9=true;return w}
for(const n of["handleChat","run","chat","handle","reply","default","processWithMarion","route","ask"])if(typeof api[n]==="function")api[n]=wrap(api[n]);if(typeof api.ChatEngine==="function"&&api.ChatEngine.prototype)for(const n of["processInput","process","handle","run","chat","reply","ask"])if(typeof api.ChatEngine.prototype[n]==="function")api.ChatEngine.prototype[n]=wrap(api.ChatEngine.prototype[n]);api.buildNyxE2EContinuityReply=i=>__E2E_CONT_PACKET(i,"nyx_e2e_continuity_cohesion");api.__e2eContinuityR9=true;})();
/* NYX_E2E_CONTINUITY_COHESION_R9_END */

/* NYX_CONTINUITY_E2E_R10_START */
(function(){"use strict";const api=module.exports&&typeof module.exports==="object"?module.exports:null;if(!api||api.__continuityE2ER10)return;const path=require("path"),root=path.join(__dirname,"..","Data","marion","runtime"),s=(()=>{try{return require(path.join(root,"marionContinuityState.js"))}catch(_){return null}})(),p=(()=>{try{return require(path.join(root,"marionConversationProgression.js"))}catch(_){return null}})(),v=(()=>{try{return require(path.join(root,"marionContextPivot.js"))}catch(_){return null}})(),o=x=>x&&typeof x==="object"&&!Array.isArray(x)?x:{},t=x=>{try{return String(x??"").replace(/[\u0000-\u001f\u007f]/g," ").replace(/\s+/g," ").trim()}catch(_){return""}};function q(i){if(typeof i==="string")return t(i);const x=o(i),a=o(x.payload),b=o(x.body);return t(x.prompt||x.rawUserText||x.userText||x.query||x.message||x.text||a.prompt||a.userText||a.text||b.prompt||b.userText||b.text)}function rr(r){const x=o(r),a=o(x.payload),f=o(x.finalEnvelope);for(const z of[x.authoritativeReply,x.directReply,x.visibleReply,x.displayReply,x.finalReply,x.reply,x.answer,x.response,x.text,x.message,f.finalReply,f.reply,f.text,a.authoritativeReply,a.directReply,a.visibleReply,a.displayReply,a.finalReply,a.reply,a.text]){const y=t(z);if(y)return y}return""}function packet(i){if(!s||!p)return null;const h=s.hydrate(i),a=p.buildProgressionPacket(h);if(!a||!a.active||!a.resolved)return null;const c=v?v.buildContextPivot(h):{};if(c.preservePriorSubject===false)return null;const r=t(a.reply),u="continuation_"+a.action;if(!r)return null;const f={contractVersion:"nyx.marion.final/1.0",signature:"MARION_FINAL_AUTHORITY",authority:"nyx_continuity_e2e_r10",intent:"contextual_directive",subIntent:u,reply:r,text:r,displayReply:r,visibleReply:r,finalReply:r,spokenText:r,final:true,marionFinal:true,handled:true,continuityHandled:true,continuityTerminal:true};const z={ok:true,handled:true,final:true,terminal:true,marionFinal:true,emit:true,blocked:false,continuityHandled:true,continuityTerminal:true,suppressGenericIdentityFallback:true,intent:"contextual_directive",subIntent:u,reply:r,text:r,answer:r,response:r,message:r,displayReply:r,visibleReply:r,finalReply:r,directReply:r,authoritativeReply:r,spokenText:r,payload:{reply:r,text:r,displayReply:r,visibleReply:r,finalReply:r,directReply:r,authoritativeReply:r,spokenText:r,finalEnvelope:f,final:true,handled:true,continuityHandled:true,continuityTerminal:true,suppressGenericIdentityFallback:true,subIntent:u,progression:a,contextPivot:c},finalEnvelope:f,meta:{replyAuthority:"nyx_continuity_e2e_r10",continuityHandled:true,continuityTerminal:true,suppressGenericIdentityFallback:true,noUserFacingDiagnostics:true},progression:a,contextPivot:c};s.recordAssistant(h,r,{activeTopic:c.activeTopic});return z}function wrap(fn){if(typeof fn!=="function"||fn.__continuityE2ER10)return fn;const w=function(){const raw=arguments[0],h=s?s.hydrate(raw):raw,x=q(h);if(s&&x)s.recordUser(h,x);const z=packet(h);if(z)return fn.constructor&&fn.constructor.name==="AsyncFunction"?Promise.resolve(z):z;const a=[...arguments];a[0]=h;const out=fn.apply(this,a),done=r=>{const x=rr(r);if(s&&x)s.recordAssistant(h,x);return r};return out&&typeof out.then==="function"?out.then(done):done(out)};w.__continuityE2ER10=true;return w}for(const n of["handleChat","run","chat","handle","reply","default","processWithMarion","route","ask"])if(typeof api[n]==="function")api[n]=wrap(api[n]);if(typeof api.ChatEngine==="function"&&api.ChatEngine.prototype)for(const n of["processInput","process","handle","run","chat","reply","ask"])if(typeof api.ChatEngine.prototype[n]==="function")api.ChatEngine.prototype[n]=wrap(api.ChatEngine.prototype[n]);api.buildNyxContinuityE2EReply=packet;api.__continuityE2ER10=true})();
/* NYX_CONTINUITY_E2E_R10_END */
