...
async function handleChat(input) {
  ...
  try {
    ...
    // Emotion precedence: never force a lane-selection prompt on vulnerable inputs.
    if (emo && (emo.bypassClarify || emo?.disclaimers?.needCrisis || emo?.disclaimers?.needSoft)) {
      if (!safeStr(norm.lane || '').trim()) norm.lane = 'general';
      if (isPlainObject(norm.turnSignals)) norm.turnSignals.forceLane = 'general';
    }
...
const session = isPlainObject(norm?.body?.session)
    ...
    const emoSupport = !!(emo && (
      emoMode === "SUPPORT" || emoMode === "CRISIS" || emoMode === "DISTRESS" ||
      !!emo.bypassClarify || !!emo?.disclaimers?.needCrisis || !!emo?.disclaimers?.needSoft ||
      (clampInt(emo?.intensity || 0, 0, 0, 100) >= 60 && Array.isArray(emo?.tags) && emo.tags.length)
    ));

    if (emoSupport && Support && typeof Support.buildSupportiveResponse === "function") {
...
${base0}`
  ...
  marionCog: cog,
    ...
    // -------------------------
    // EMOTION OVERRIDE RETURN++++
    // - If the user signals distress (e.g., "I am hurting"), we must not fall back to lane selection.
    // - This has precedence over discovery prompts and bridge/lane nudges.
    // -------------------------
    if (emo && (emo.bypassClarify || emo?.disclaimers?.needCrisis || emo?.disclaimers?.needSoft) && supportPrefix) {
      const reply0emo = finalizeReply(
        supportPrefix,
        "I’m here with you. What’s the hardest part right now?"
      );

      // Keep lane stable + avoid any accidental bridge override.
      const lane0emo = safeStr(norm.lane || "general") || "general";

      const loopE = detectAndPatchLoop(session, lane0emo, reply0emo);
      const replyE = loopE.tripped ? finalizeReply("I’m here with you. Tell me what’s going on, in one sentence.") : reply0emo;

      const sigLineE = detectSignatureLine(replyE);

      const coreNextE = finalizeSpineTurn({
        corePrev,
        norm,
        lane: lane0emo,
        topic: safeStr(corePrev?.topic || ""),
        actionTaken: loopE.tripped ? "emotion_override_loop_break" : "emotion_override",
        followUps: [],
        pendingAsk: pendingAskObj("emotion_support", "clarify", "Ask what’s going on, gently.", true),
        decision: { ...corePlan, move: "deliver", stage: "deliver", speak: "" },
        assistantSummary: "emotion_override",
        marionCog: cog,
        updateReason: "emotion_override",
      });

      const routePatchE = {
        lane: lane0emo,
        ...(sigLineE ? { lastSigTransition: sigLineE } : {}),
        ...loopE.patch,
        __spineState: coreNextE,
        __emotionLast: { mode: safeStr(emo?.mode || ""), intensity: clampInt(emo?.intensity || 0, 0, 0, 100), at: nowMs() },
      };

      return buildContract({
        reply: replyE,
        lane: lane0emo,
        sessionPatch: mergeSessionPatch(baseCogPatch, routePatchE),
        meta: metaBase({
          route: "emotion_override",
          loop: { tripped: loopE.tripped, sig: loopE.sig, n: loopE.n },
          spine: {
            v: Spine.SPINE_VERSION,
            rev: coreNextE.rev,
            lane: coreNextE.lane,
            stage: coreNextE.stage,
            move: "deliver",
          },
        }),
      });
