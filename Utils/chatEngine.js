...
/* MARION_PRIVATE_TECHNICAL_FINAL_MISMATCH_GATE_V5_START */
(function marionPrivateTechnicalFinalMismatchGateV5(){
  "use strict";
  const VERSION="marion.chatEngine.privateTechnicalFinalMismatchGate/5.0";
  const sessions=new Map(),TTL=45*60*1000,MAX=1000;
  function O(v){return v&&typeof v==="object"&&!Array.isArray(v)?v:{};}
  function T(v,n=8000){try{return String(v==null?"":v).replace(/[\u0000-\u001f\u007f]/g," ").replace(/\s+/g," ").trim().slice(0,n);}catch(_){return"";}}
  function N(v){return T(v).toLowerCase().replace(/[’‘]/g,"'").replace(/[^a-z0-9]+/g," ").replace(/\s+/g," ").trim();}
  function prompt(v){const x=O(v),p=O(x.payload),m=O(x.meta),b=O(x.body);return T(x.rawUserText||x.userText||x.prompt||x.query||x.text||x.message||b.rawUserText||b.userText||b.prompt||b.text||p.rawUserText||p.userText||p.prompt||p.text||m.rawUserText||m.userText||m.prompt||m.text);}
  function sid(v){const x=O(v),p=O(x.payload),m=O(x.meta);return T(x.sessionId||x.conversationId||x.threadId||p.sessionId||p.conversationId||m.sessionId||"private-marion",180);}
  function privateCtx(v){const x=O(v),p=O(x.payload),m=O(x.meta);const s=T([x.route,x.path,x.source,x.inputChannel,x.adminInterfaceScope,m.route,p.route].join(" ")).toLowerCase();return x.privateAdminConversation===true||x.marionAdminConversation===true||x.directMarionAdminInterface===true||x.adminVerified===true||x.passwordFreeTestChat===true||p.marionAdminConversation===true||m.marionAdminConversation===true||/private\/marion|marion_admin|admin_conversation/.test(s);}
  function classify(q){const n=N(q);const social=/^(?:hi|hello|hey|good morning|good afternoon|good evening|thanks|thank you|okay|ok|got it)(?: marion)?$/.test(n);const contextual=/^(?:go deeper|continue|keep going|next|next step|next steps|what next|what now|then what|what should be fixed first|what should we fix first|why is that the first priority|why first|what could break|what could go wrong|what is the safest implementation order|safest implementation order|how do we validate the repair|how should we validate the repair|how do we test the repair|what is the main risk|what happens after that|what is the first concrete defect|what should i inspect first)$/.test(n);const tech=/\b(?:surgical autopsy|autopsy|javascript|index js|chatengine|intent router|domain router|routing file|runtime|backend|frontend|bridge|final envelope|state spine|module|function|payload|syntax|patch|hotfix|implementation order|validate the repair|test the repair)\b/.test(n);const legal=!tech&&/\b(?:legal advice|legal risk|lawyer|attorney|court|tribunal|lawsuit|liability|negligence|damages|contract|agreement|clause|breach|copyright|licensing rights|trademark|patent|jurisdiction|statute|regulation|compliance)\b/.test(n);return{social,contextual,tech,legal,n};}
  function carry(v){const x=O(v),nodes=[x,O(x.routing),O(x.marionIntent),O(x.meta),O(x.sessionPatch),O(x.memoryPatch),O(x.previousMemory),O(x.state),O(x.payload),O(x.finalEnvelope)],fields=["domain","primaryDomain","selectedDomain","knowledgeDomain","activeFeatureLane","activeTask","currentTask","lastValidTask","activeSubject","continuationAnchor","technicalTarget","targetFile","targetPath","currentObjective","nextAction","effectivePrompt","normalizedUserIntent"];let s="";for(const node of nodes)for(const f of fields){const z=T(node[f],600);if(z)s+=" "+z;}return s.slice(0,9000);}
  function techCarry(v){const t=carry(v).toLowerCase();return /\btechnical\b/.test(t)||/\b(?:javascript|index\.?js|chatengine|intent router|domain router|routing|runtime|backend|frontend|bridge|final envelope|state spine|module|function|payload|code|patch|repair|implementation)\b/.test(t);}
  function subject(v,q){const x=O(v),nodes=[x,O(x.routing),O(x.marionIntent),O(x.meta),O(x.sessionPatch),O(x.previousMemory),O(x.payload)],fields=["activeTask","currentTask","lastValidTask","activeSubject","continuationAnchor","technicalTarget","targetFile","targetPath"];for(const node of nodes)for(const f of fields){const z=T(node[f],700);if(z&&!classify(z).contextual&&!classify(z).social)return z;}return T(q,700)||"the active JavaScript routing repair";}
  function reply(v){const x=O(v),p=O(x.payload),r=O(x.result),f=O(x.finalEnvelope||p.finalEnvelope||r.finalEnvelope);for(const z of[x.reply,x.directReply,x.visibleReply,x.displayReply,x.finalReply,x.answer,x.response,x.output,x.text,x.message,p.reply,p.visibleReply,p.displayReply,p.text,p.message,r.reply,r.visibleReply,r.displayReply,r.text,r.message,f.reply,f.visibleReply,f.displayReply,f.text,f.message]){const t=T(z);if(t)return t;}return"";}
  function legalFallback(v){const t=T(v).toLowerCase();return /\bi can (?:give|provide|frame) general legal[- ]risk triage\b/.test(t)||(/\bnot legal advice\b/.test(t)&&/\blegal category\b/.test(t))||/\bjurisdiction sensitivity\b/.test(t);}
  function metaLeak(v){return /\b(?:immediately preceding turn|must remain the authority|active code target|older unrelated lane|continuation anchor|preserve the active subject|reply authority|current turn authority)\b/i.test(T(v));}
  function candidate(value,q){const seen=new WeakSet();let best="",score=-1;function walk(v,d,k){if(d>7||v==null)return;if(typeof v==="string"){const t=T(v);if(!t||t===q||legalFallback(t)||metaLeak(t))return;const l=t.toLowerCase();let s=/reply|answer|response|output|text|message|final/i.test(String(k||""))?3:0;if(/\b(?:router|routing|javascript|runtime|state|bridge|module|function|payload|validation|test|implementation|precedence|session|domain)\b/.test(l))s+=5;if(t.length>=90)s+=2;if(s>score){best=t;score=s;}return;}if(typeof v!=="object")return;try{if(seen.has(v))return;seen.add(v);}catch(_){return;}if(Array.isArray(v)){for(let i=0;i<Math.min(v.length,40);i++)walk(v[i],d+1,k);return;}let keys=[];try{keys=Object.keys(v).slice(0,120);}catch(_){return;}for(const ck of keys){if(/^(?:req|res|socket|connection|stream|headers)$/i.test(ck))continue;let c;try{c=v[ck];}catch(_){continue;}walk(c,d+1,ck);}}walk(value,0,"");return score>=7?best:"";}
  function stamp(node,rep,sub,recovered){if(!node||typeof node!=="object"||Array.isArray(node))return node;for(const k of["reply","directReply","publicReply","visibleReply","displayReply","finalReply","answer","response","output","text","message"])node[k]=rep;node.domain="technical";node.primaryDomain="technical";node.selectedDomain="technical";node.knowledgeDomain="technical";node.activeFeatureLane="technical";node.activeTask=sub;node.activeSubject=sub;node.r18CLawRealWorldAssessment=false;node.lawTechnicalSurgeryGuard=true;node.staleLawCarrySuppressed=true;node.semanticHealth=recovered?"recovered":"ready";return node;}
  function suppress(node,sub){if(!node||typeof node!=="object"||Array.isArray(node))return node;for(const k of["reply","directReply","publicReply","visibleReply","displayReply","finalReply","answer","response","output","text","message"])node[k]="";node.ok=false;node.emit=false;node.blocked=true;node.awaitingMarion=true;node.suppressUserFacingReply=true;node.domain="technical";node.primaryDomain="technical";node.selectedDomain="technical";node.knowledgeDomain="technical";node.activeTask=sub;node.failureSignature="ROUTE_DOMAIN_MISMATCH";node.semanticHealth="degraded";return node;}
  function prune(){const now=Date.now();for(const[k,v]of sessions)if(!v||now-Number(v.updatedAt||0)>TTL)sessions.delete(k);if(sessions.size>MAX){for(const[k]of Array.from(sessions.entries()).sort((a,b)=>a[1].updatedAt-b[1].updatedAt).slice(0,sessions.size-MAX))sessions.delete(k);}}
  function project(value,input){if(!privateCtx(input))return value;const q=prompt(input),c=classify(q),key=sid(input);if(O(input).newSession===true||O(input).firstTurn===true||O(input).resetSession===true||O(input).freshSession===true)sessions.delete(key);const prev=sessions.get(key)||{},tc=techCarry(input)||techCarry(value);let lane=prev.lane||"";if(c.social)lane="";else if(c.legal)lane="law";else if(c.tech||tc)lane="technical";const sub=subject(input,q);if(c.social)sessions.delete(key);else if(lane)sessions.set(key,{lane,subject:prev.subject||sub,updatedAt:Date.now()});prune();const should=!c.social&&!c.legal&&(c.tech||(c.contextual&&(lane==="technical"||prev.lane==="technical"||tc)));if(!should)return value;const current=reply(value),bad=legalFallback(current)||metaLeak(current)||/\blaw\b/i.test(T(O(value).domain||O(O(value).routing).domain));if(!bad)return value;const replacement=candidate(value,q);if(typeof value==="string")return replacement||"";const out={...O(value)},active=prev.subject||sub;if(replacement){stamp(out,replacement,active,true);out.payload=stamp({...O(out.payload)},replacement,active,true);out.result=stamp({...O(out.result)},replacement,active,true);out.finalEnvelope=stamp({...O(out.finalEnvelope)},replacement,active,true);out.meta={...O(out.meta),privateTechnicalMismatchGateVersion:VERSION,semanticMismatchRecovered:true,expectedDomain:"technical",semanticHealth:"recovered"};}else{suppress(out,active);out.payload=suppress({...O(out.payload)},active);out.result=suppress({...O(out.result)},active);out.finalEnvelope=suppress({...O(out.finalEnvelope)},active);out.meta={...O(out.meta),privateTechnicalMismatchGateVersion:VERSION,semanticMismatchRecovered:false,expectedDomain:"technical",semanticHealth:"degraded",failureSignature:"ROUTE_DOMAIN_MISMATCH"};}return out;}
  function wrap(fn,name){if(typeof fn!=="function"||fn.__marionPrivateTechnicalMismatchV5)return fn;const w=function(){const input=arguments&&arguments.length?arguments[0]:{},r=fn.apply(this,arguments),p=v=>project(v,input);return r&&typeof r.then==="function"?r.then(p):p(r);};try{Object.keys(fn).forEach(k=>w[k]=fn[k]);}catch(_){}w.__marionPrivateTechnicalMismatchV5=true;w.__wrappedName=name;return w;}
  try{const api=module.exports&&typeof module.exports==="object"?module.exports:null;if(!api)return;for(const n of["handleChat","run","chat","handle","reply","normalizeCoordinatorOutputForPipeline","normalizeVisibleFinalReplyFields","finalTransportPacket"])if(typeof api[n]==="function")api[n]=wrap(api[n],n);api.MARION_PRIVATE_TECHNICAL_FINAL_MISMATCH_GATE_VERSION=VERSION;api.projectMarionPrivateTechnicalFinalMismatch=project;}catch(_){}
})();
/* MARION_PRIVATE_TECHNICAL_FINAL_MISMATCH_GATE_V5_END */
