...
/**
 * Utils/musicKnowledge.js — v2.75 (BULLETPROOF OUTPUT SHAPE + CHIP CONTRACT FIX + LOOP-DAMPENER + PARSE HARDEN)
 *
 * Non-negotiable goals:
 *  - Access music sources from 1950 through 2024 (as files exist on disk).
 *  - Top 10 ALWAYS prefer Top10 Store when present (Data/top10_by_year_v1.json).
 *  - 1950–1959 prefers Year-End Singles spine if available.
 *  - Deterministic, safe, never throws.
 *
 * v2.75 critical fixes:
 *  ✅ CONTRACT FIX: ALWAYS returns followUps (plural) — never "followUp".
 *  ✅ LOOP DAMPENER: session.__musicLastSig prevents re-emitting identical content.
 *  ✅ PARSE HARDEN: supports "top10 1980", "billboard top 10 1980", "#1 1980", "year end 1980".
 *  ✅ DEFAULT OUTPUT: "Top 10" is the primary user-facing content; chart is internal.
 */
...
// =========================
const MK_VERSION =
  "musicKnowledge v2.75 (Output shape fixed; followUps contract; loop dampener; parse hardened; store-first)";
...
function parseCommand(msg) {
  ...
  // top 10 variants:
  // "top 10 1988", "top10 1988", "billboard top 10 1988", "top ten 1988"
  ...
  // "top 10" with no year -> use lastMusicYear
  if (/\b(?:billboard\s+)?top\s*(?:10|ten)\b/.test(t) && !/\d{4}/.test(t)) return { kind: "top10" };

  // year-end explicit (we keep supported but do NOT auto-enter it)
  ...
  // #1 1988 OR plain #1 (uses lastMusicYear)
...
function getTopByYear(year, chart, limit = 10) {
  ...
  // 1) Top10 Store (preferred for YEAR_END_CHART)
  if (c === YEAR_END_CHART) {
...
function formatTopList(year, chart, limit = 10) {
  ...
  // IMPORTANT: user-facing label stays "Top 10", chart shown as descriptor
...
function getNumberOneLine(year, chart) {
  ...
  // Prefer store for #1 when possible
...
function handleChat({ text, session }) {
  ...
  if (cmd && cmd.kind === "top10") {
    ...
    return outShape({
      ...
      sessionPatch: canonicalPatch(session, { lastMusicYear: y }),
    });
  }

  // ---- explicit yearend request (supported but not default)
  ...
  if (cmd && cmd.kind === "number1") {
    ...
    const resolved = resolveChartForYear(y, session.lastMusicChart || session.activeMusicChart);
