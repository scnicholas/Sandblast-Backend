...
const VOICE_ROUTE_VERSION = "voiceRoute v1.6.0 SYNTH-ENDPOINT-HEALTH + PATH-RESOLUTION + DIRECT-BUFFER-HARDEN";
...
async function health(){try{const info=ttsHealth?await Promise.resolve(ttsHealth()):null;const env=info&&typeof info==="object"&&info.env&&typeof info.env==="object"?info.env:{};const configured=!!(DEFAULT_VOICE_UUID||(info&&typeof info==="object"&&(info.configured||info.voiceConfigured||info.ready)));return{ok:!!delegateTts&&(!info||info.ok!==false),enabled:!!delegateTts,version:VOICE_ROUTE_VERSION,ttsModuleLoaded:!!ttsMod,ttsModulePath:ttsLoad.path||undefined,ttsDelegateBound:!!delegateTts,ttsHealthBound:!!ttsHealth,voiceConfigured:configured,synthesizeEndpointConfigured:env.hasSynthesizeEndpoint===true,synthesizeEndpointExplicitlyConfigured:env.synthesizeEndpointExplicitlyConfigured===true,synthesizeEndpointSource:safeStr(env.synthesizeEndpointSource||"unavailable"),providerEndpointEnv:"RESEMBLE_SYNTH_URL",provider:DEFAULT_PROVIDER,loadError:ttsLoadError||undefined,tts:info&&typeof info==="object"?info:null};}catch(err){return{ok:false,enabled:!!delegateTts,version:VOICE_ROUTE_VERSION,ttsModuleLoaded:!!ttsMod,ttsModulePath:ttsLoad.path||undefined,ttsDelegateBound:!!delegateTts,ttsHealthBound:!!ttsHealth,voiceConfigured:!!DEFAULT_VOICE_UUID,synthesizeEndpointConfigured:false,synthesizeEndpointExplicitlyConfigured:false,synthesizeEndpointSource:"unavailable",providerEndpointEnv:"RESEMBLE_SYNTH_URL",provider:DEFAULT_PROVIDER,error:safeStr(err&&(err.message||err)||"tts_health_failed")};}}
