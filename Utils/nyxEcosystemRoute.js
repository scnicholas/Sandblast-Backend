...
router.get("/content/status", (_req, res) => res.json(content.status()));
router.get("/content/catalog/:source", (req, res) => { const result=content.catalog(String(req.params.source||"").toLowerCase()); res.status(result.status==="invalid"?404:200).json(result); });
router.get("/release/validate", (_req, res) => { const result=content.releaseValidation(); res.status(result.releaseReady?200:503).json(result); });
...
module.exports.contentRegistry = content;
module.exports.releaseValidation = content.releaseValidation;
