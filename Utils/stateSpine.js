...
(function(){
  "use strict";
  const V="nyx.guideOrchestration.stateSpine/4.0-steps10-11-12",EC="nyx.guideExecutionState/1.0",SC="nyx.guideStateTransition/1.0",LANES=new Set(["home","search","live","watch","roku","news","about","apps"]),MAP={sandblast_home:"home",sandblast_radio:"live",sandblast_tv:"watch",sandblast_cartoons:"watch",sandblast_classics:"watch",sandblast_roku:"roku",synapse:"news",lingosentinel:"about",apps:"apps",about:"about",nyx_guide:"search",guide_input:"search"};
  function o(v){return v&&typeof v==="object"&&!Array.isArray(v)?v:{}}function x(v,n=120){return String(v==null?"":v).replace(/[\u0000-\u001f\u007f]/g,"").replace(/\s+/g," ").trim().slice(0,n)}function lane(v,f="home"){const s=x(v,32).toLowerCase().replace(/[^a-z0-9_-]+/g,"");return LANES.has(s)?s:f}
  function find(v,args){let plan={},exec={},result={},ctx={},prior={};const seen=new Set();function w(q,d){if(!q||typeof q!=="object"||d>6||seen.has(q))return;seen.add(q);q=o(q);if(!Object.keys(plan).length&&Array.isArray(o(q.guideActionPlan).actions))plan=o(q.guideActionPlan);if(!Object.keys(exec).length&&o(q.nyxGuideExecution).contract)exec=o(q.nyxGuideExecution);if(!Object.keys(result).length&&o(q.guideActionResult||q.guideExecutionReceipt).status)result=o(q.guideActionResult||q.guideExecutionReceipt);if(!Object.keys(ctx).length)ctx=o(q.guideContext||q.publicGuideContinuity);if(!Object.keys(prior).length)prior=o(q.nyxGuideExecutionState||o(q.sessionPatch).nyxGuideExecutionState||o(q.state).nyxGuideExecutionState);for(const k of["payload","body","meta","result","finalEnvelope","routing","runtimeState","state","session","sessionPatch","memoryPatch","composerContext"])w(q[k],d+1)}for(const q of Array.from(args||[]).concat([v]))w(q,0);return{plan,exec,result,ctx,prior}}
  function normalize(f){const p=o(f.prior),r=o(f.result),e=o(f.exec),c=o(f.ctx),actions=Array.isArray(o(f.plan).actions)?f.plan.actions:[],first=o(actions[0]),status=x(r.status||e.status||p.status||(actions.length?"pending_user_gesture":"idle"),32).toLowerCase(),old=lane(p.currentLane||c.currentLane||e.currentLane,"home"),target=lane(r.currentLane||r.targetLane||MAP[x(r.target||first.target,64)]||first.lane||old,old),rollback=lane(r.rollbackLane||e.rollbackLane||p.rollbackLane||old,old),completed=status==="completed",failed=status==="failed"||status==="cancelled",current=completed?target:failed?rollback:old,changed=current!==old||status!==p.status||x(r.actionId||e.actionId||first.id,80)!==x(p.lastActionId,80),revision=Math.max(0,Number(p.revision||c.revision||e.revision||0)||0)+(changed?1:0),actionId=x(r.actionId||e.actionId||first.id,80),history=Array.isArray(p.executionKeys)?p.executionKeys.slice(-7):[],key=x(r.receiptId||r.idempotencyKey||e.idempotencyKey||actionId,80);if(key&&!history.includes(key))history.push(key);return{contract:EC,transitionContract:SC,version:V,revision,status,currentLane:current,previousLane:changed?old:lane(p.previousLane||c.previousLane||old,old),rollbackLane:rollback,planId:x(r.planId||e.planId||o(f.plan).planId||p.planId,80),lastActionId:actionId,lastCompletedActionId:completed?actionId:x(p.lastCompletedActionId,80),lastFailedActionId:failed?actionId:x(p.lastFailedActionId,80),pendingActionIds:status.startsWith("pending")?actions.map(a=>x(o(a).id,80)).filter(Boolean).slice(0,6):[],executionKeys:history,requiresUserGesture:true,autoExecute:false,idempotencyRequired:true,rollbackRequired:true,publicSessionOnly:true,privateMemoryAccess:false,noRawUserTextStored:true,updatedAt:Date.now(),expiresAt:Date.now()+30*60*1000}}
  function project(v,args){if(!v||typeof v!=="object"||Array.isArray(v))return v;const f=find(v,args);if(!Object.keys(f.prior).length&&!Object.keys(f.exec).length&&!Object.keys(f.result).length&&!Array.isArray(f.plan.actions))return v;const state=normalize(f),out={...v,nyxGuideExecutionState:state,nyxGuideStateTransition:{contract:SC,version:V,revision:state.revision,status:state.status,currentLane:state.currentLane,previousLane:state.previousLane,rollbackLane:state.rollbackLane,planId:state.planId,actionId:state.lastActionId,publicSessionOnly:true}};out.sessionPatch={...o(out.sessionPatch),nyxGuideExecutionState:state};if(o(out.state))out.state={...out.state,nyxGuideExecutionState:state};if(o(out.stateSpine))out.stateSpine={...out.stateSpine,nyxGuideExecutionState:state};return out}
  function wrap(fn){if(typeof fn!=="function"||fn.__nyx101112State)return fn;const w=function(){const a=arguments,r=fn.apply(this,a);return r&&typeof r.then==="function"?r.then(v=>project(v,a)):project(r,a)};try{Object.keys(fn).forEach(k=>w[k]=fn[k])}catch(_){}w.__nyx101112State=true;return w}
...
/* MARION_STRATEGIC_FLOW_LAYERS_15_16_17_STATE_SPINE_V17_START */
...
/* MARION_STRATEGIC_FLOW_LAYERS_15_16_17_STATE_SPINE_V17_END */

/* MARION_COMPLETION_FLOW_LAYERS_18_19_20_STATE_SPINE_V20_START */
(function marionCompletionFlowStateSpineV20(){
  "use strict";
  try{
    const api=module.exports&&typeof module.exports==="object"?module.exports:null;if(!api)return;
    const registry=require("../Data/marion/runtime/conversation/marionConversationLayerRegistry.js");
    ...
    api.MARION_COMPLETION_STATE_BOUNDED=true;
    api.MARION_COMPLETION_STATE_PRIVATE=true;
...
/* MARION_LEAN_UI_PRESENCE_STATE_V1_START */
(function(){"use strict";const a=module.exports;if(!a||a.__leanPresenceStateV1)return;const V="nyx.marion.leanUiPresenceState/1.0",H=28,S=new Set(["idle","listening","thinking","speaking","complete","error"]);function s(v){try{return String(v==null?"":v).replace(/\s+/g," ").trim()}catch(_){return""}}a.normalizeMarionVisualPresenceState=function(x,f="idle"){const q=x&&typeof x==="object"?x:{state:x},r=s(q.state||q.visualState||q.presenceState||f).toLowerCase(),state=S.has(r)?r:f;return{version:V,state,authority:"widget",transient:["listening","thinking","speaking","complete"].includes(state),persistAcrossTurn:state==="error",audioDriven:q.audioDriven!==false,noUserFacingDiagnostics:true,executionAuthorized:false,hardStopLayer:H,updatedAt:Date.now()}};a.MARION_LEAN_UI_PRESENCE_STATE_VERSION=V;a.__leanPresenceStateV1=true})();
 /* MARION_LEAN_UI_PRESENCE_STATE_V1_END */
