...
const VERSION = "affectEngine v3.8.0 R18AB-AI-CYBER-PROTECTION + ANTI-LOOP-AFFECT-BRIDGE + BRIDGE-HARDENED NORMALIZED-HANDOFF + YEAR-SPEECH";
...
/* R17B: affect pacing + subtle personality */
(function(){try{const V="affectEngine/R17B-pacing-personality-coherence";function t(v){return v==null?"":String(v).replace(/\s+/g," ").trim()}function n(v){return t(v).toLowerCase()}function pace(v){v=n(v);if(/frustr|stuck|annoyed|tired/.test(v))return"slow_grounded";if(/pass|good|held/.test(v))return"brief_confident";if(/next|continue|keep going/.test(v))return"measured_forward";if(/still there|are you there/.test(v))return"short_reassuring";return"steady_present"}const old=module.exports.runAffectEngine;if(typeof old==="function"){module.exports.runAffectEngine=function(input,opts){const out=old.apply(this,arguments);if(out&&typeof out==="object"){const p=pace([input&&input.text,input&&input.prompt,input&&input.assistantDraft].join(" "));out.r17bContinuity={version:V,conversationPacing:p,microPersonality:"warm_controlled",longSessionCoherence:true};out.affectState=Object.assign({},out.affectState||{},{conversationPacing:p,microPersonality:"steady_mac_facing"});out.unifiedTurn=Object.assign({},out.unifiedTurn||{},{longSessionCoherence:true,turnPacing:p});}return out};}module.exports.MARION_R17B_AFFECT_VERSION=V;}catch(_){}})();
...
/* R18A/R18B affect carry: adaptive AI + protective cyber posture */
(function(){try{const V="affectEngine/R18AB-ai-cyber-protection";function T(v){return v==null?"":String(v).replace(/\s+/g," ").trim()}function N(v){return T(v).toLowerCase()}function D(v){v=N(v);return{ai:/\b(ai|artificial intelligence|model|reasoning|agent|automation|adaptive|intelligence|llm|machine learning)\b/.test(v),cyber:/\b(cyber|security|protect|identity|permission|access|token|secret|auth|authentication|authorization|least privilege|risk|threat|anomaly|credential)\b/.test(v)}}const old=module.exports.runAffectEngine;if(typeof old==="function"&&!old.__marionR18ABAffect){const fn=function(input,opts){const out=old.apply(this,arguments);if(out&&typeof out==="object"){const k=D([input&&input.text,input&&input.prompt,input&&input.assistantDraft].join(" "));out.r18AIDomainAdaptability={version:V,active:!!k.ai,frame:k.ai?"goal_context_data_risk_next_move":"baseline"};out.r18CybersecurityProtectiveProtocol={version:V,active:!!k.cyber,macScoped:true,leastPrivilege:true,explicitConfirmationRequired:!!k.cyber,noCovertMonitoring:true,noAutonomousEnforcement:true,secretRedaction:true};out.affectState=Object.assign({},out.affectState||{},{aiAdaptiveMode:k.ai?"applied_assessment":"baseline",protectivePosture:k.cyber?"cautious_verification":"baseline"});out.unifiedTurn=Object.assign({},out.unifiedTurn||{},{r18AIDomainAdaptability:!!k.ai,r18CybersecurityProtectiveProtocol:!!k.cyber,baselinePreserved:"r16m-r17c"});}return out};Object.defineProperty(fn,"__marionR18ABAffect",{value:true});module.exports.runAffectEngine=fn;}module.exports.MARION_R18AB_AFFECT_VERSION=V;}catch(_){}})();
