 ...
 *
 * v1.1.3 (AI KNOWLEDGE WIRE++++ + AI LAYER++++ + TRACE BIT ak++++ + FAIL-OPEN PRESERVED++++)
 * ✅ Adds aiKnowledge integration (FAIL-OPEN; NO RAW TEXT)
 * ✅ Adds aiKnowledgeHints to contract + trace bit ak
 * ✅ Adds AI layer tags/signals (AI + agents + ethics + cross-domain couplings)
 * ✅ Keeps: psychologyKnowledge + cyberKnowledge + EnglishKnowledge + financeKnowledge
 * ✅ Keeps: Law/Ethics/RiskBridge/Cyber/English/Fin/Strategy layers
 */

const MARION_VERSION = "marionSO v1.1.3";
...
function clampFinanceHints(hints) {
  ...
  return {
    ...
    queryKey: safeStr(h.queryKey || "", 18),
    packs: isPlainObject(h.packs)
      ? { primary: safeStr(h.packs.primary || "", 64), versions: isPlainObject(h.packs.versions) ? h.packs.versions : {} }
      : {},
    focus: safeStr(h.focus || "", 32),
    stance: safeStr(h.stance || "", 32),
    principles: uniqBounded(h.principles || [], 8),
...
function queryFinanceKnowledge(norm, session, cog) {
  ...
  try {
    ...
    if (packs) {
      return clampFinanceHints({ enabled: true, queryKey: q.queryKey, packs: { primary: "", versions: packs }, confidence: 0, reason: "packs_only" });
    }
...
function computeEthicsLayer(norm, psych, seed) {
  ...
  return { ethicsTags: uniqBounded(tags.map((x) => safeStr(x, 32)), 8), ethicsSignals: uniqBounded(signals.map((x) => safeStr(x, 40)), 6) };
}
...
function computeRiskBridge(norm, psych, ethics, lawSeed) {
  ...
  return { riskTier: tier, riskDomains: uniqBounded(domains, 6), riskSignals: uniqBounded(signals, 6), lawOverrides: isPlainObject(lawOverrides) ? { ...lawOverrides } : {} };
}
...
function applyLawLayer(seed, psych) {
  ...
  return { ...out, lawTags: tags.slice(0, 8).map((x) => safeStr(x, 32)), lawReasons: reasons.slice(0, 8).map((x) => safeStr(x, 40)) };
}
...
function computeEnglishLayer(norm, seed) {
  ...
  const technicalDensity =
    /\b(api|cors|endpoint|resolver|session|deterministic|contract|telemetry|governor|policy)\b/.test(text);
  ...
  return { englishTags: uniqBounded(tags.map((x) => safeStr(x, 24)), 8), englishSignals: uniqBounded(signals.map((x) => safeStr(x, 40)), 6) };
}
...
function computeAILayer(norm) {
  ...
  if (aiCore || llm || agents) {
    signals.push(AI.SIGNALS.DEFINE_TERMS, AI.SIGNALS.USE_EXAMPLES, AI.SIGNALS.MENTION_LIMITS);
  }
...
function mediate(norm, session, opts = {}) {
  try {
    ...
    const psychSeed = computePsychologyReasoningObject(n, s, { mode, intent, dominance, budget, actionable, textEmpty, stalled }, now);
    ...
    let cog = {
      ...
      handoff: {
        ...
        allowSameTurnSplit: true,
        marionTagSuggested:
          intent === "ADVANCE" ? MARION_STYLE_CONTRACT.tags.ok : intent === "STABILIZE" ? MARION_STYLE_CONTRACT.tags.hold : MARION_STYLE_CONTRACT.tags.ok,
      },
    ...
    const fail = {
      ...
      marionStyle: MARION_STYLE_CONTRACT,
      handoff: { marionEndsHard: true, nyxBeginsAfter: true, allowSameTurnSplit: true, marionTagSuggested: MARION_STYLE_CONTRACT.tags.retry },
