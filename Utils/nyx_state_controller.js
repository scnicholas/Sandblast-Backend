...
(function(){
  "use strict";
  const V="nyx.guideOrchestration.stateController/4.0-steps10-11-12",EC="nyx.guideExecution/1.0",SC="nyx.guideStateTransition/1.0",RC="nyx.guideReleaseGate/1.0",LANES=new Set(["home","search","live","watch","roku","news","about","apps"]),MAP={sandblast_home:"home",sandblast_radio:"live",sandblast_tv:"watch",sandblast_cartoons:"watch",sandblast_classics:"watch",sandblast_roku:"roku",synapse:"news",lingosentinel:"about",apps:"apps",about:"about",nyx_guide:"search",guide_input:"search"};
  const C=typeof module!=="undefined"&&module.exports?(module.exports.default||module.exports):(typeof window!=="undefined"?window.NyxStateController:null);if(!C||!C.prototype)return;const p=C.prototype;
  function o(v){return v&&typeof v==="object"&&!Array.isArray(v)?v:{}}function x(v,n=120){return String(v==null?"":v).replace(/[\u0000-\u001f\u007f]/g,"").replace(/\s+/g," ").trim().slice(0,n)}function lane(v,f="home"){const s=x(v,32).toLowerCase().replace(/[^a-z0-9_-]+/g,"");return LANES.has(s)?s:f}function emit(self,n,d){try{const target=self&&self.options&&self.options.eventTarget||typeof window!=="undefined"&&window;if(target&&target.dispatchEvent)target.dispatchEvent(new CustomEvent(n,{detail:d}))}catch(_){} }
  p.configureGuideReleaseGate=function(input={}){const q=o(input);this._guideReleaseGate={contract:RC,version:V,releaseReady:q.releaseReady===true,rollbackSafeMode:q.rollbackSafeMode===true,state:x(q.releaseReady===true?"ready":q.rollbackSafeMode?"rollback":"blocked",24),checkedAt:Date.now(),diagnosticsRedacted:true};emit(this,"nyx:guide:releasegate",this._guideReleaseGate);return this._guideReleaseGate};
  p.canExecuteGuideAction=function(action,plan={}){const a=o(action),g=o(this._guideReleaseGate),id=x(a.id,80);if(!id||a.requiresUserGesture===false||a.autoExecute===true)return false;if(g.rollbackSafeMode||g.releaseReady===false)return false;const e=o(this._guideExecutionState);if(e.locked&&e.actionId===id&&Date.now()<Number(e.lockExpiresAt||0))return false;const done=Array.isArray(e.completedKeys)?e.completedKeys:[];return!done.includes(x(a.idempotencyKey||id,80))};
  p.beginGuideAction=function(action,plan={}){const a=o(action),pl=o(plan);if(!this.canExecuteGuideAction(a,pl))return null;const continuity=typeof this.getPublicContinuitySnapshot==="function"?o(this.getPublicContinuitySnapshot()):{},current=lane(continuity.lane||continuity.currentLane||"home"),target=lane(a.lane||MAP[x(a.target,64)]||current,current),now=Date.now(),receipt={contract:EC,stateContract:SC,version:V,receiptId:`ui_${now.toString(36)}_${Math.random().toString(36).slice(2,8)}`,planId:x(pl.planId,80),actionId:x(a.id,80),idempotencyKey:x(a.idempotencyKey||a.id,80),status:"selected",currentLane:current,previousLane:lane(continuity.previousLane||current,current),targetLane:target,rollbackLane:current,revision:Math.max(0,Number(continuity.revision||o(this._guideExecutionState).revision||0)||0)+1,requiresUserGesture:true,autoExecute:false,selectedAt:now};this._guideExecutionState={...receipt,locked:true,lockExpiresAt:now+1200,completedKeys:Array.isArray(o(this._guideExecutionState).completedKeys)?o(this._guideExecutionState).completedKeys.slice(-15):[]};emit(this,"nyx:guide:actionselected",receipt);return receipt};
  p.completeGuideAction=function(receipt,success=true,reason=""){const r=o(receipt),e=o(this._guideExecutionState),ok=success===true,current=ok?lane(r.targetLane||e.targetLane,e.currentLane):lane(r.rollbackLane||e.rollbackLane,e.currentLane),keys=Array.isArray(e.completedKeys)?e.completedKeys.slice(-15):[],key=x(r.idempotencyKey||e.idempotencyKey||r.actionId,80);if(ok&&key&&!keys.includes(key))keys.push(key);this._guideExecutionState={...e,...r,status:ok?"completed":"failed",currentLane:current,previousLane:lane(e.currentLane||r.currentLane,"home"),revision:Math.max(Number(e.revision||0),Number(r.revision||0))+1,locked:false,lockExpiresAt:0,completedKeys:keys,reason:x(reason,80),completedAt:Date.now()};if(typeof this.configurePublicContinuity==="function")this.configurePublicContinuity({lane:current,previousLane:this._guideExecutionState.previousLane,revision:this._guideExecutionState.revision,lastAction:r.actionId});emit(this,ok?"nyx:guide:actioncompleted":"nyx:guide:actionfailed",this._guideExecutionState);return this._guideExecutionState};
  p.rollbackGuideAction=function(receipt,reason="rollback"){return this.completeGuideAction(receipt,false,reason)};
  const old=p.getGuideSnapshot;if(typeof old==="function"&&!old.__nyx101112){p.getGuideSnapshot=function(){return{...o(old.call(this)),nyxGuideExecutionState:o(this._guideExecutionState),nyxGuideReleaseGate:o(this._guideReleaseGate),guideExecutionContract:EC,guideStateTransitionContract:SC,guideReleaseGateContract:RC}};p.getGuideSnapshot.__nyx101112=true}
