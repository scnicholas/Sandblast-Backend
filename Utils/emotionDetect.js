...
function _fallbackEmotion(text) {
  ...
  if (/\b(depressed|hopeless|empty|numb)\b/.test(t)) return { primary: "sadness", secondary: "hopelessness", intensity: 0.86, confidence: 0.86 };
  if (/\b(sad|grief|heartbroken|down)\b/.test(t)) return { primary: "sadness", secondary: "hurt", intensity: 0.72, confidence: 0.8 };
  if (/\b(anxious|panic|worried|afraid|overwhelmed)\b/.test(t)) return { primary: "fear", secondary: "anxiety", intensity: 0.76, confidence: 0.82 };
...
function detectEmotion(input = {}) {
  ...
  const supportFlags = {
    highDistress: base.intensity >= 0.8 || /\b(depressed|hopeless|can't go on|nothing matters)\b/i.test(text),
    needsContainment: base.intensity >= 0.72,
    needsStabilization: ["fear","sadness","anger"].includes(base.primary) && base.intensity >= 0.65,
    vulnerable: ["fear","sadness"].includes(base.primary)
  };
  ...
  return {
    ...
    psychology: {
      interpretation: base.primary === "sadness" ? "possible emotional heaviness with withdrawal risk" : base.primary === "fear" ? "possible activation && uncertainty requiring containment" : "stable informational posture",
    ...
    support: {
      tone: base.primary === "sadness" ? "gentle" : base.primary === "fear" ? "steady" : "clear",
      followup: base.primary !== "neutral",
      advice_level: base.intensity >= 0.75 ? "low" : "medium",
      timing_profile: _safeObj(nuanceDef.timing_profile)
    },
    guard: { diagnosis_block: true, safe_to_continue: true, escalation_required: false },
    marion_handoff: {
      locked_emotion: {
        primaryEmotion: base.primary === "sadness" ? "sadness" : base.primary,
