...
const VERSION = "nyx.contentDataRegistry/1.1-phase3-collection-key";
...
function collectionKey(value){const v=object(value),declared=String(object(v.contentReadiness).collectionKey||"").trim();return /^[a-z][a-z0-9_-]{0,63}$/i.test(declared)?declared:""}
function rows(value){if(Array.isArray(value))return value;const v=object(value),nested=object(v.data),declared=collectionKey(v),keys=[declared,"items","slots","programs","entries","schedule","channels","movies","cartoons","classics","languages","apps"].filter((key,index,list)=>key&&list.indexOf(key)===index);for(const key of keys){if(Array.isArray(v[key]))return v[key];if(Array.isArray(nested[key]))return nested[key]}return[]}
function inspect(name){const definition=DEFINITIONS[name];if(!definition)return{source:name,status:"invalid",reason:"unknown_source"};const found=resolveSource(name);if(!found)return{source:name,required:definition.required,status:"pending",reason:"file_not_supplied",expectedPaths:definition.paths};try{const raw=fs.readFileSync(found.file,"utf8"),data=JSON.parse(raw),list=rows(data),key=collectionKey(data)||(["channels","slots","items"].find(k=>Array.isArray(object(data)[k]))||""),valid=!!(data&&typeof data==="object"&&list.length);return{source:name,required:definition.required,status:valid?"ready":"invalid",reason:valid?"":"empty_or_unrecognized_catalog",relativePath:found.relativePath,collectionKey:key,itemCount:list.length,bytes:Buffer.byteLength(raw),checksum:crypto.createHash("sha256").update(raw).digest("hex").slice(0,16),updatedAt:String(object(data).updatedAt||object(data).lastStructuredAt||"")}}catch(error){return{source:name,required:definition.required,status:"invalid",reason:error instanceof SyntaxError?"json_parse_failed":"read_failed",relativePath:found.relativePath}}
}
...
function catalog(name){const report=inspect(name);if(report.status!=="ready")return{...report,contract:"nyx.publicCatalog/1.0",items:[]};const found=resolveSource(name),data=JSON.parse(fs.readFileSync(found.file,"utf8"));return{contract:"nyx.publicCatalog/1.0",version:VERSION,source:name,status:"ready",items:rows(data),meta:{collectionKey:report.collectionKey,itemCount:report.itemCount,checksum:report.checksum,updatedAt:report.updatedAt}}}
...
module.exports={VERSION,DEFINITIONS,resolveSource,collectionKey,rows,inspect,status,catalog,releaseValidation};
