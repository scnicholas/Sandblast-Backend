 ...
 *
 * Build: nyx-bulletproof-v1.40-2025-12-18
 */
...
try {
  ...
  // safe fallback (domain stays general unless heuristics say music)
}
...
const BUILD_TAG = "nyx-bulletproof-v1.40-2025-12-18";
...
const RL_MAX = Number(process.env.RATE_LIMIT_PER_MIN || 120); // generous (2 req/sec)
...
// ------------------------------------------------------------
//
// We support 3 levels of continuity:
// 1) meta.sessionId (preferred, stable)
// 2) Fingerprint -> sessionId mapping (server-side) if widget drops sessionId
// 3) Hard fallback fingerprint session (still stable enough per user)
//
// This is what actually kills “it still loops” when the widget is inconsistent.
//
const SESS = new Map(); // sid -> session
const FP = new Map();   // fingerprint -> { sid, expiresAt }
const SESS_TTL_MS = 6 * 60 * 60 * 1000;   // 6h
const FP_TTL_MS = 45 * 60 * 1000;         // 45m for dropped-sessionId recovery
...
function fingerprint(req) {
  // Keep it stable but not too unique; good enough for continuity
...
function resolveSessionId(req, incomingMeta) {
  ...
  // Create a new sessionId and map it to fingerprint
...
function extractYear(text) {
  if (musicKB && typeof musicKB.extractYear === "function") {
    try { return musicKB.extractYear(text); } catch {}
  }
...
// sanitize labels (prevents “Cultural thread: Cultural thread:”)
...
function safePickBest(fields) {
  ...
  // priority: artist+title+year > artist+year > title > artist
...
// ------------------------------------------------------------
// Response helper (always returns stable sessionId)
// ------------------------------------------------------------
function send(res, sid, sess, stepName, reply, advance) {
  const outMeta = {
    sessionId: sid,
...
app.post("/api/sandblast-gpt", (req, res) => {
  ...
  // Merge in meta.laneDetail/mem if provided (but DO NOT allow it to wipe server state)
  // The server is authoritative to prevent widget-side state loss from causing loops.
  ...
  // lane detection (server authoritative)
  ...
  if (isGreeting) {
    ...
    const reply =
      "Good to have you. Choose a lane: Music history, Sandblast TV, News Canada, or Sponsors.";
    ...
    LAST_DEBUG = { at: new Date().toISOString(), sid, stepName, lane: sess.currentLane, message: userMessage };
    return send(res, sid, sess, stepName, reply, true);
  }
  ...
  if (domain === "music_history") {
    ...
    if (!sess.laneDetail.artist) {
      // quick aliases you demanded earlier + KB detection
      const n = msgN;
    ...
    if (bestFull) {
      ...
      LAST_DEBUG = { at: new Date().toISOString(), sid, stepName, slots, message: userMessage };
      return send(res, sid, sess, stepName, reply, true);
    }
    ...
    if (slots.artist && slots.year && slots.title && !bestFull) {
      ...
      // Find likely year for the provided title (title-only)
    ...
    // Artist+year locked but no title: absolute anti-loop escalation
    if (slots.artist && slots.year && !slots.title) {
      ...
      // If we are repeating OR the user asked #1 intent, force-anchor immediately
      ...
      // Ask once (only once) for title
      sess._lastStepName = askStep;
      sess._lastPromptSig = askSig;
      saveSession(sid, sess);

      LAST_DEBUG = { at: new Date().toISOString(), sid, step: askStep, slots, message: userMessage };
      return send(res, sid, sess, askStep, askReply, true);
    }
    ...
    if (slots.artist && !slots.year && !slots.title) {
      ...
      // If repeating, force progress with guidance (never same sentence twice)
      const finalReply = sess._repeatCount >= 1
        ? `We have the artist locked. Give me just one piece now: a year or a title.`
        : reply;
      ...
      LAST_DEBUG = { at: new Date().toISOString(), sid, stepName, slots, message: userMessage };
      return send(res, sid, sess, stepName, finalReply, true);
    }
    ...
    if (slots.year && !slots.artist && !slots.title) {
      ...
      const finalReply = sess._repeatCount >= 1
        ? `We already have the year. Now I need either the artist or the title.`
        : reply;
    ...
    // Default prompt
    {
      ...
      const finalReply = sess._repeatCount >= 1
        ? `Give me either: artist + year, or a song title.`
        : reply;
      ...
      LAST_DEBUG = { at: new Date().toISOString(), sid, stepName, slots, message: userMessage };
      return send(res, sid, sess, stepName, finalReply, false);
    }
  }
