...
app.post("/api/sandblast-gpt", async (req, res) => {
  try {
    ...
    // Wrap with Nyx tone + realism + proof point + next action
    const wrapped = nyxPersonality.wrapWithNyxTone(basePayload, userMessage, meta);
...
app.post("/api/tts", async (req, res) => {
  try {
    ...
    if (!OPENAI_API_KEY) {
      return res.status(500).json({ error: "TTS not configured: OPENAI_API_KEY missing." });
    }
