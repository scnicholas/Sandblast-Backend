...
function cleanReplyForUser(v) {
  ...
  return hasUserVisibleDebugLeak(t) || isPublicControlPolicyLeak(t) ? "" : t;
}
...
function buildLingoLinkPublicAnswerFromPacket(packet) {
  ...
  const source = [
    src.userText, src.message, src.text, src.query, src.inputText, src.originalText,
    payload.userText, payload.message, payload.text, payload.query, payload.originalText,
    finalEnvelope.userText, finalEnvelope.message, finalEnvelope.text,
    input.userText, input.message, input.text, input.query
  ].map(cleanText).join(" ");
  if (/\b(?:lingolink|lingo link|language sphere|languagesphere)\b/i.test(source) &&
      /\b(?:explain|what|does|do|clear sentence|one sentence|multilingual|language)\b/i.test(source)) {
...
function buildPublicChatResponse(packet, reply) {
  ...
  if (!safeReply) safeReply = buildLingoLinkPublicAnswerFromPacket(src);
...
async function callComposeMarionResponseRuntime(input, upstream) {
  ...
  try {
    let out = null;
    if (typeof composeMarionResponseMod.composeMarionResponse === "function") out = await composeMarionResponseMod.composeMarionResponse(packet);
    else if (typeof composeMarionResponseMod.default === "function") out = await composeMarionResponseMod.default(packet);
    else if (typeof composeMarionResponseMod === "function") out = await composeMarionResponseMod(packet);
