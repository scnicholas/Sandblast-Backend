...
app.post('/api/sandblast-gpt', async (req, res) => {
  try {
    ...
    const replyText = userMessage
      ? `Backend brain received: "${userMessage}". This is the temporary logic layer before full OpenAI integration.`
...
app.post('/api/tts', async (req, res) => {
  ...
  try {
    ...
    console.error('Error calling ElevenLabs TTS:', error.response?.data || error.message);
    return res.status(500).json({
      success: false,
