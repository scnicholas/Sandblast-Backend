...
describe("FinanceScenarioCalculator", () => {
  const mod = loadModule([
    "../../../Data/marion/runtime/finance/layer06_analysis_execution/FinanceScenarioCalculator.js",
    "../../../FinanceScenarioCalculator.js",
    "../../../finance/FinanceScenarioCalculator.js",
    "../../../finance/layer06_analysis_execution/FinanceScenarioCalculator.js",
    "../../../Data/finance/layer06_analysis_execution/FinanceScenarioCalculator.js",
    "../../../Data/Domains/finance/layer06_analysis_execution/FinanceScenarioCalculator.js",
    "../../../Domains/finance/layer06_analysis_execution/FinanceScenarioCalculator.js"
  ]);

  const FinanceScenarioCalculator = pickExport(mod, ["FinanceScenarioCalculator", "ScenarioCalculator"]);

  test("constructs without throwing", () => {
    expect(() => new FinanceScenarioCalculator()).not.toThrow();
  });

  test("calculates cash runway when cash and monthly burn are present", () => {
    const calculator = new FinanceScenarioCalculator();
    const envelope = sampleLayer05Envelope();

    const result = callFirst(calculator, ["calculate", "calculateScenarios", "run", "execute", "process"], {
      scenarioFrame: envelope.scenarioFrame,
      normalizedMetrics: envelope.normalizedMetrics,
      assumptions: envelope.assumptions
    });

    const runway = result.scenarioResults.scenarioOutputs.find((item) => item.scenarioType === "cash_runway");
    expect(runway).toBeTruthy();
    expect(runway.executionStatus).toBe("calculated");
    expect(runway.value).toBe(6);
    expect(runway.unit).toBe("months");
  });

  test("calculates revenue shock when baseline revenue and decline are present", () => {
    const calculator = new FinanceScenarioCalculator();
    const envelope = sampleLayer05Envelope();
    ...
    const shock = result.scenarioResults.scenarioOutputs.find((item) => item.scenarioType === "revenue_shock");
    expect(shock).toBeTruthy();
    expect(shock.executionStatus).toBe("calculated");
    expect(shock.outputs.estimatedMonthlyRevenueLoss).toBe(300);
  });

  test("returns missing state when scenario inputs are absent", () => {
    const calculator = new FinanceScenarioCalculator();

    const result = callFirst(calculator, ["calculate", "calculateScenarios", "run", "execute", "process"], {
      scenarioFrame: { scenarioRequired: true, scenarioTypes: ["cash_runway_survival"] },
      normalizedMetrics: []
    });

    const strings = flattenStrings(result).join(" ");
    expect(strings).toContain("missing_required_inputs");
    expect(strings).toContain("cash_on_hand");
  });

  test("preserves assumption trace", () => {
    const calculator = new FinanceScenarioCalculator();
    const envelope = sampleLayer05Envelope();
    ...
    expect(result.scenarioResults.assumptionTrace.length).toBeGreaterThan(0);
    expect(flattenStrings(result.scenarioResults.assumptionTrace).join(" ")).toContain("assume");
  });

  test("output is JSON-serializable", () => {
    const calculator = new FinanceScenarioCalculator();
    const result = callFirst(calculator, ["calculate", "calculateScenarios", "run", "execute", "process"], sampleLayer05Envelope());
