...
describe("FinancePeerComparator", () => {
  const mod = loadModule([
    "../../../Data/marion/runtime/finance/layer06_analysis_execution/FinancePeerComparator.js",
    "../../../FinancePeerComparator.js",
    "../../../finance/FinancePeerComparator.js",
    "../../../finance/layer06_analysis_execution/FinancePeerComparator.js",
    "../../../Data/finance/layer06_analysis_execution/FinancePeerComparator.js",
    "../../../Data/Domains/finance/layer06_analysis_execution/FinancePeerComparator.js",
    "../../../Domains/finance/layer06_analysis_execution/FinancePeerComparator.js"
  ]);

  const FinancePeerComparator = pickExport(mod, ["FinancePeerComparator", "PeerComparator"]);

  test("constructs without throwing", () => {
    expect(() => new FinancePeerComparator()).not.toThrow();
  });

  test("builds a peer comparison matrix across companies", () => {
    const comparator = new FinancePeerComparator();
    const envelope = sampleLayer05Envelope();

    const result = callFirst(comparator, ["compare", "comparePeers", "run", "execute", "process"], {
      ...
      normalizedEntities: envelope.normalizedEntities,
      normalizedPeriods: envelope.normalizedPeriods
    });

    expect(result).toBeTruthy();
    expect(result.peerComparison.peerRequired).toBe(true);
    expect(result.peerComparison.comparisonMatrix.length).toBeGreaterThanOrEqual(2);
  });

  test("calculates a leader and laggard when peer numeric values are available", () => {
    const comparator = new FinancePeerComparator();
    const envelope = sampleLayer05Envelope();
    ...
    const revenueComparison = result.peerComparison.metricComparisons.find((item) => item.canonicalMetric === "revenue");
    expect(revenueComparison).toBeTruthy();
    expect(revenueComparison.executionStatus).toBe("comparison_calculated");
    expect(revenueComparison.leader.canonicalName.toLowerCase()).toContain("microsoft");
  });

  test("returns partial comparison when only one peer has numeric values", () => {
    const comparator = new FinancePeerComparator();

    const result = callFirst(comparator, ["compare", "comparePeers", "run", "execute", "process"], {
      queryText: "Compare Apple and Microsoft revenue.",
      analysisPlan: { primaryRoute: "peer_comparison", secondaryRoutes: [] },
      normalizedEntities: {
        companies: [
          { entityId: "a", canonicalName: "Apple Inc.", ticker: "AAPL" },
          { entityId: "m", canonicalName: "Microsoft Corporation", ticker: "MSFT" }
        ]
      },
      normalizedMetrics: [
        { normalizedMetricId: "m1", canonicalMetric: "revenue", value: 1000, entityId: "a" }
      ]
    });

    const strings = flattenStrings(result).join(" ");
    expect(strings).toContain("insufficient_peer_numeric_values");
  });

  test("output is JSON-serializable", () => {
    const comparator = new FinancePeerComparator();
    const result = callFirst(comparator, ["compare", "comparePeers", "run", "execute", "process"], sampleLayer05Envelope());
