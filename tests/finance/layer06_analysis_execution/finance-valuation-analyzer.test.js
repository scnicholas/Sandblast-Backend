...
describe("FinanceValuationAnalyzer", () => {
  const mod = loadModule([
    "../../../Data/marion/runtime/finance/layer06_analysis_execution/FinanceValuationAnalyzer.js",
    "../../../FinanceValuationAnalyzer.js",
    "../../../finance/FinanceValuationAnalyzer.js",
    "../../../finance/layer06_analysis_execution/FinanceValuationAnalyzer.js",
    "../../../Data/finance/layer06_analysis_execution/FinanceValuationAnalyzer.js",
    "../../../Data/Domains/finance/layer06_analysis_execution/FinanceValuationAnalyzer.js",
    "../../../Domains/finance/layer06_analysis_execution/FinanceValuationAnalyzer.js"
  ]);

  const FinanceValuationAnalyzer = pickExport(mod, ["FinanceValuationAnalyzer", "ValuationAnalyzer"]);

  test("constructs without throwing", () => {
    expect(() => new FinanceValuationAnalyzer()).not.toThrow();
  });

  test("detects valuation requirement from analysis route", () => {
    const analyzer = new FinanceValuationAnalyzer();
    const envelope = sampleLayer05Envelope({ analysisPlan: { primaryRoute: "valuation_analysis", secondaryRoutes: [] } });

    const result = callFirst(analyzer, ["analyze", "analyzeValuation", "run", "execute", "process"], {
      queryText: "Analyze valuation and PE ratio.",
    ...
    expect(result.valuationResults.valuationRequired).toBe(true);
  });

  test("uses direct PE ratio value when available", () => {
    const analyzer = new FinanceValuationAnalyzer();
    const envelope = sampleLayer05Envelope();

    const result = callFirst(analyzer, ["analyze", "analyzeValuation", "run", "execute", "process"], {
      queryText: "Analyze PE ratio valuation.",
      analysisPlan: { primaryRoute: "valuation_analysis", secondaryRoutes: [] },
      normalizedMetrics: envelope.normalizedMetrics,
      evidenceRequirements: envelope.evidenceRequirements
    });

    const pe = result.valuationResults.valuationChecks.find((item) => item.valuationType === "price_earnings_ratio");
    expect(pe).toBeTruthy();
    expect(pe.executionStatus).toBe("direct_value_available");
    expect(pe.value).toBe(30);
  });

  test("returns missing values when valuation inputs are incomplete", () => {
    const analyzer = new FinanceValuationAnalyzer();

    const result = callFirst(analyzer, ["analyze", "analyzeValuation", "run", "execute", "process"], {
      queryText: "Calculate PE ratio.",
      analysisPlan: { primaryRoute: "valuation_analysis", secondaryRoutes: [] },
      normalizedMetrics: [{ normalizedMetricId: "eps", canonicalMetric: "eps", value: 5 }],
      evidenceRequirements: []
    });

    const strings = flattenStrings(result).join(" ");
    expect(strings).toContain("missing_values");
    expect(strings).toContain("market_price");
  });

  test("output is JSON-serializable", () => {
    const analyzer = new FinanceValuationAnalyzer();
    const result = callFirst(analyzer, ["analyze", "analyzeValuation", "run", "execute", "process"], sampleLayer05Envelope());
