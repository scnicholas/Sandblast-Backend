...
describe("FinanceEvidenceBindingEnvelope", () => {
  const mod = loadModule([
    "../../../Data/marion/runtime/finance/layer07_evidence_binding/FinanceEvidenceBindingEnvelope.js",
    "../../../FinanceEvidenceBindingEnvelope.js",
    "../../../finance/FinanceEvidenceBindingEnvelope.js",
    "../../../finance/layer07_evidence_binding/FinanceEvidenceBindingEnvelope.js",
    "../../../Data/finance/layer07_evidence_binding/FinanceEvidenceBindingEnvelope.js",
    "../../../Data/Domains/finance/layer07_evidence_binding/FinanceEvidenceBindingEnvelope.js",
    "../../../Domains/finance/layer07_evidence_binding/FinanceEvidenceBindingEnvelope.js"
  ]);

  const FinanceEvidenceBindingEnvelope = pickExport(mod, [
    "FinanceEvidenceBindingEnvelope",
    "EvidenceBindingEnvelope"
  ]);
  ...
  function callEnvelopeFactory(EnvelopeClass, payload) {
    ...
    const instance = new EnvelopeClass(payload);
    return instance;
  }

  test("constructs or creates without throwing", () => {
    expect(() => {
      callEnvelopeFactory(FinanceEvidenceBindingEnvelope, {
        originalQuery: "Bind evidence.",
        boundEvidence: { evidenceBoundResults: [] }
      });
    }).not.toThrow();
  });

  test("creates stable Layer 07 trace metadata and envelope fields", () => {
    const envelope = callEnvelopeFactory(FinanceEvidenceBindingEnvelope, {
      requestId: "evidence-request-001",
      originalQuery: "Bind evidence.",
      normalizedMetrics: sampleLayer06Envelope().normalizedMetrics,
      normalizedSources: sampleLayer06Envelope().normalizedSources,
      boundEvidence: {
    ...
    expect(envelope.runtimeLayer).toBe("layer07_evidence_binding");

    expectDeepKey(envelope, ["requestId", "traceId", "schemaVersion", "version"]);
    expectDeepKey(envelope, ["boundEvidence", "resultSupportScores", "verificationGaps", "evidenceReadiness"]);
    expectDeepKey(envelope, ["nextLayerHandoff", "diagnostics"]);
  });

  test("marks evidence as blocked when blocking gaps exist", () => {
    const envelope = callEnvelopeFactory(FinanceEvidenceBindingEnvelope, {
      originalQuery: "Bind evidence.",
      boundEvidence: {
        evidenceBoundResults: [
          { resultId: "valuation_pe", resultType: "valuation", resultName: "price_earnings_ratio" }
        ]
      },
      resultSupportScores: [
        {
    ...
    expect(envelope.evidenceReadiness.status).toBe("blocked_pending_evidence");
    expect(envelope.diagnostics.ok).toBe(false);
    expect(envelope.nextLayerHandoff.requiresEvidenceVerification).toBe(true);
  });
  ...
  test("validates required envelope shape", () => {
    ...
    if (typeof FinanceEvidenceBindingEnvelope.validate === "function") {
      const validation = FinanceEvidenceBindingEnvelope.validate(envelope);
