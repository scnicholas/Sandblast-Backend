...
describe("FinanceEvidenceBindingController", () => {
  const mod = loadModule([
    "../../../Data/marion/runtime/finance/layer07_evidence_binding/FinanceEvidenceBindingController.js",
    "../../../FinanceEvidenceBindingController.js",
    "../../../finance/FinanceEvidenceBindingController.js",
    "../../../finance/layer07_evidence_binding/FinanceEvidenceBindingController.js",
    "../../../Data/finance/layer07_evidence_binding/FinanceEvidenceBindingController.js",
    "../../../Data/Domains/finance/layer07_evidence_binding/FinanceEvidenceBindingController.js",
    "../../../Domains/finance/layer07_evidence_binding/FinanceEvidenceBindingController.js"
  ]);

  const FinanceEvidenceBindingController = pickExport(mod, [
    "FinanceEvidenceBindingController",
    "EvidenceBindingController"
  ]);

  test("constructs without throwing", () => {
    expect(() => new FinanceEvidenceBindingController()).not.toThrow();
  });

  test("consumes a Layer 06 execution envelope and returns a Layer 07 evidence envelope", () => {
    const controller = new FinanceEvidenceBindingController();
    const result = callFirst(
      controller,
      ["bind", "run", "execute", "process"],
      sampleLayer06Envelope()
    );

    expect(result).toBeTruthy();
    expect(result.domain).toBe("finance");
    expect(result.runtimeLayer).toBe("layer07_evidence_binding");

    expectDeepKey(result, ["sourceRequirementMap", "boundEvidence", "resultSupportScores", "verificationGaps"]);
    expectDeepKey(result, ["evidenceReadiness", "nextLayerHandoff", "diagnostics"]);
  });

  test("binds calculated outputs to sources and scores claim support", () => {
    const controller = new FinanceEvidenceBindingController();
    const result = callFirst(
      controller,
      ["bind", "run", "execute", "process"],
    ...
    expect(result.boundEvidence.evidenceBoundResults.length).toBeGreaterThan(0);
    expect(result.resultSupportScores.length).toBeGreaterThan(0);
    ...
    expect(strings).toContain("support");
  });

  test("propagates evidence caveats and verification gaps", () => {
    const controller = new FinanceEvidenceBindingController();
    const input = sampleLayer06Envelope({
      normalizedSources: [
        {
          sourceId: "unknown_source",
          sourceType: "external_source",
          sourceLabel: "unknown_blog",
      ...
      normalizedMetrics: [
        {
          normalizedMetricId: "orphan_metric",
          canonicalMetric: "revenue",
          value: 1000,
          period: "FY2024",
          verificationRequired: true
      ...
      ratioResults: {
        calculatedRatios: [
          {
            ratioExecutionId: "orphan_ratio",
            ratioType: "gross_margin",
            value: 40,
            unit: "percent",
            executionStatus: "calculated",
            sourceMetricIds: ["orphan_metric"]
          }
        ],
        directRatios: []
      },
    ...
    const strings = flattenStrings(result).join(" ");
    expect(strings).toContain("requires_verification");
    expect(result.verificationGaps.length).toBeGreaterThan(0);
  });

  test("preserves upstream execution and normalized lineage", () => {
    const controller = new FinanceEvidenceBindingController();
    const result = callFirst(
      controller,
      ["bind", "run", "execute", "process"],
    ...
    expect(result.normalizedSources.length).toBeGreaterThan(0);
    expect(result.executionQuality.status).toBe("executed");

    const strings = flattenStrings(result).join(" ");
    expect(strings).toContain("fy2024");
    expect(strings).toContain("annual_report");
  });
