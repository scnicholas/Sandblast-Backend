...
describe("FinanceClaimSupportScorer", () => {
  const mod = loadModule([
    "../../../Data/marion/runtime/finance/layer07_evidence_binding/FinanceClaimSupportScorer.js",
    "../../../FinanceClaimSupportScorer.js",
    "../../../finance/FinanceClaimSupportScorer.js",
    "../../../finance/layer07_evidence_binding/FinanceClaimSupportScorer.js",
    "../../../Data/finance/layer07_evidence_binding/FinanceClaimSupportScorer.js",
    "../../../Data/Domains/finance/layer07_evidence_binding/FinanceClaimSupportScorer.js",
    "../../../Domains/finance/layer07_evidence_binding/FinanceClaimSupportScorer.js"
  ]);

  const FinanceClaimSupportScorer = pickExport(mod, [
    "FinanceClaimSupportScorer",
    "ClaimSupportScorer"
  ]);

  test("constructs without throwing", () => {
    expect(() => new FinanceClaimSupportScorer()).not.toThrow();
  });

  test("scores strong or adequate support when metric and primary-source lineage exists", () => {
    const scorer = new FinanceClaimSupportScorer();

    const result = callFirst(
      scorer,
      ["score", "scoreClaims", "run", "execute", "process"],
      {
        boundEvidence: {
          evidenceBoundResults: [
            {
              ...
              requiresVerification: false,
              linkedMetrics: [
                { normalizedMetricId: "m1", canonicalMetric: "gross_profit" },
                { normalizedMetricId: "m2", canonicalMetric: "revenue" }
              ],
              linkedSources: [
                {
                  sourceId: "src_annual_report",
    ...
    expect(result.resultSupportScores.length).toBe(1);
    expect(["strong_support", "adequate_support"]).toContain(result.resultSupportScores[0].supportStatus);
    expect(result.resultSupportScores[0].canUseInFinalSynthesis).toBe(true);
  });

  test("marks unsupported results as unusable for final synthesis", () => {
    const scorer = new FinanceClaimSupportScorer();
    ...
    expect(result.resultSupportScores[0].supportStatus).toBe("unsupported");
    expect(result.resultSupportScores[0].canUseInFinalSynthesis).toBe(false);
  });

  test("caveats scenario results that depend on unconfirmed assumptions", () => {
    const scorer = new FinanceClaimSupportScorer();
          ...
          evidenceBoundResults: [
            {
              ...
              sourceLineageComplete: true,
              requiresVerification: false,
              linkedMetrics: [{ normalizedMetricId: "m_cash" }],
              linkedSources: [{ authorityClass: "user_supplied" }],
              attachedRequirements: []
            }
          ]
        },
        assumptions: [{ requiresConfirmation: true }],
        riskFlags: []
      }
    );

    expect(result.resultSupportScores[0].shouldCaveat).toBe(true);
  });
