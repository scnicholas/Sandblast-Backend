...
describe("FinanceResultEvidenceBinder", () => {
  const mod = loadModule([
    "../../../Data/marion/runtime/finance/layer07_evidence_binding/FinanceResultEvidenceBinder.js",
    "../../../FinanceResultEvidenceBinder.js",
    "../../../finance/FinanceResultEvidenceBinder.js",
    "../../../finance/layer07_evidence_binding/FinanceResultEvidenceBinder.js",
    "../../../Data/finance/layer07_evidence_binding/FinanceResultEvidenceBinder.js",
    "../../../Data/Domains/finance/layer07_evidence_binding/FinanceResultEvidenceBinder.js",
    "../../../Domains/finance/layer07_evidence_binding/FinanceResultEvidenceBinder.js"
  ]);

  const FinanceResultEvidenceBinder = pickExport(mod, [
    "FinanceResultEvidenceBinder",
    "ResultEvidenceBinder"
  ]);

  test("constructs without throwing", () => {
    expect(() => new FinanceResultEvidenceBinder()).not.toThrow();
  });

  test("binds ratio, trend, peer, scenario, and valuation results to metric/source lineage", () => {
    ...
    const binder = new FinanceResultEvidenceBinder();
    const result = callFirst(
      binder,
      ["bind", "bindEvidence", "run", "execute", "process"],
      {
        ...envelope,
    ...
    expect(result.boundEvidence.evidenceBoundResults.length).toBeGreaterThanOrEqual(5);
    ...
    expect(strings).toContain("bound");
  });

  test("attaches mapped requirements to applicable bound results", () => {
    const envelope = sampleLayer06Envelope();
    const sourceRequirementMap = [
      {
        requirementCode: "official_financial_statement_source",
    ...
    const gross = result.boundEvidence.evidenceBoundResults.find((item) => item.resultName === "gross_margin");
    expect(gross).toBeTruthy();
    expect(gross.attachedRequirements.length).toBeGreaterThan(0);
    expect(gross.linkedMetricIds).toContain("m_gross_profit_a_2024");
    expect(gross.linkedSources.length).toBeGreaterThan(0);
  });

  test("returns unsupported status when lineage and sources are absent", () => {
    const binder = new FinanceResultEvidenceBinder();
    const envelope = sampleLayer06Envelope({
      normalizedMetrics: [],
      normalizedSources: [],
      ratioResults: {
        calculatedRatios: [
          {
            ratioExecutionId: "ratio_without_lineage",
            ratioType: "gross_margin",
            value: 40,
            unit: "percent",
            executionStatus: "calculated",
    ...
    const strings = flattenStrings(result).join(" ");
    expect(strings).toContain("unsupported");
  });
