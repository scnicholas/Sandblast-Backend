...
describe("FinanceSourceRequirementMapper", () => {
  const mod = loadModule([
    "../../../Data/marion/runtime/finance/layer07_evidence_binding/FinanceSourceRequirementMapper.js",
    "../../../FinanceSourceRequirementMapper.js",
    "../../../finance/FinanceSourceRequirementMapper.js",
    "../../../finance/layer07_evidence_binding/FinanceSourceRequirementMapper.js",
    "../../../Data/finance/layer07_evidence_binding/FinanceSourceRequirementMapper.js",
    "../../../Data/Domains/finance/layer07_evidence_binding/FinanceSourceRequirementMapper.js",
    "../../../Domains/finance/layer07_evidence_binding/FinanceSourceRequirementMapper.js"
  ]);

  const FinanceSourceRequirementMapper = pickExport(mod, [
    "FinanceSourceRequirementMapper",
    "SourceRequirementMapper"
  ]);

  test("constructs without throwing", () => {
    expect(() => new FinanceSourceRequirementMapper()).not.toThrow();
  });

  test("maps official financial statement requirements to ratio/trend/peer results", () => {
    const mapper = new FinanceSourceRequirementMapper();
    const result = callFirst(
      mapper,
      ["map", "mapRequirements", "run", "execute", "process"],
      sampleLayer06Envelope()
    );

    expect(result).toBeTruthy();
    expect(Array.isArray(result.sourceRequirementMap)).toBe(true);

    const statementRow = result.sourceRequirementMap.find(
      (row) => row.requirementCode === "official_financial_statement_source"
    );

    expect(statementRow).toBeTruthy();
    expect(statementRow.appliesToResultTypes).toContain("ratio");
    expect(statementRow.appliesToResultTypes).toContain("trend");
    expect(statementRow.appliesToResultTypes).toContain("peer_comparison");
  });

  test("maps valuation and scenario requirements to correct result classes", () => {
    const mapper = new FinanceSourceRequirementMapper();
    const result = callFirst(
      mapper,
      ["map", "mapRequirements", "run", "execute", "process"],
    ...
    expect(strings).toContain("valuation");
    expect(strings).toContain("scenario");
    expect(strings).toContain("price_earnings_ratio");
  });

  test("infers scenario assumption disclosure when scenario outputs exist", () => {
    const mapper = new FinanceSourceRequirementMapper();
    const envelope = sampleLayer06Envelope({
      evidenceRequirements: []
    });
    ...
    const strings = flattenStrings(result).join(" ");
    expect(strings).toContain("inferred_scenario_assumption_disclosure");
  });
