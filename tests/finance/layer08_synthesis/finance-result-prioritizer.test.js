...
describe("FinanceResultPrioritizer", () => {
  const mod = loadModule([
    "../../../Data/marion/runtime/finance/layer08_synthesis/FinanceResultPrioritizer.js",
    "../../../FinanceResultPrioritizer.js",
    "../../../finance/FinanceResultPrioritizer.js",
    "../../../finance/layer08_synthesis/FinanceResultPrioritizer.js",
    "../../../Data/finance/layer08_synthesis/FinanceResultPrioritizer.js",
    "../../../Data/Domains/finance/layer08_synthesis/FinanceResultPrioritizer.js",
    "../../../Domains/finance/layer08_synthesis/FinanceResultPrioritizer.js"
  ]);

  const FinanceResultPrioritizer = pickExport(mod, [
    "FinanceResultPrioritizer",
    "ResultPrioritizer"
  ]);

  test("constructs without throwing", () => {
    expect(() => new FinanceResultPrioritizer()).not.toThrow();
  });

  test("prioritizes supported evidence-bound results", () => {
    const prioritizer = new FinanceResultPrioritizer();
    const result = callFirst(
      prioritizer,
      ["prioritize", "prioritizeResults", "run", "execute", "process"],
      sampleLayer07Envelope()
    );

    expect(result).toBeTruthy();
    expect(Array.isArray(result.prioritizedResults)).toBe(true);
    expect(result.prioritizedResults.length).toBeGreaterThan(0);
    expect(result.prioritizedResults[0].priorityScore).toBeGreaterThanOrEqual(result.prioritizedResults[result.prioritizedResults.length - 1].priorityScore);
  });

  test("groups ratios, trends, comparisons, scenarios, valuations, and caveated items", () => {
    const prioritizer = new FinanceResultPrioritizer();
    const result = callFirst(
      prioritizer,
      ["prioritize", "prioritizeResults", "run", "execute", "process"],
    ...
    expect(result.resultGroups.calculations.length).toBeGreaterThan(0);
    expect(result.resultGroups.trends.length).toBeGreaterThan(0);
    expect(result.resultGroups.comparisons.length).toBeGreaterThan(0);
    expect(result.resultGroups.scenarios.length).toBeGreaterThan(0);
    expect(result.resultGroups.valuations.length).toBeGreaterThan(0);
    expect(result.resultGroups.caveated.length).toBeGreaterThan(0);
  });

  test("excludes unsupported results from synthesis", () => {
    const prioritizer = new FinanceResultPrioritizer();
    const envelope = sampleLayer07Envelope({
      boundEvidence: {
        evidenceBoundResults: [
          {
    ...
    expect(result.prioritizedResults.length).toBe(0);
    expect(result.diagnostics.warnings).toContain("all_results_excluded_from_synthesis");
  });
