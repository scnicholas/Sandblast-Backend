...
describe("FinanceSynthesisController", () => {
  const mod = loadModule([
    "../../../Data/marion/runtime/finance/layer08_synthesis/FinanceSynthesisController.js",
    "../../../FinanceSynthesisController.js",
    "../../../finance/FinanceSynthesisController.js",
    "../../../finance/layer08_synthesis/FinanceSynthesisController.js",
    "../../../Data/finance/layer08_synthesis/FinanceSynthesisController.js",
    "../../../Data/Domains/finance/layer08_synthesis/FinanceSynthesisController.js",
    "../../../Domains/finance/layer08_synthesis/FinanceSynthesisController.js"
  ]);

  const FinanceSynthesisController = pickExport(mod, [
    "FinanceSynthesisController",
    "SynthesisController"
  ]);

  test("constructs without throwing", () => {
    expect(() => new FinanceSynthesisController()).not.toThrow();
  });

  test("consumes a Layer 07 evidence-bound envelope and returns Layer 08 synthesis envelope", () => {
    const controller = new FinanceSynthesisController();
    const result = callFirst(
      controller,
      ["synthesize", "prepare", "run", "execute", "process"],
      sampleLayer07Envelope()
    );

    expect(result).toBeTruthy();
    expect(result.domain).toBe("finance");
    expect(result.runtimeLayer).toBe("layer08_synthesis");
    expectDeepKey(result, ["prioritizedResults", "caveats", "answerPlan", "answerSections", "finalAnswerPackage"]);
    expectDeepKey(result, ["synthesisReadiness", "nextLayerHandoff", "diagnostics"]);
  });

  test("prepares supported findings, caveats, evidence notes, and assumption notes", () => {
    const controller = new FinanceSynthesisController();
    const result = callFirst(
      controller,
      ["synthesize", "prepare", "run", "execute", "process"],
    ...
    expect(result.prioritizedResults.length).toBeGreaterThan(0);
    expect(result.caveats.length).toBeGreaterThan(0);
    expect(result.assumptionNotes.length).toBeGreaterThan(0);
    expect(result.answerSections.length).toBeGreaterThan(0);
  ...
  test("blocks or downgrades final rendering when evidence is blocked", () => {
    const controller = new FinanceSynthesisController();
    const result = callFirst(
      controller,
      ["synthesize", "prepare", "run", "execute", "process"],
    ...
    expect(result.nextLayerHandoff.requiresUserClarification || result.nextLayerHandoff.requiresEvidenceVerification).toBe(true);
    expect(result.synthesisReadiness.status).toBe("synthesis_blocked_or_partial");
  });

  test("preserves upstream normalized, evidence, and execution context", () => {
    const controller = new FinanceSynthesisController();
    const result = callFirst(
      controller,
      ["synthesize", "prepare", "run", "execute", "process"],
    ...
    expect(result.evidenceReadiness.status).toBe("needs_evidence_caveats");
  });
