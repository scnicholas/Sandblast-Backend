...
describe("FinanceResponseToneGuard", () => {
  const mod = loadModule(["../../../Data/marion/runtime/finance/layer09_final_response/FinanceResponseToneGuard.js", "../../../FinanceResponseToneGuard.js", "../../../finance/FinanceResponseToneGuard.js", "../../../finance/layer09_final_response/FinanceResponseToneGuard.js", "../../../Data/finance/layer09_final_response/FinanceResponseToneGuard.js", "../../../Data/Domains/finance/layer09_final_response/FinanceResponseToneGuard.js", "../../../Domains/finance/layer09_final_response/FinanceResponseToneGuard.js"]);
  const FinanceResponseToneGuard = pickExport(mod, ["FinanceResponseToneGuard", "ResponseToneGuard"]);

  test("constructs without throwing", () => {
    expect(() => new FinanceResponseToneGuard()).not.toThrow();
  });

  test("softens direct buy/sell advice and guarantee language", () => {
    const guard = new FinanceResponseToneGuard();
    const result = callFirst(guard, ["guard", "guardTone", "run", "execute", "process"], {
      finalResponseText: "You should buy this because it guarantees returns and will rise.",
      renderedSections: [],
      finalResponseBlocks: [],
      caveatsApplied: [],
      blockedClaims: []
    ...
    expect(text).toContain("additional verified evidence");
    expect(result.toneGuardFindings.length).toBeGreaterThan(0);
  });

  test("adds finance posture when response lacks evidence framing", () => {
    const guard = new FinanceResponseToneGuard();
    const result = callFirst(guard, ["guard", "guardTone", "run", "execute", "process"], {
      finalResponseText: "Gross margin is 40%.",
      renderedSections: [],
      finalResponseBlocks: []
    });

    expect(result.finalResponseText.toLowerCase()).toContain("based on the provided figures");
  });

  test("output is JSON-serializable", () => {
    const guard = new FinanceResponseToneGuard();
    const result = callFirst(guard, ["guard", "guardTone", "run", "execute", "process"], { finalResponseText: "Based on evidence, revenue increased." });
