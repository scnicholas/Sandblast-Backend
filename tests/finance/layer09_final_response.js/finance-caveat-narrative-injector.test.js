...
describe("FinanceCaveatNarrativeInjector", () => {
  const mod = loadModule(["../../../Data/marion/runtime/finance/layer09_final_response/FinanceCaveatNarrativeInjector.js", "../../../FinanceCaveatNarrativeInjector.js", "../../../finance/FinanceCaveatNarrativeInjector.js", "../../../finance/layer09_final_response/FinanceCaveatNarrativeInjector.js", "../../../Data/finance/layer09_final_response/FinanceCaveatNarrativeInjector.js", "../../../Data/Domains/finance/layer09_final_response/FinanceCaveatNarrativeInjector.js", "../../../Domains/finance/layer09_final_response/FinanceCaveatNarrativeInjector.js"]);
  const FinanceCaveatNarrativeInjector = pickExport(mod, ["FinanceCaveatNarrativeInjector", "CaveatNarrativeInjector"]);

  test("constructs without throwing", () => {
    expect(() => new FinanceCaveatNarrativeInjector()).not.toThrow();
  });

  test("injects caveat text into the final response", () => {
    const injector = new FinanceCaveatNarrativeInjector();
    const result = callFirst(injector, ["inject", "injectCaveats", "run", "execute", "process"], {
      finalResponseText: "Based on the provided figures, gross margin is 40%.",
      renderedSections: [],
      finalResponseBlocks: [],
      caveats: sampleLayer08Envelope().caveats,
      assumptionNotes: sampleLayer08Envelope().assumptionNotes,
      evidenceNotes: [],
      blockedItems: [],
      resultSupportScores: []
    });

    expect(result.finalResponseText.toLowerCase()).toContain("caveats");
    expect(result.caveatsApplied.length).toBeGreaterThan(0);
    expect(result.finalResponseBlocks.some((block) => block.blockType === "caveats_applied")).toBe(true);
  });

  test("collects blocked claims from blockedItems and unsupported results", () => {
    const injector = new FinanceCaveatNarrativeInjector();
    const result = callFirst(injector, ["inject", "injectCaveats", "run", "execute", "process"], {
      finalResponseText: "Based on the provided figures, valuation is not ready.",
      renderedSections: [],
      finalResponseBlocks: [],
      caveats: [],
      blockedItems: [{ blockedItemId: "b1", type: "result", code: "valuation_pe", reason: "Market price evidence is missing." }],
      resultSupportScores: [{ resultId: "orphan_ratio", supportStatus: "unsupported" }]
    });

    const strings = flattenStrings(result).join(" ");
    expect(strings).toContain("blocked or excluded items");
    expect(strings).toContain("market price evidence is missing");
    expect(result.blockedClaims.length).toBeGreaterThanOrEqual(2);
  });

  test("output is JSON-serializable", () => {
    const injector = new FinanceCaveatNarrativeInjector();
    const result = callFirst(injector, ["inject", "injectCaveats", "run", "execute", "process"], { finalResponseText: "Text." });
