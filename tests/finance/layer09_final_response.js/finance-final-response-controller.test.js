...
describe("FinanceFinalResponseController", () => {
  const mod = loadModule(["../../../Data/marion/runtime/finance/layer09_final_response/FinanceFinalResponseController.js", "../../../FinanceFinalResponseController.js", "../../../finance/FinanceFinalResponseController.js", "../../../finance/layer09_final_response/FinanceFinalResponseController.js", "../../../Data/finance/layer09_final_response/FinanceFinalResponseController.js", "../../../Data/Domains/finance/layer09_final_response/FinanceFinalResponseController.js", "../../../Domains/finance/layer09_final_response/FinanceFinalResponseController.js"]);
  const FinanceFinalResponseController = pickExport(mod, ["FinanceFinalResponseController", "FinalResponseController"]);

  test("constructs without throwing", () => {
    expect(() => new FinanceFinalResponseController()).not.toThrow();
  });

  test("consumes a Layer 08 synthesis envelope and returns a Layer 09 final response envelope", () => {
    const controller = new FinanceFinalResponseController();
    const result = callFirst(controller, ["render", "prepare", "run", "execute", "process"], sampleLayer08Envelope());

    expect(result).toBeTruthy();
    expect(result.domain).toBe("finance");
    expect(result.runtimeLayer).toBe("layer09_final_response");
    expect(typeof result.finalResponseText).toBe("string");
    expect(result.finalResponseText.length).toBeGreaterThan(0);
    expectDeepKey(result, ["renderedSections", "finalResponseBlocks", "responseReadiness", "nextLayerHandoff"]);
  });

  test("preserves finance findings, caveats, and assumption notes in the rendered response", () => {
    const controller = new FinanceFinalResponseController();
    const result = callFirst(controller, ["render", "prepare", "run", "execute", "process"], sampleLayer08Envelope());

    const strings = flattenStrings(result).join(" ");
    expect(strings).toContain("gross margin");
    expect(strings).toContain("cash runway");
    expect(strings).toContain("caveat");
    expect(strings).toContain("user-supplied");
  });

  test("softens unsafe finance recommendation language through the tone guard", () => {
    const input = sampleLayer08Envelope({
      answerSections: [
        {
          sectionId: "bad_section",
          sectionType: "safe_findings",
          title: "Supported Findings",
          order: 1,
          renderHint: "findings",
          includeInFinalAnswer: true,
          content: "You should buy this because it guarantees returns and will rise."
        }
      ],
      caveats: [],
      assumptionNotes: [],
    ...
    const controller = new FinanceFinalResponseController();
    const result = callFirst(controller, ["render", "prepare", "run", "execute", "process"], input);

    const text = result.finalResponseText.toLowerCase();
    expect(text).not.toContain("you should buy");
    expect(text).not.toContain("guarantees returns");
    expect(result.toneGuardFindings.length).toBeGreaterThan(0);
  });
