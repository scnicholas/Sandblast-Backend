...
describe("FinanceSectionRenderer", () => {
  const mod = loadModule(["../../../Data/marion/runtime/finance/layer09_final_response/FinanceSectionRenderer.js", "../../../FinanceSectionRenderer.js", "../../../finance/FinanceSectionRenderer.js", "../../../finance/layer09_final_response/FinanceSectionRenderer.js", "../../../Data/finance/layer09_final_response/FinanceSectionRenderer.js", "../../../Data/Domains/finance/layer09_final_response/FinanceSectionRenderer.js", "../../../Domains/finance/layer09_final_response/FinanceSectionRenderer.js"]);
  const FinanceSectionRenderer = pickExport(mod, ["FinanceSectionRenderer", "SectionRenderer"]);

  test("constructs without throwing", () => {
    expect(() => new FinanceSectionRenderer()).not.toThrow();
  });

  test("renders Layer 08 answer sections into response-ready blocks", () => {
    const renderer = new FinanceSectionRenderer();
    const result = callFirst(renderer, ["render", "renderSections", "run", "execute", "process"], sampleLayer08Envelope());

    expect(result).toBeTruthy();
    expect(Array.isArray(result.renderedSections)).toBe(true);
    expect(result.renderedSections.length).toBeGreaterThan(0);
    expect(Array.isArray(result.finalResponseBlocks)).toBe(true);

    const strings = flattenStrings(result).join(" ");
    expect(strings).toContain("supported findings");
    expect(strings).toContain("gross margin");
    expect(strings).toContain("cash runway");
  });

  test("preserves caveat sections in rendered text", () => {
    const renderer = new FinanceSectionRenderer();
    const result = callFirst(renderer, ["render", "renderSections", "run", "execute", "process"], sampleLayer08Envelope());
    const strings = flattenStrings(result).join(" ");

    expect(strings).toContain("caveats and limits");
    expect(strings).toContain("user-supplied cash and burn assumptions");
  });
