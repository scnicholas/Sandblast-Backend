...
function buildExpressStub() {
  const response = {
    ...
    set(name,value){ this.headers=this.headers||{}; this.headers[String(name).toLowerCase()]=String(value); return this; },
    setHeader(name,value){ return this.set(name,value); },
    header(name,value){ return this.set(name,value); },
    getHeader(name){ return this.headers&&this.headers[String(name).toLowerCase()]; },
    type(){ return this; },
  ...
  function express(){
    const routes=[];
    const app={locals:{},_routes:routes,_router:{stack:[]},disable(){return app;},set(){return app;},use(){return app;},listen(_p,cb){if(cb)cb();return{close(done){if(done)done();}};}};
    for(const method of ["get","post","options","head","all","delete","put","patch"]){
      app[method]=function(paths,...handlers){routes.push({method:method.toUpperCase(),paths:Array.isArray(paths)?paths:[paths],handlers});return app;};
    }
    return app;
  }
...
function loadIndex(){
  ...
  Module._load=function(request,parent,isMain){
    ...
    if(request==="compression"){const e=new Error("optional");e.code="MODULE_NOT_FOUND";throw e;}
  ...
  let backend;
  try{backend=require("../index.js");}
  finally{console.log=originalLog;Module._load=originalLoad;}
  return {backend,expressStub};
}
module.exports={loadIndex,buildExpressStub};
