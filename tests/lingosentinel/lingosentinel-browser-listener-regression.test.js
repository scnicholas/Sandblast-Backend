 ...
 *
 * VERSION_MARKER: LINGOSENTINEL_BROWSER_LISTENER_REGRESSION_V1
 *
 * Purpose:
...
const VERSION_MARKER = 'LINGOSENTINEL_BROWSER_LISTENER_REGRESSION_V1';
...
const TIMEOUT_MS = Number(process.env.LS_BROWSER_LISTENER_TIMEOUT_MS || 12000);
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = transport.request({
      ...
      headers: {
        ...
        'User-Agent': 'LingoSentinel-Browser-Listener-Regression/1.0'
      },
      timeout: TIMEOUT_MS
    }, (res) => {
      let raw = '';
...
function requireAblyOrSkip() {
  try {
    ...
    console.log('↷ skipped live subscribe/receive because ably npm package is not installed');
    console.log('  run npm install ably or set LS_REQUIRE_LIVE_RECEIVE=true to enforce it');
    return null;
  }
}
...
function waitForConnection(client) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      cleanup();
      reject(new Error('Timed out waiting for Ably browser-listener connection.'));
    }, TIMEOUT_MS);
...
function subscribeAndWait(channel) {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      cleanup();
      reject(new Error(`Timed out waiting for ${EVENT_NAME} on ${CHANNEL}.`));
    }, TIMEOUT_MS);
    ...
    function handler(message) {
      const text = JSON.stringify(message && message.data ? message.data : {});
      if (!/regression|sandbox|lingosentinel|LingoSentinel/i.test(text)) return;
      cleanup();
      resolve(message);
    }

    try {
      const result = channel.subscribe(EVENT_NAME, handler);
...
async function liveReceiveRegression() {
  ...
  try {
    ...
    const receivedPromise = subscribeAndWait(channel);
...
runAll([
  test('version marker is present', () => {
    assert.strictEqual(VERSION_MARKER, 'LINGOSENTINEL_BROWSER_LISTENER_REGRESSION_V1');
  }),
