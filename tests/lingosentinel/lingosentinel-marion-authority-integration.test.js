...
/**
 * LingoSentinel Marion Authority Integration Test
 *
 * Purpose:
 * Confirms the LingoSentinel Gateway produces Marion-safe payloads
 * that can be handed to Marion Bridge without letting LingoSentinel become authority.
 *
...
describe("LingoSentinel Marion Authority Integration", () => {
  test("Marion Bridge imports successfully", () => {
    expect(marionBridge).toBeDefined();
  });

  test("LingoSentinel Gateway payload keeps Marion as final authority", () => {
  ...
  test("LingoSentinel Gateway does not force translation authority", () => {
  ...
  test("LingoSentinel Gateway unknown-language fallback still preserves Marion authority", () => {
  ...
  test("Marion Bridge exposes callable handler or safe skip contract", () => {
     ...
     * but no callable request handler. That is not a LingoSentinel failure.
     * It tells us Marion Bridge needs a formal integration handler next.
     */
    expect(handler === null || typeof handler === "function").toBe(true);
  });

  test("Marion Bridge accepts LingoSentinel Gateway payload when callable", async () => {
    const handler = resolveBridgeHandler(marionBridge);
    const payload = buildMarionBridgePayload("  Bonjour,   comment ca va ?  ");
  ...
  test("Gateway package remains stable across multiple language cases", () => {
    ...
    for (const item of cases) {
      const result = runLingoSentinelGateway(item.input);
