...
/**
 * LingoSentinel Language Detection Regression Test
 *
 * Purpose:
...
describe("LingoSentinel Language Detection Regression", () => {
  ...
  test("detects English input", () => {
    ...
    expect(result.source).toBe("LingoSentinelLanguageDetect");
  });
  ...
  test("respects disabled detection config", () => {
    ...
    expect(result.supported).toBe(true);
    expect(result.reason).toBe("lingosentinel_detection_disabled");
  });
