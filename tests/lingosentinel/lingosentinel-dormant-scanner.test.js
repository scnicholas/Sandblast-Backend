...
/**
 * LingoSentinel Dormant Scanner Test
 *
 * Purpose:
 * Confirms LingoSentinel can produce a dormant scanner heartbeat and
...
const {
  ...
  SCANNER_VERSION
} = require("../../Data/marion/runtime/LingoSentinel/LingoSentinelDormantScanner");

describe("LingoSentinel Dormant Scanner", () => {
  test("module exports scanner functions", () => {
    ...
    expect(SCANNER_VERSION).toBe("nyx.lingosentinel.dormantScanner/0.1");
  });
  ...
  test("builds dormant scanner heartbeat", () => {
    ...
    expect(heartbeat.scanner).toBe("LingoSentinelDormantScanner");
    ...
    expect(heartbeat.authority.lingoSentinelAdvisoryOnly).toBe(true);
  ...
  test("runDormantScanner without input returns heartbeat only", () => {
    ...
    expect(result.scanner).toBe("LingoSentinelDormantScanner");
