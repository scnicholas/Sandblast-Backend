...
/**
 * LingoSentinel Scanner Gateway Carry Test
 *
 * Purpose:
 * Confirms LingoSentinelGateway carries dormant scanner metadata safely.
 *
 * This protects:
...
function assertScannerCarry(packet) {
  ...
  expect(packet.scannerHeartbeat.scanner).toBe("LingoSentinelDormantScanner");
  expect(packet.scannerHeartbeat.authority).toBeDefined();
  expect(packet.scannerHeartbeat.authority.finalAuthority).toBe("Marion");
  expect(packet.scannerHeartbeat.authority.lingoSentinelAdvisoryOnly).toBe(true);
  ...
  expect(packet.dormantScanner.authority.lingoSentinelAdvisoryOnly).toBe(true);
...
describe("LingoSentinel Scanner Gateway Carry", () => {
  ...
  test("scanner telemetry is included without replacing gateway telemetry", () => {
    ...
    expect(result.telemetry.source).toBe("LingoSentinelGateway");
    ...
    expect(serializedTelemetry).toContain("LingoSentinelGateway");
  ...
  test("Marion Bridge payload carries scanner metadata safely", () => {
    ...
    expect(payload.scannerHeartbeat.scanner).toBe("LingoSentinelDormantScanner");
    expect(payload.dormantScanner.source).toBe("LingoSentinelDormantScanner");
