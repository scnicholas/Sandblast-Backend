...
async function main() {
  ...
  assert.strictEqual(typeof bridge.marionPrivateRuntimeIdentityProjection, "function");

  const composerIdentity = composer.marionPrivateRuntimeIdentityProjection(
    { conversationId: "local-test-regression", privateAdminConversation: true, directMarionAdminInterface: true, verified: true },
    {},
    "trace-regression"
  );
  assert.strictEqual(composerIdentity.scope, "private_admin");
  assert.strictEqual(composerIdentity.audience, "owner");
  assert.strictEqual(composerIdentity.surfaceAgent, "Marion");
  assert.strictEqual(composerIdentity.publicSurfaceOnly, false);
  assert.strictEqual(composerIdentity.publicFallbackBlocked, true);
  assert.strictEqual(composerIdentity.memoryPartition, "private:admin:local-test-regression");
  assert.strictEqual(safety.isPrivateRuntimeIdentity(composerIdentity), true);

  const bridgeIdentity = bridge.marionPrivateRuntimeIdentityProjection(
    { conversationId: "local-test-regression", privateAdminConversation: true },
    { verified: true },
    "trace-regression"
  );
  assert.strictEqual(bridgeIdentity.memoryPartition, "private:admin:local-test-regression");
  ...
  assert.strictEqual(composed.meta.memoryPartition, "private:admin:local-test-regression");
  assert.strictEqual(composed.payload.memoryPartition, "private:admin:local-test-regression");
  assert.strictEqual(composed.finalEnvelope.memoryPartition, "private:admin:local-test-regression");
  assert.strictEqual(composed.spokenText, composed.reply);
  assert.strictEqual(composed.textSpeak, composed.reply);

  const indexSource = fs.readFileSync(path.join(root, "index.js"), "utf8");
  assert.match(indexSource, /marion\.adminTextRuntimeHandler\/2\.(?:0-private-primitive-serialization-hardlock|1-route-alias-private-identity-hardlock)/);
