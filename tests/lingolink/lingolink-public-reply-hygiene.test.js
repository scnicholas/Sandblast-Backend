...
/**
 * LingoLink Public Reply Hygiene Test
 *
 * Purpose:
 * Protects the user-facing response surface.
 *
 * LingoLink metadata may travel through the backend, but it must not be dumped
 * into the public reply text.
 *
 * This test confirms:
 * - LingoLink metadata is transport metadata.
...
const INTERNAL_LEAK_PATTERNS = Object.freeze([
  ...
  /\bLingoLinkTranslationAdvisor\b/i,
  /\bLingoLinkGateway\b/i,
  /\bLingoLinkNormalizer\b/i,
  /\bLingoLinkLanguageDetect\b/i,
  /\bLingoLinkGlossaryGuard\b/i,
  /\bMARION::FINAL::/i,
  /\bnyx\.marion\./i,
  /\bTypeError\b/i,
  /\bReferenceError\b/i,
...
describe("LingoLink Public Reply Hygiene", () => {
  test("leak detector catches known internal fields", () => {
    ...
    expect(hasInternalLeak("LingoLinkGateway runtimeTelemetry")).toBe(true);
  ...
  test("French gateway transport metadata does not create public reply leak", () => {
    const packet = runLingoLinkGateway("Bonjour, comment ca va?");
    const publicReply = choosePotentialPublicReply(packet);
  ...
  test("Spanish gateway transport metadata does not create public reply leak", () => {
    const packet = runLingoLinkGateway("Hola, como estas?");
    const publicReply = choosePotentialPublicReply(packet);
  ...
  test("protected glossary terms are carried as metadata, not public diagnostics", () => {
    const packet = runLingoLinkGateway("Bonjour, Marion utilise LingoLink.");

    expect(packet.glossaryMeta.foundInOriginal).toContain("Marion");
    expect(packet.glossaryMeta.foundInOriginal).toContain("LingoLink");
