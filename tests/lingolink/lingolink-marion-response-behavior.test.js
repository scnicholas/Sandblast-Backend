...
/**
 * LingoLink Marion Response Behavior Test
 *
 * Purpose:
 * Confirms Marion-facing LingoLink payloads preserve the correct behavioral contract:
 *
 * - LingoLink can provide advisory metadata.
...
function assertLingoLinkAdvisory(payload) {
...
describe("LingoLink Marion Response Behavior", () => {
  test("LingoLink English payload preserves Marion authority", () => {
    const payload = buildMarionBridgePayload("Hello, how are you today?");

    assertMarionAuthority(payload);
    assertLingoLinkAdvisory(payload);
  ...
  test("LingoLink French payload advises translation without taking authority", () => {
  ...
  test("LingoLink Spanish payload advises translation without taking authority", () => {
  ...
  test("LingoLink unknown-language fallback remains Marion-safe", () => {
  ...
  test("LingoLink glossary preservation remains available to Marion", () => {
    const result = runLingoLinkGateway("Bonjour, Marion utilise LingoLink.");

    expect(result).toBeDefined();

    assertMarionAuthority(result);
    assertLingoLinkAdvisory(result);
  ...
  test("Marion Bridge can receive French LingoLink payload when callable", async () => {
    const handler = resolveBridgeHandler(marionBridge);
    const payload = buildMarionBridgePayload("Bonjour, comment ca va?");
  ...
  test("Marion Bridge can receive Spanish LingoLink payload when callable", async () => {
    const handler = resolveBridgeHandler(marionBridge);
    const payload = buildMarionBridgePayload("Hola, como estas?");
  ...
  test("Marion-facing payload does not promote LingoLink to final authority", () => {
    const cases = [
