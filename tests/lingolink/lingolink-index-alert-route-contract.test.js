...
/**
 * LingoLink Index Alert Route Contract Test
 *
 * Purpose:
 * Confirms index.js contains the static integration surfaces needed to
 * transport LingoLink alert/scanner/correlation metadata safely.
 *
...
describe("LingoLink Index Alert Route Contract", () => {
  ...
  test("index.js contains LingoLink alert/scanner integration surfaces", () => {
    const source = readIndexSource();
  ...
  test("index route contract keeps LingoLink alert/scanner as transport metadata", () => {
    const payload = buildMarionBridgePayload("??? ###");
