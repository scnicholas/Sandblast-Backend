...
} = require('../../Data/marion/runtime/LingoLinkResponseEnvelope');

test('creates successful LingoLink response envelope', () => {
  const envelope = createLingoLinkResponseEnvelope({
    requestId: 'll_res_1',
    sourceLanguage: 'en',
    targetLanguage: 'fr',
    mode: 'translate',
  ...
  assert.equal(envelope.requestId, 'll_res_1');
...
test('creates fallback response', () => {
  const envelope = createLingoLinkFallbackResponse({
...
test('validates successful response envelope', () => {
  ...
  const validation = validateLingoLinkResponseEnvelope(envelope);
...
test('rejects successful response without finalText', () => {
  const validation = validateLingoLinkResponseEnvelope({
    ok: true,
    finalText: '',
    confidence: 0.9,
