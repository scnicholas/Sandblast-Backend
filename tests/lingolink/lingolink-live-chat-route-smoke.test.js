...
/**
 * LingoLink Live Chat Route Smoke Test
 *
 * Purpose:
 ...
 *
 * Enable with:
 *   $env:SB_LINGOLINK_LIVE_ROUTE_TEST="true"
 *
 * Optional:
 *   $env:SB_LINGOLINK_LIVE_BASE_URL="http://localhost:3000"
 *   $env:SB_LINGOLINK_LIVE_CHAT_PATH="/api/chat"
 *
 * Why optional:
...
const LIVE_ENABLED = process.env.SB_LINGOLINK_LIVE_ROUTE_TEST === "true";
const BASE_URL = process.env.SB_LINGOLINK_LIVE_BASE_URL || "http://localhost:3000";

const CHAT_PATH_CANDIDATES = [
  process.env.SB_LINGOLINK_LIVE_CHAT_PATH,
  "/api/chat",
  "/chat",
  "/api/marion/chat",
  "/api/nyx/chat",
...
async function postToFirstAvailableChatRoute(message) {
  ...
  for (const routePath of CHAT_PATH_CANDIDATES) {
    ...
    const response = await requestJson(targetUrl, {
      ...
      input: message,
      text: message,
      source: "lingolink-live-chat-route-smoke"
    });
...
describeLive("LingoLink Live Chat Route Smoke", () => {
