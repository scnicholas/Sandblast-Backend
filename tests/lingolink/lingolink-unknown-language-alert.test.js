...
/**
 * LingoLink Unknown Language Alert Test
 *
 * Purpose:
...
const {
  ...
  ALERT_VERSION
} = require("../../Data/marion/runtime/LingoLinkUnknownLanguageAlert");

describe("LingoLink Unknown Language Alert", () => {
  test("module exports alert functions", () => {
    ...
    expect(ALERT_VERSION).toBe("nyx.lingolink.unknownLanguageAlert/0.1");
  });
  ...
  test("builds unknown language alert packet", () => {
    ...
    expect(alert.authority.lingoLinkAdvisoryOnly).toBe(true);
  ...
  test("summary keeps alert metadata compact and Marion-safe", () => {
    ...
    expect(summary.source).toBe("LingoLinkUnknownLanguageAlert");
  });
