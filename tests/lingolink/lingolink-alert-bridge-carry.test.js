...
/**
 * LingoLink Alert Bridge Carry Test
 *
 * Purpose:
 * Confirms the LingoLink alert/scanner/correlation carry survives
...
describe("LingoLink Alert Bridge Carry", () => {
  ...
  test("Marion Bridge can receive unknown-language LingoLink alert payload when callable", async () => {
    const handler = resolveBridgeHandler(marionBridge);
    const payload = buildMarionBridgePayload("??? ###");
  ...
  test("Marion Bridge can receive supported-language LingoLink payloads when callable", async () => {
    const handler = resolveBridgeHandler(marionBridge);
