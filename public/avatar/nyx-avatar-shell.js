...
(function () {
  ...
  function setHeroSrc(avatar, src) {
    try {
      ...
      if (hero.dataset.baseSrc === src && hero.src) return;
      hero.dataset.baseSrc = src;
  ...
  window.NyxAvatarShell = {
    mount(rootEl, opts) {
      ...
      activeInstance = {
        ...
        triggerSettle() {
          try {
            avatar.wrap.classList.remove("is-settling");
            void avatar.wrap.offsetWidth;
            avatar.wrap.classList.add("is-settling");
            setTimeout(() => avatar.wrap.classList.remove("is-settling"), 360);
          } catch (_) {}
        },
        setHero(url) {
    ...
    applyDirective(directive) {
      if (activeInstance) activeInstance.applyDirective(directive);
    },
    triggerSettle() {
      if (activeInstance) activeInstance.triggerSettle();
    },
    getInstance() {
      return activeInstance;
    }
