...
(function attachLingoSentinelWidgetIntegrationHook(globalScope) {
  ...
  function normalizeInput(input) {
    ...
    return {
      requestId: cleanString(payload.requestId || createRequestId("phase10_widget")),
  ...
  function safeFailure(requestId, error) { return { requestId: cleanString(requestId), ok: false, error: cleanString(error || "LINGOSENTINEL_WIDGET_INTEGRATION_FAILED"), publicBoundary: true, internalRoutesExposed: false, tokensExposed: false, telemetryExposed: false }; }
  async function translateWidgetText(input, options) {
    const opts = options && typeof options === "object" ? options : {}; const payload = normalizeInput(input); const bridge = resolveBridge(opts);
    ...
    try { const result = await bridge.translate(payload, { endpoint: assertPublicEndpoint(opts.endpoint || DEFAULT_TRANSLATION_ENDPOINT, DEFAULT_TRANSLATION_ENDPOINT), requestId: payload.requestId }); return result && result.ok === true ? result : safeFailure(result && result.requestId || payload.requestId, result && result.error || "LINGOSENTINEL_WIDGET_INTEGRATION_TRANSLATION_FAILED"); }
    catch (error) { return safeFailure(payload.requestId, error && error.message || "LINGOSENTINEL_WIDGET_INTEGRATION_EXCEPTION"); }
  }
  function initializeIdentity(options) { const bridge = resolveBridge(options); return bridge && typeof bridge.getIdentity === "function" ? bridge.getIdentity(options) : null; }
  async function requestRealtimeToken(input, options) {
    const opts = options && typeof options === "object" ? options : {}; const bridge = resolveBridge(opts);
  ...
  async function initializeRealtime(input, options) {
    ...
    try { const result = await bridge.connectRealtime({ roomId: payload.roomId, mode: payload.mode, displayName: payload.displayName }, opts); return result && result.ok === true ? result : safeFailure(payload.requestId, result && result.error || "LINGOSENTINEL_REALTIME_INITIALIZATION_FAILED"); }
    catch (error) { return safeFailure(payload.requestId, error && error.message || "LINGOSENTINEL_REALTIME_INITIALIZATION_EXCEPTION"); }
  }
  async function sendEnglishMessage(input) {
    const client = globalScope.LingoSentinelPublicMessageClient;
    if (!client || typeof client.send !== "function") return safeFailure("", "LINGOSENTINEL_MESSAGE_CLIENT_UNAVAILABLE");
    try { return await client.send(input || {}); } catch (error) { return safeFailure("", error && error.message || "LINGOSENTINEL_MESSAGE_SEND_EXCEPTION"); }
  }
  function initializeConversation(options) {
    const controller = globalScope.LingoSentinelWidgetConversationController;
    if (!controller || typeof controller.create !== "function") return null;
    return controller.create(options || {});
  }
  function getRealtimeClient(options) { const bridge = resolveBridge(options); return bridge && typeof bridge.getRealtimeClient === "function" ? bridge.getRealtimeClient(options) : null; }
  function getApproxByteSize(value) { const text = String(value == null ? "" : value); return typeof TextEncoder !== "undefined" ? new TextEncoder().encode(text).length : text.length; }
  const hook = Object.freeze({
    version: "lingosentinel.widgetIntegrationHook/10C-layers1-7",
    endpoint: DEFAULT_TRANSLATION_ENDPOINT, tokenEndpoint: DEFAULT_TOKEN_ENDPOINT,
    translateWidgetText, initializeIdentity, requestRealtimeToken, initializeRealtime,
    sendEnglishMessage, initializeConversation, getRealtimeClient, normalizeInput,
    assertPublicEndpoint, createRequestId, getApproxByteSize
  });
